"""Tests for tools/deck/check_fifth_slice_combo_recipe_consistency.py (M52-05)."""
...
class TestFifthSliceComboRecipeConsistency(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.recipes = load_json(M52_RECIPE_DRAFTS)
        cls.validation = load_json(M52_VALIDATION)
        cls.report = build_fifth_slice_consistency_report(cls.recipes, cls.validation)

    def test_consistency_checks_all_fifth_slice_drafts(self):
        summary = self.report["summary"]

        self.assertEqual("M52-05", self.report["version"])
        self.assertTrue(summary["ready_for_m52_06"])
    ...
    def test_promotion_remains_blocked_by_human_selection(self):
        summary = self.report["summary"]
    ...
    def test_next_target_is_m52_06(self):
        next_target = self.report["next_target"]

        self.assertEqual("M52-06", next_target["milestone"])
        self.assertEqual("Fifth-slice blocker repair candidates", next_target["task"])

    def test_output_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp)
            json_path = out / "m52_05.json"
            md_path = out / "m52_05.md"

            write_json(self.report, json_path)
            write_markdown(self.report, md_path)

            loaded = json.loads(json_path.read_text(encoding="utf-8"))
            self.assertEqual("M52-05", loaded["version"])
            self.assertIn("M52-05 Fifth-Slice Combo-To-Recipe Consistency", md_path.read_text(encoding="utf-8"))
