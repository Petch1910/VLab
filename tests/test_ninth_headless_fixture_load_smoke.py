"""Tests for tools/deck/build_ninth_headless_fixture_load_smoke.py."""
...
import tests.test_ninth_fixture_deck_text_export as deck_text_fixture  # noqa: E402
...
class TestNinthHeadlessFixtureLoadSmoke(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        deck_text_fixture.TestNinthFixtureDeckTextExport.setUpClass()
        cls.fixture = deck_text_fixture.TestNinthFixtureDeckTextExport.fixture
        cls.m70_02_report = deck_text_fixture.TestNinthFixtureDeckTextExport.report
        cls.deck_text = deck_text_fixture.TestNinthFixtureDeckTextExport.deck_text
        cls.report = build_ninth_headless_fixture_load_smoke(cls.fixture, cls.m70_02_report, cls.deck_text)
    ...
    def test_offline_load_smoke_creates_deck_code(self):
        ...
        self.assertEqual("M70-03", self.report["version"])
        ...
        self.assertFalse(summary["ready_for_m70_04"])
    ...
    def test_count_line_parser_reads_main_ride_and_empty_g_sections(self):
        ...
        self.assertEqual("G Series Aqua Force Fixture (m68_recipe_001)", parsed["metadata"]["Name"])
    ...
    def test_deck_code_round_trips_to_vanguard_deck_shape(self):
        ...
        self.assertEqual("G Series Aqua Force Fixture (m68_recipe_001)", deck["name"])
    ...
    def test_scope_does_not_mutate_runtime_ui_bot_g_zone_stride_or_aqua_runtime(self):
        scope = self.report["scope"]

        self.assertTrue(scope["offline_headless_fixture_load_smoke"])
        self.assertTrue(scope["ninth_fixture_headless_load_smoke"])
    ...
    def test_m70_02_not_ready_blocks_smoke(self):
        report_input = copy.deepcopy(self.m70_02_report)
        report_input["summary"]["export_ready"] = False

        report = build_ninth_headless_fixture_load_smoke(self.fixture, report_input, self.deck_text)
        codes = [issue["code"] for issue in report["issues"]]

        self.assertFalse(report["summary"]["offline_load_ready"])
        self.assertFalse(report["summary"]["deck_code_created"])
        self.assertIn("m70_02_export_not_ready", codes)

    def test_non_empty_g_zone_blocks_smoke(self):
        deck_text = self.deck_text + "1 G-BT01-001TH\n"

        report = build_ninth_headless_fixture_load_smoke(self.fixture, self.m70_02_report, deck_text)
        codes = [issue["code"] for issue in report["issues"]]

        self.assertFalse(report["summary"]["offline_load_ready"])
        self.assertIn("g_zone_not_empty", codes)

    def test_fixture_deck_text_mismatch_blocks_smoke(self):
        deck_text = self.deck_text.replace("G-BT02-018TH", "G-BT02-999TH", 1)

        report = build_ninth_headless_fixture_load_smoke(self.fixture, self.m70_02_report, deck_text)
        codes = [issue["code"] for issue in report["issues"]]

        self.assertFalse(report["summary"]["offline_load_ready"])
        self.assertIn("fixture_deck_text_mismatch", codes)

    def test_unity_result_can_complete_m70_03_gate(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["summary"]["ready_for_m70_04"])

    def test_unity_rejected_result_blocks_m70_04(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(report["summary"]["ready_for_m70_04"])
    ...
    def test_output_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            json_path = out / "m70_03.json"
            md_path = out / "m70_03.md"
            ...
            self.assertEqual("M70-03", loaded["version"])
            self.assertNotIn("_deck_code", loaded)
            self.assertIn("M70-03 Ninth Fixture Headless Load Smoke", md_path.read_text(encoding="utf-8"))
