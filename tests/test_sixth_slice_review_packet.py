"""Tests for tools/deck/build_sixth_slice_review_packet.py."""
...
from tools.deck.build_sixth_slice_review_packet import (  # noqa: E402
    M55_01_SELECTION,
    M55_02_READINESS,
    M55_03_PROBE,
    M55_04_GATE,
    M56_01_SCAFFOLD,
    build_sixth_slice_review_packet,
    load_json,
    write_csv,
    write_json,
    write_markdown,
...
class TestSixthSliceReviewPacket(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.report = build_sixth_slice_review_packet(
            cls.selection,
            cls.readiness,
            cls.probe,
            cls.gate,
    ...
    def test_review_packet_ready_for_m56_03(self):
        summary = self.report["summary"]

        self.assertEqual("M56-02", self.report["version"])
        self.assertTrue(summary["ready_for_m56_03"])
        self.assertEqual(1, summary["fixture_scaffold_item_count"])
        self.assertEqual(11, summary["manual_card_item_count"])
        self.assertEqual(70, summary["candidate_edge_item_count"])
        self.assertEqual(82, summary["total_review_item_count"])

    def test_evidence_matches_probe_and_scaffold_counts(self):
        evidence = self.report["evidence_summary"]

        self.assertTrue(evidence["m55_04_offline_recipe_pipeline_allowed"])
        self.assertTrue(evidence["m55_04_runtime_promotion_closed"])
        self.assertTrue(evidence["m56_01_scaffold_ready"])
        self.assertEqual(77, evidence["probe_card_count"])
        self.assertEqual(2069, evidence["probe_edge_count"])
        self.assertEqual(11, evidence["expected_manual_review_count"])
        self.assertEqual(70, evidence["expected_candidate_edge_count"])
        self.assertEqual(11, evidence["rebuilt_manual_review_count"])
        self.assertEqual(70, evidence["rebuilt_candidate_edge_count"])
    ...
    def test_scope_and_policy_are_review_only(self):
        ...
        self.assertTrue(policy["ritual_retire_text_requires_manual_review"])
    ...
    def test_fixture_scaffold_item_blocks_recipe_until_reviewed(self):
        ...
        self.assertEqual("m56_01_fixture_scaffold", item["item_id"])
        ...
        self.assertTrue(item["ritual_or_retire_text_manual_review_required"])
        self.assertIn("m56_04_recipe_validator_uses_scaffold", item["blocked_until"])
    ...
    def test_candidate_edges_are_advisory_inputs(self):
        ...
        self.assertEqual(70, len(edges))
        self.assertEqual("candidate_edge", first["item_type"])
        self.assertEqual("synergy", first["status"])
        self.assertGreater(first["net_score"], 0)
        self.assertIn("m56_04_recipe_validator", first["blocked_until"])
        self.assertEqual("consider_for_m56_03_advisory_recipe_draft", first["recommended_next_action"])
    ...
    def test_runtime_or_saved_deck_flag_blocks_ready(self):
        ...
        self.assertFalse(report["summary"]["ready_for_m56_03"])
        self.assertFalse(report["evidence_summary"]["m55_04_runtime_promotion_closed"])

    def test_output_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp)
            json_path = out / "m56_02.json"
            md_path = out / "m56_02.md"
            csv_path = out / "m56_02.csv"
            ...
            self.assertEqual("M56-02", loaded["version"])
            self.assertIn("M56-02 Sixth-Slice Review Packet", md_path.read_text(encoding="utf-8"))
            with csv_path.open(encoding="utf-8") as handle:
                rows = list(csv.DictReader(handle))
            self.assertEqual(82, len(rows))
