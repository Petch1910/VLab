"""Tests for tools/deck/validate_sixth_slice_repaired_recipe.py (M57-05)."""
...
class TestSixthSliceRepairedRecipeValidation(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.g_zone_decision = build_sixth_slice_g_zone_stride_decision_artifact(
            cls.accepted_artifact,
            selected_option="main_deck_only_review_no_runtime_promotion",
        )
        cls.report = build_sixth_slice_repaired_validation_report(cls.accepted_artifact, cls.g_zone_decision)

    def test_repaired_recipe_validation_passes_and_opens_fixture_gate(self):
        summary = self.report["summary"]

        self.assertEqual("M57-05", self.report["version"])
        ...
        self.assertEqual("main_deck_only_review_no_runtime_promotion", summary["selected_g_zone_option_id"])
        ...
        self.assertTrue(summary["ready_for_m57_06"])
    ...
    def test_g_zone_boundary_is_main_deck_only_and_runtime_stays_disabled(self):
        policy = self.report["validation_policy"]
        context = self.report["g_zone_decision_context"]
        summary = self.report["summary"]
        consistency = self.report["repaired_recipe_consistency"]

        self.assertEqual("m57_03_repaired_quantity_preview", policy["validated_input"])
        ...
        self.assertEqual(["g_zone_support_deferred"], policy["suppressed_by_m57_04_boundary_issue_codes"])
        self.assertEqual("main_deck_only_review_no_runtime_promotion", context["selected_option_id"])
    ...
    def test_consistency_passes_for_pair_cards(self):
        ...
        self.assertEqual("m56_recipe_001", consistency["recipe_id"])
        ...
        self.assertEqual("G-BT12-062TH", consistency["source_card_id"])
        self.assertEqual("G-BT12-066TH", consistency["target_card_id"])
    ...
    def test_scope_does_not_promote_runtime_fixture(self):
        ...
        self.assertFalse(scope["changes_g_zone_decision_artifact"])
    ...
    def test_deferred_g_zone_decision_blocks_fixture_gate(self):
        deferred_decision = build_sixth_slice_g_zone_stride_decision_artifact(
            self.accepted_artifact,
            selected_option="defer_until_g_zone_runtime_exists",
        )

        report = build_sixth_slice_repaired_validation_report(self.accepted_artifact, deferred_decision)

        self.assertFalse(report["summary"]["main_deck_only_boundary_applied"])
        self.assertFalse(report["summary"]["ready_for_m57_06"])
    ...
    def test_missing_acceptance_blocks_fixture_gate_even_if_counts_pass(self):
        ...
        decision = build_sixth_slice_g_zone_stride_decision_artifact(
            accepted,
            selected_option="main_deck_only_review_no_runtime_promotion",
        )

        report = build_sixth_slice_repaired_validation_report(accepted, decision)

        self.assertEqual(1, report["summary"]["validator_passed_count"])
        self.assertFalse(report["summary"]["human_acceptance_recorded"])
        self.assertFalse(report["summary"]["ready_for_m57_06"])

    def test_next_target_is_m57_06(self):
        next_target = self.report["next_target"]

        self.assertEqual("M57-06", next_target["milestone"])
        self.assertEqual("Sixth-slice runtime fixture promotion gate", next_target["task"])

    def test_output_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp)
            json_path = out / "m57_05.json"
            md_path = out / "m57_05.md"

            write_json(self.report, json_path)
            write_markdown(self.report, md_path)

            loaded = json.loads(json_path.read_text(encoding="utf-8"))
            self.assertEqual("M57-05", loaded["version"])
            self.assertIn("M57-05 Sixth-Slice Repaired Recipe Validation Rerun", md_path.read_text(encoding="utf-8"))
