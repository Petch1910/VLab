"""Tests for tools/deck/build_fifth_slice_semantic_compatibility_probe.py."""
...
from tools.deck.build_fifth_slice_semantic_compatibility_probe import (  # noqa: E402
    M51_01_SELECTION,
    M51_02_READINESS,
    build_fifth_slice_semantic_compatibility_probe,
    load_json,
    write_json,
    write_markdown,
)
...
class TestFifthSliceSemanticCompatibilityProbe(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.selection = load_json(M51_01_SELECTION)
        cls.readiness = load_json(M51_02_READINESS)
        cls.report = build_fifth_slice_semantic_compatibility_probe(cls.selection, cls.readiness)

    def test_probe_targets_fifth_slice(self):
        selected = self.report["selected_target"]
        summary = self.report["summary"]

        self.assertEqual("M51-03", self.report["version"])
    ...
    def test_normalization_reuses_selected_slice_probe_without_writing_intermediates(self):
        ...
        self.assertTrue(normalization["maps_m51_readiness_to_semantic_scaleout_ready"])
    ...
    def test_all_stage_readiness_flags_pass(self):
        ...
        self.assertTrue(readiness["fifth_slice_semantic_compatibility_probe_passed"])
        self.assertTrue(readiness["ready_for_m51_04"])
    ...
    def test_stage_summaries_match_source_backed_probe(self):
        ...
        self.assertEqual(106, summary["source_card_count"])
        self.assertEqual(106, summary["semantic_card_count"])
        self.assertEqual(4, summary["manual_review_card_count"])
        self.assertEqual(3075, summary["pair_graph_edge_count"])
        self.assertEqual(142, summary["candidate_edge_count"])
        self.assertEqual(106, stages["b2_semantic_tags"]["cards_with_any_semantic_tag"])
        self.assertEqual(95, stages["b3_requirement_provider"]["cards_with_providers"])
        self.assertEqual(184, stages["c5_compatibility"]["status_counts"]["manual_review_required"])
        self.assertEqual(142, stages["c5_compatibility"]["status_counts"]["synergy"])
    ...
    def test_failed_readiness_routes_to_repair(self):
        ...
        report = build_fifth_slice_semantic_compatibility_probe(self.selection, readiness)

        self.assertFalse(report["readiness"]["ready_for_m51_04"])
        self.assertEqual("M51-repair-semantic", report["next_target"]["milestone"])

    def test_next_target_is_m51_04(self):
        self.assertEqual("M51-04", self.report["next_target"]["milestone"])
        self.assertEqual("Fifth-slice recipe pipeline entry gate", self.report["next_target"]["task"])

    def test_output_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp)
            json_path = out / "m51_03.json"
            md_path = out / "m51_03.md"

            write_json(self.report, json_path)
            write_markdown(self.report, md_path)

            loaded = json.loads(json_path.read_text(encoding="utf-8"))
            self.assertEqual("M51-03", loaded["version"])
            self.assertIn("M51-03 Fifth-Slice Semantic", md_path.read_text(encoding="utf-8"))
