"""Tests for tools/deck/build_eighth_slice_recipe_pipeline_entry_gate.py."""
...
import tests.test_eighth_slice_semantic_compatibility_probe as eighth_probe_fixture  # noqa: E402
from tools.deck.build_eighth_slice_recipe_pipeline_entry_gate import (  # noqa: E402
    build_eighth_slice_recipe_pipeline_entry_gate,
    write_json,
    write_markdown,
)


class TestEighthSliceRecipePipelineEntryGate(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        eighth_probe_fixture.TestEighthSliceSemanticCompatibilityProbe.setUpClass()
        cls.readiness = eighth_probe_fixture.TestEighthSliceSemanticCompatibilityProbe.readiness
        cls.probe = eighth_probe_fixture.TestEighthSliceSemanticCompatibilityProbe.report
        cls.report = build_eighth_slice_recipe_pipeline_entry_gate(cls.readiness, cls.probe)

    def test_gate_allows_offline_m64_pipeline(self):
        summary = self.report["summary"]
        decision = self.report["decision"]

        self.assertEqual("M63-04", self.report["version"])
        ...
        self.assertTrue(summary["ready_for_m64"])
        self.assertTrue(decision["offline_recipe_pipeline_allowed"])
        self.assertEqual("M64 Eighth-slice offline recipe pipeline", decision["recommended_next_queue"])

    def test_evidence_matches_m63_02_and_m63_03_outputs(self):
        ...
        self.assertEqual(121, evidence["semantic_card_count"])
        self.assertEqual(6, evidence["manual_review_card_count"])
        self.assertEqual(3398, evidence["pair_graph_edge_count"])
        self.assertEqual(355, evidence["candidate_edge_count"])
        self.assertEqual("requires_eighth_slice_fixture_scaffold", evidence["policy_reuse_decision"])
        self.assertTrue(evidence["new_format_or_mechanic_fixtures_required"])
        self.assertFalse(evidence["g_zone_fixture_boundary_required"])

    def test_requires_fixture_scaffold_before_recipe_validation(self):
        decision = self.report["decision"]
        queue = self.report["proposed_m64_queue"]

        self.assertTrue(decision["fixture_scaffold_required_before_recipe_validation"])
        self.assertEqual("M64-01", queue[0]["milestone"])
        self.assertEqual("Eighth-slice fixture scaffold", queue[0]["task"])
        self.assertEqual("M64-closeout", queue[-1]["milestone"])
    ...
    def test_failed_fixture_readiness_blocks_gate(self):
        ...
        report = build_eighth_slice_recipe_pipeline_entry_gate(readiness, self.probe)
        codes = [issue["code"] for issue in report["issues"]]

        self.assertFalse(report["summary"]["ready_for_m64"])
    ...
    def test_failed_probe_blocks_gate(self):
        ...
        probe["readiness"]["ready_for_m63_04"] = False

        report = build_eighth_slice_recipe_pipeline_entry_gate(self.readiness, probe)
        codes = [issue["code"] for issue in report["issues"]]
    ...
    def test_next_target_is_m64_01(self):
        self.assertEqual("M64-01", self.report["next_target"]["milestone"])
        self.assertEqual("Eighth-slice fixture scaffold", self.report["next_target"]["task"])

    def test_output_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp)
            json_path = out / "m63_04.json"
            md_path = out / "m63_04.md"

            write_json(self.report, json_path)
            write_markdown(self.report, md_path)

            loaded = json.loads(json_path.read_text(encoding="utf-8"))
            self.assertEqual("M63-04", loaded["version"])
            self.assertIn("M63-04 Eighth-Slice Recipe", md_path.read_text(encoding="utf-8"))
