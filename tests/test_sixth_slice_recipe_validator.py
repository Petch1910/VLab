"""Tests for tools/deck/validate_sixth_slice_recipe_drafts.py (M56-04)."""
...
class TestSixthSliceRecipeValidator(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.recipe_report = load_json(RECIPE_DRAFTS)
        cls.card_rows = load_card_rows(_all_card_ids(cls.recipe_report))
        cls.report = build_sixth_slice_validation_report(cls.recipe_report, cls.card_rows)

    def test_validator_runs_all_sixth_slice_drafts(self):
        summary = self.report["summary"]

        self.assertEqual("M56-04", self.report["version"])
        self.assertTrue(summary["ready_for_m56_05"])
    ...
    def test_manual_review_overlap_blocks_runtime_readiness(self):
        ...
        self.assertEqual(12, summary["blocked_by_manual_review_count"])
        self.assertEqual(12, summary["manual_review_overlap_recipe_count"])
        self.assertEqual(12, summary["grade_profile_review_recipe_count"])
    ...
    def test_issue_counts_are_stable(self):
        ...
        self.assertEqual(12, issue_counts["manual_review_card_overlap"])
        self.assertEqual(12, issue_counts["grade_profile_review"])
        self.assertEqual(12, issue_counts["g_zone_support_deferred"])
        self.assertEqual(12, issue_counts["human_recipe_selection_pending"])

    def test_next_target_is_m56_05(self):
        next_target = self.report["next_target"]

        self.assertEqual("M56-05", next_target["milestone"])
        self.assertEqual("Sixth-slice combo-to-recipe consistency", next_target["task"])

    def test_output_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp)
            json_path = out / "m56_04.json"
            md_path = out / "m56_04.md"

            write_json(self.report, json_path)
            write_markdown(self.report, md_path)

            loaded = json.loads(json_path.read_text(encoding="utf-8"))
            self.assertEqual("M56-04", loaded["version"])
            self.assertIn("M56-04 Sixth-Slice Recipe Validation Report", md_path.read_text(encoding="utf-8"))
