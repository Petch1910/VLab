"""Tests for tools/deck/validate_ninth_slice_repaired_recipe.py (M69-05)."""
...
import tests.test_ninth_slice_system_decision_artifact as decision_fixture  # noqa: E402
from tools.deck.build_ninth_slice_system_decision_artifact import (  # noqa: E402
    build_ninth_slice_system_decision_artifact,
)
from tools.deck.validate_ninth_slice_repaired_recipe import (  # noqa: E402
    build_ninth_slice_repaired_validation_report,
    write_json,
    write_markdown,
)
...
class TestNinthSliceRepairedRecipeValidation(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        decision_fixture.TestNinthSliceSystemDecisionArtifact.setUpClass()
        cls.accepted_artifact = decision_fixture.TestNinthSliceSystemDecisionArtifact.accepted_artifact
        cls.system_decision = build_ninth_slice_system_decision_artifact(
        ...
        cls.report = build_ninth_slice_repaired_validation_report(cls.accepted_artifact, cls.system_decision)

    def test_repaired_recipe_validation_passes_and_opens_fixture_gate(self):
        summary = self.report["summary"]

        self.assertEqual("M69-05", self.report["version"])
        self.assertEqual("m68_recipe_001", summary["selected_recipe_id"])
        self.assertEqual("m69_01_m68_recipe_001_repair_review", summary["selected_review_item_id"])
        ...
        self.assertTrue(summary["ready_for_m69_06"])
    ...
    def test_system_boundary_is_main_deck_only_and_runtime_stays_disabled(self):
        ...
        self.assertEqual("m69_03_repaired_quantity_preview", policy["validated_input"])
        self.assertTrue(policy["human_selection_required"])
        self.assertTrue(policy["human_acceptance_required"])
        self.assertTrue(policy["repair_acceptance_required"])
        self.assertTrue(policy["system_boundary_required"])
        self.assertTrue(policy["main_deck_only_validation"])
        self.assertTrue(policy["must_not_enable_g_zone_or_stride_runtime"])
        self.assertTrue(policy["must_not_enable_aqua_force_battle_order_runtime"])
        self.assertEqual(
            [
            ...
            policy["suppressed_by_m69_04_boundary_issue_codes"],
        )
        self.assertEqual(G_OPTION, context["selected_g_zone_option_id"])
        self.assertEqual(STRIDE_OPTION, context["selected_stride_option_id"])
        self.assertEqual(AQUA_OPTION, context["selected_aqua_force_option_id"])
        self.assertTrue(context["main_deck_only_boundary_applied"])
        self.assertTrue(context["g_zone_boundary_applied"])
        self.assertTrue(context["stride_boundary_applied"])
        self.assertTrue(context["aqua_force_boundary_applied"])
        self.assertFalse(context["g_zone_runtime_enabled"])
        self.assertFalse(context["stride_runtime_enabled"])
        self.assertFalse(context["aqua_force_battle_order_runtime_enabled"])
        ...
        self.assertEqual(0, summary["stride_deferred_recipe_count"])
        self.assertEqual(0, summary["aqua_force_battle_order_deferred_recipe_count"])
        self.assertFalse(consistency["g_zone_support_deferred"])
        self.assertFalse(consistency["stride_support_deferred"])
        self.assertFalse(consistency["aqua_force_battle_order_support_deferred"])

    def test_consistency_passes_for_pair_cards(self):
        consistency = self.report["repaired_recipe_consistency"]

        self.assertEqual("m68_recipe_001", consistency["recipe_id"])
        ...
        self.assertEqual("G-BT02-070TH", consistency["source_card_id"])
        self.assertEqual("G-CB02-015TH", consistency["target_card_id"])
    ...
    def test_deferred_g_zone_decision_blocks_fixture_gate(self):
        deferred_decision = build_ninth_slice_system_decision_artifact(
        ...
        report = build_ninth_slice_repaired_validation_report(self.accepted_artifact, deferred_decision)

        self.assertFalse(report["summary"]["main_deck_only_boundary_applied"])
        self.assertFalse(report["summary"]["g_zone_boundary_applied"])
        self.assertTrue(report["summary"]["stride_boundary_applied"])
        self.assertTrue(report["summary"]["aqua_force_boundary_applied"])
        self.assertFalse(report["summary"]["ready_for_m69_06"])
        self.assertEqual(1, report["summary"]["validator_passed_count"])
        self.assertEqual(1, report["summary"]["g_zone_deferred_recipe_count"])
        self.assertEqual(0, report["summary"]["stride_deferred_recipe_count"])
        self.assertEqual(0, report["summary"]["aqua_force_battle_order_deferred_recipe_count"])
    ...
    def test_deferred_stride_decision_blocks_fixture_gate(self):
        deferred_decision = build_ninth_slice_system_decision_artifact(
            self.accepted_artifact,
            selected_g_zone_option=G_OPTION,
        ...
        self.assertFalse(report["summary"]["stride_boundary_applied"])
        self.assertTrue(report["summary"]["aqua_force_boundary_applied"])
        self.assertFalse(report["summary"]["ready_for_m69_06"])
        self.assertEqual(1, report["summary"]["validator_passed_count"])
        self.assertEqual(0, report["summary"]["g_zone_deferred_recipe_count"])
        self.assertEqual(1, report["summary"]["stride_deferred_recipe_count"])
        self.assertEqual(0, report["summary"]["aqua_force_battle_order_deferred_recipe_count"])
        self.assertEqual(["stride_support_deferred"], report["repaired_recipe_validation"]["review_codes"])
        self.assertFalse(report["summary"]["runtime_promotion_allowed"])

    def test_deferred_aqua_force_decision_blocks_fixture_gate(self):
        deferred_decision = build_ninth_slice_system_decision_artifact(
            self.accepted_artifact,
            selected_g_zone_option=G_OPTION,
            selected_stride_option=STRIDE_OPTION,
        ...
        self.assertTrue(report["summary"]["g_zone_boundary_applied"])
        self.assertTrue(report["summary"]["stride_boundary_applied"])
        self.assertFalse(report["summary"]["aqua_force_boundary_applied"])
        self.assertFalse(report["summary"]["ready_for_m69_06"])
        self.assertEqual(1, report["summary"]["validator_passed_count"])
        self.assertEqual(0, report["summary"]["g_zone_deferred_recipe_count"])
        self.assertEqual(0, report["summary"]["stride_deferred_recipe_count"])
        self.assertEqual(1, report["summary"]["aqua_force_battle_order_deferred_recipe_count"])
        self.assertEqual(
            ["aqua_force_battle_order_support_deferred"],
    ...
    def test_missing_acceptance_blocks_fixture_gate_even_if_counts_pass(self):
        ...
        decision = build_ninth_slice_system_decision_artifact(
            accepted,
            selected_g_zone_option=G_OPTION,
            selected_stride_option=STRIDE_OPTION,
            selected_aqua_force_option=AQUA_OPTION,
        ...
        report = build_ninth_slice_repaired_validation_report(accepted, decision)

        self.assertEqual(1, report["summary"]["validator_passed_count"])
        self.assertFalse(report["summary"]["human_acceptance_recorded"])
        self.assertFalse(report["summary"]["ready_for_m69_06"])

    def test_next_target_is_m69_06(self):
        next_target = self.report["next_target"]

        self.assertEqual("M69-06", next_target["milestone"])
        self.assertEqual("Ninth-slice runtime fixture promotion gate", next_target["task"])

    def test_output_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp)
            json_path = out / "m69_05.json"
            md_path = out / "m69_05.md"

            write_json(self.report, json_path)
            write_markdown(self.report, md_path)

            loaded = json.loads(json_path.read_text(encoding="utf-8"))
            self.assertEqual("M69-05", loaded["version"])
            self.assertIn("M69-05 Ninth-Slice Repaired Recipe Validation Rerun", md_path.read_text(encoding="utf-8"))
