"""Tests for tools/deck/build_eighth_slice_semantic_compatibility_probe.py."""
...
import tests.test_eighth_slice_fixture_readiness as eighth_readiness_fixture  # noqa: E402
from tools.deck.build_eighth_slice_semantic_compatibility_probe import (  # noqa: E402
    build_eighth_slice_semantic_compatibility_probe,
    write_json,
    write_markdown,
)


class TestEighthSliceSemanticCompatibilityProbe(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        eighth_readiness_fixture.TestEighthSliceFixtureReadiness.setUpClass()
        cls.selection = eighth_readiness_fixture.TestEighthSliceFixtureReadiness.selection
        cls.readiness = eighth_readiness_fixture.TestEighthSliceFixtureReadiness.report
        cls.report = build_eighth_slice_semantic_compatibility_probe(cls.selection, cls.readiness)

    def test_probe_targets_eighth_slice(self):
        selected = self.report["selected_target"]
        summary = self.report["summary"]

        self.assertEqual("M63-03", self.report["version"])
    ...
    def test_normalization_reuses_selected_slice_probe_without_writing_intermediates(self):
        ...
        self.assertTrue(normalization["maps_m63_readiness_to_semantic_scaleout_ready"])
    ...
    def test_all_stage_readiness_flags_pass(self):
        ...
        self.assertTrue(readiness["eighth_slice_semantic_compatibility_probe_passed"])
        self.assertTrue(readiness["ready_for_m63_04"])
    ...
    def test_stage_summaries_match_source_backed_probe(self):
        ...
        self.assertEqual(121, summary["source_card_count"])
        self.assertEqual(121, summary["semantic_card_count"])
        self.assertEqual(6, summary["manual_review_card_count"])
        self.assertEqual(3398, summary["pair_graph_edge_count"])
        self.assertEqual(355, summary["candidate_edge_count"])
        ...
        self.assertEqual(106, stages["b3_requirement_provider"]["cards_with_providers"])
        self.assertEqual(summary["candidate_edge_count"], stages["c5_compatibility"]["status_counts"]["synergy"])
        self.assertEqual(256, stages["c5_compatibility"]["status_counts"]["manual_review_required"])
        self.assertEqual(444, stages["c5_compatibility"]["status_counts"]["missing_data"])
        self.assertEqual(2343, stages["c5_compatibility"]["status_counts"]["mixed"])
    ...
    def test_failed_readiness_routes_to_repair(self):
        ...
        report = build_eighth_slice_semantic_compatibility_probe(self.selection, readiness)

        self.assertFalse(report["readiness"]["ready_for_m63_04"])
        self.assertEqual("M63-repair-semantic", report["next_target"]["milestone"])

    def test_next_target_is_m63_04(self):
        self.assertEqual("M63-04", self.report["next_target"]["milestone"])
        self.assertEqual("Eighth-slice recipe pipeline entry gate", self.report["next_target"]["task"])

    def test_output_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp)
            json_path = out / "m63_03.json"
            md_path = out / "m63_03.md"

            write_json(self.report, json_path)
            write_markdown(self.report, md_path)

            loaded = json.loads(json_path.read_text(encoding="utf-8"))
            self.assertEqual("M63-03", loaded["version"])
            self.assertIn("M63-03 Eighth-Slice Semantic", md_path.read_text(encoding="utf-8"))
