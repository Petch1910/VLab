"""Tests for tools/deck/export_seventh_fixture_deck_text.py."""
...
import tests.test_seventh_runtime_fixture_schema_validator as schema_fixture  # noqa: E402
from tools.deck.export_seventh_fixture_deck_text import (  # noqa: E402
    build_seventh_fixture_deck_text_export,
    write_deck_text,
    write_json,
    write_markdown,
)
...
class TestSeventhFixtureDeckTextExport(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        schema_fixture.TestSeventhRuntimeFixtureSchemaValidator.setUpClass()
        cls.fixture = schema_fixture.TestSeventhRuntimeFixtureSchemaValidator.fixture
        cls.validation_report = schema_fixture.TestSeventhRuntimeFixtureSchemaValidator.report
        cls.report = build_seventh_fixture_deck_text_export(cls.fixture, cls.validation_report)
        cls.deck_text = cls.report["_deck_text"]

    def test_export_is_ready_for_seventh_headless_fixture_smoke(self):
        summary = self.report["summary"]
        fixture = self.report["fixture_summary"]

        self.assertEqual("M62-02", self.report["version"])
        self.assertTrue(summary["export_ready"])
        self.assertEqual(0, summary["blocking_issue_count"])
        self.assertTrue(summary["ready_for_m62_03"])
    ...
    def test_text_uses_count_line_deck_codec_shape(self):
        ...
        self.assertIn("Name: G Series Neo Nectar Fixture (m60_recipe_001)", deck_text)
        ...
        self.assertIn("Bloom/token runtime is disabled", deck_text)
        self.assertIn("BloomTokenBoundary: manual_semantic_review_only_no_runtime_promotion", deck_text)
    ...
    def test_scope_does_not_mutate_runtime_ui_bot_or_system_runtime(self):
        ...
        self.assertTrue(scope["seventh_fixture_deck_text_export"])
    ...
    def test_invalid_m62_01_validation_blocks_export(self):
        validation_report = copy.deepcopy(self.validation_report)
        validation_report["summary"]["schema_valid"] = False
        validation_report["summary"]["ready_for_m62_02"] = False

        report = build_seventh_fixture_deck_text_export(self.fixture, validation_report)
        codes = [issue["code"] for issue in report["issues"]]

        self.assertFalse(report["summary"]["export_ready"])
        self.assertFalse(report["summary"]["ready_for_m62_03"])
        self.assertEqual("", report["_deck_text"])
        self.assertIn("m62_01_validation_not_ready", codes)

    def test_malformed_main_deck_row_blocks_export(self):
        fixture = copy.deepcopy(self.fixture)
        fixture["main_deck"][0]["quantity"] = 0

        report = build_seventh_fixture_deck_text_export(fixture, self.validation_report)
        codes = [issue["code"] for issue in report["issues"]]
    ...
    def test_output_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp)
            text_path = out / "seventh_fixture.txt"
            json_path = out / "seventh_fixture.json"
            md_path = out / "seventh_fixture.md"
            ...
            self.assertEqual("M62-02", loaded["version"])
            self.assertNotIn("_deck_text", loaded)
            self.assertIn("M62-02 Seventh Fixture Deck Text Export", md_path.read_text(encoding="utf-8"))
