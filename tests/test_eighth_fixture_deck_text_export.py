"""Tests for tools/deck/export_eighth_fixture_deck_text.py."""
...
import tests.test_eighth_runtime_fixture_schema_validator as schema_fixture  # noqa: E402
from tools.deck.export_eighth_fixture_deck_text import (  # noqa: E402
    build_eighth_fixture_deck_text_export,
    write_deck_text,
    write_json,
    write_markdown,
)
...
class TestEighthFixtureDeckTextExport(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        schema_fixture.TestEighthRuntimeFixtureSchemaValidator.setUpClass()
        cls.fixture = schema_fixture.TestEighthRuntimeFixtureSchemaValidator.fixture
        cls.validation_report = schema_fixture.TestEighthRuntimeFixtureSchemaValidator.report
        cls.report = build_eighth_fixture_deck_text_export(cls.fixture, cls.validation_report)
        cls.deck_text = cls.report["_deck_text"]

    def test_export_is_ready_for_eighth_headless_fixture_smoke(self):
        summary = self.report["summary"]
        fixture = self.report["fixture_summary"]

        self.assertEqual("M66-02", self.report["version"])
        self.assertTrue(summary["export_ready"])
        self.assertEqual(0, summary["blocking_issue_count"])
        self.assertTrue(summary["ready_for_m66_03"])
    ...
    def test_text_uses_count_line_deck_codec_shape(self):
        ...
        self.assertIn("Name: Link Joker/Legion Mate Kagero Fixture (m64_recipe_001)", deck_text)
        ...
        self.assertIn("Lock/Unlock runtime is disabled", deck_text)
        self.assertIn("Legion/Mate runtime is disabled", deck_text)
        self.assertIn("LockBoundary: main_deck_only_review_no_runtime_promotion", deck_text)
        self.assertIn("LegionBoundary: main_deck_only_review_no_runtime_promotion", deck_text)
    ...
    def test_scope_does_not_mutate_runtime_ui_bot_or_system_runtime(self):
        ...
        self.assertTrue(scope["eighth_fixture_deck_text_export"])
    ...
    def test_invalid_m66_01_validation_blocks_export(self):
        validation_report = copy.deepcopy(self.validation_report)
        validation_report["summary"]["schema_valid"] = False
        validation_report["summary"]["ready_for_m66_02"] = False

        report = build_eighth_fixture_deck_text_export(self.fixture, validation_report)
        codes = [issue["code"] for issue in report["issues"]]

        self.assertFalse(report["summary"]["export_ready"])
        self.assertFalse(report["summary"]["ready_for_m66_03"])
        self.assertEqual("", report["_deck_text"])
        self.assertIn("m66_01_validation_not_ready", codes)

    def test_malformed_main_deck_row_blocks_export(self):
        fixture = copy.deepcopy(self.fixture)
        fixture["main_deck"][0]["quantity"] = 0

        report = build_eighth_fixture_deck_text_export(fixture, self.validation_report)
        codes = [issue["code"] for issue in report["issues"]]
    ...
    def test_output_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp)
            text_path = out / "eighth_fixture.txt"
            json_path = out / "eighth_fixture.json"
            md_path = out / "eighth_fixture.md"
            ...
            self.assertEqual("M66-02", loaded["version"])
            self.assertNotIn("_deck_text", loaded)
            self.assertIn("M66-02 Eighth Fixture Deck Text Export", md_path.read_text(encoding="utf-8"))
