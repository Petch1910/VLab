"""Tests for tools/deck/build_eighth_slice_recipe_draft_model.py (M64-03)."""
...
import tests.test_eighth_slice_review_packet as eighth_packet_fixture  # noqa: E402
from tools.deck.build_eighth_slice_recipe_draft_model import (  # noqa: E402
    build_eighth_slice_recipe_drafts,
    write_json,
    write_markdown,
)


class TestEighthSliceRecipeDraftModel(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        eighth_packet_fixture.TestEighthSliceReviewPacket.setUpClass()
        cls.packet = eighth_packet_fixture.TestEighthSliceReviewPacket.report
        cls.scaffold = eighth_packet_fixture.TestEighthSliceReviewPacket.scaffold
        cls.report = build_eighth_slice_recipe_drafts(cls.packet, cls.scaffold)

    def test_recipe_drafts_are_ready_for_validator(self):
        summary = self.report["summary"]

        self.assertEqual("M64-03", self.report["version"])
        self.assertTrue(summary["ready_for_m64_04"])
        self.assertEqual(355, summary["candidate_edge_input_count"])
        self.assertEqual(5, summary["candidate_edges_skipped_for_trigger_grade4_or_missing"])
    ...
    def test_scope_is_offline_draft_only(self):
        ...
        self.assertTrue(policy["pair_anchored_from_m64_02_candidate_edges"])
        ...
        self.assertTrue(policy["grade4_cards_excluded_until_format_support"])
        self.assertTrue(policy["lock_legion_text_requires_manual_review"])
    ...
    def test_lock_legion_gates_keep_drafts_out_of_runtime(self):
        for recipe in self.report["recipe_drafts"]:
            ...
            self.assertIn("m64_04_recipe_validator", recipe["review_blockers"])
            self.assertIn("m64_05_combo_recipe_consistency_check", recipe["review_blockers"])
            self.assertIn("lock_legion_manual_review_policy", recipe["review_blockers"])
            self.assertIn("grade4_format_support_decision", recipe["review_blockers"])
            ...
            self.assertTrue(metadata["lock_runtime_support_deferred"])
            self.assertTrue(metadata["legion_runtime_support_deferred"])
            self.assertTrue(metadata["draft_only"])
            self.assertTrue(metadata["requires_m64_04_validator"])
    ...
    def test_skipped_candidate_edges_explain_trigger_grade4_or_missing(self):
        self.assertEqual(5, len(self.report["skipped_candidate_edges"]))
    ...
    def test_next_target_is_m64_04(self):
        next_target = self.report["next_target"]

        self.assertEqual("M64-04", next_target["milestone"])
        self.assertEqual("Eighth-slice recipe validator", next_target["task"])

    def test_output_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp)
            json_path = out / "m64_03.json"
            md_path = out / "m64_03.md"

            write_json(self.report, json_path)
            write_markdown(self.report, md_path)

            loaded = json.loads(json_path.read_text(encoding="utf-8"))
            self.assertEqual("M64-03", loaded["version"])
            self.assertIn("M64-03 Eighth-Slice Recipe Draft Model", md_path.read_text(encoding="utf-8"))
