"""Tests for tools/deck/build_sixth_slice_human_repair_review_packet.py (M57-01)."""
...
from tools.deck.build_sixth_slice_human_repair_review_packet import (  # noqa: E402
    M56_CLOSEOUT,
    M56_DRAFTS,
    M56_REPAIRS,
    build_sixth_slice_human_repair_review_packet,
    load_json,
    write_csv,
    write_json,
    write_markdown,
...
class TestSixthSliceHumanRepairReviewPacket(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.closeout = load_json(M56_CLOSEOUT)
        cls.repairs = load_json(M56_REPAIRS)
        cls.drafts = load_json(M56_DRAFTS)
        cls.report = build_sixth_slice_human_repair_review_packet(
            cls.closeout,
            cls.repairs,
            cls.drafts,
        )

    def test_packet_exports_all_sixth_slice_repair_candidates(self):
        summary = self.report["summary"]

        self.assertEqual("M57-01", self.report["version"])
        self.assertEqual(12, summary["review_item_count"])
        self.assertEqual(12, summary["ready_for_human_repair_review_count"])
        self.assertEqual(12, summary["complete_manual_repair_candidate_count"])
        self.assertEqual(12, summary["complete_grade_profile_candidate_count"])
        self.assertEqual(12, summary["g_zone_deferred_item_count"])
    ...
    def test_first_item_carries_pair_manual_grade_and_g_zone_context(self):
        item = self.report["review_items"][0]
        pair = item["pair"]

        self.assertEqual("m57_01_m56_recipe_001_repair_review", item["review_item_id"])
        self.assertEqual("sixth_slice_human_repair_and_g_zone_review", item["item_type"])
        self.assertEqual("m56_recipe_001", item["recipe_id"])
        self.assertEqual("G-BT12-062TH->G-BT12-066TH", item["source_candidate_edge"])
        ...
        self.assertEqual("m56_recipe_001_manual_overlap_pkg_001", item["manual_overlap_package_id"])
        ...
        self.assertEqual("m56_recipe_001_grade_profile_pkg_001", item["grade_profile_package_id"])
        ...
        self.assertEqual("m56_recipe_001_g_zone_deferred_pkg_001", item["g_zone_package_id"])
    ...
    def test_each_item_requires_human_decision_and_stays_non_runtime(self):
        for item in self.report["review_items"]:
            ...
            self.assertTrue(item["g_zone_deferred"])
            self.assertFalse(item["g_zone_can_be_repaired_in_m57_01"])
            ...
            self.assertEqual(
                "select_one_recipe_review_manual_grade_repairs_and_record_g_zone_decision_before_m57_02",
    ...
    def test_grade_packages_are_complete_source_backed_substitutions(self):
        ...
        for item in self.report["review_items"]:
            ...
            for card in item["grade_removals"]:
                self.assertEqual("m56_03_sixth_slice_recipe_draft_model", card["source"])

    def test_decision_options_do_not_record_selection_acceptance_or_g_zone_decision(self):
        item = self.report["review_items"][0]
        ...
        self.assertEqual(
            [
                ...
                "request_different_repair_or_g_zone_scope",
    ...
    def test_scope_and_policy_do_not_record_selection_or_acceptance(self):
        ...
        self.assertTrue(policy["m57_02_must_record_explicit_selection"])
        self.assertTrue(policy["m57_03_must_record_explicit_acceptance"])
        self.assertTrue(policy["m57_04_must_record_g_zone_decision"])
        self.assertTrue(policy["m57_05_must_rerun_validation"])

    def test_missing_closeout_gate_blocks_m57_02_readiness(self):
        ...
        self.assertFalse(report["summary"]["ready_for_m57_02"])

    def test_next_target_is_m57_02(self):
        next_target = self.report["next_target"]

        self.assertEqual("M57-02", next_target["milestone"])
        self.assertEqual("Sixth-slice human-selected recipe artifact", next_target["task"])

    def test_output_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp)
            json_path = out / "m57_01.json"
            md_path = out / "m57_01.md"
            csv_path = out / "m57_01.csv"
            ...
            self.assertEqual("M57-01", loaded["version"])
            self.assertIn("M57-01 Sixth-Slice Human Repair Review Packet", md_path.read_text(encoding="utf-8"))
            rows = list(csv.DictReader(csv_path.read_text(encoding="utf-8").splitlines()))
            self.assertEqual(12, len(rows))
            self.assertEqual("m56_recipe_001", rows[0]["recipe_id"])
