"""Tests for tools/deck/build_fifth_headless_fixture_load_smoke.py."""
...
from tools.deck.build_fifth_headless_fixture_load_smoke import (  # noqa: E402
    DEFAULT_DECK_TEXT,
    DEFAULT_FIXTURE,
    DEFAULT_M54_02_REPORT,
    HEADLESS_RULESET,
    HEADLESS_SEED,
    _deck_from_deck_code,
    build_fifth_headless_fixture_load_smoke,
    load_json,
...
class TestFifthHeadlessFixtureLoadSmoke(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.fixture = load_json(DEFAULT_FIXTURE)
        cls.m54_02_report = load_json(DEFAULT_M54_02_REPORT)
        cls.deck_text = DEFAULT_DECK_TEXT.read_text(encoding="utf-8")
        cls.report = build_fifth_headless_fixture_load_smoke(cls.fixture, cls.m54_02_report, cls.deck_text)
    ...
    def test_offline_load_smoke_creates_deck_code(self):
        ...
        self.assertEqual("M54-03", self.report["version"])
    ...
    def test_count_line_parser_reads_main_ride_and_empty_g_sections(self):
        ...
        self.assertEqual("Link Joker Legion Gold Paladin Fixture (m52_recipe_001)", parsed["metadata"]["Name"])
        self.assertEqual("link_joker_legion_mate", parsed["metadata"]["Format"])
        self.assertEqual("vanguard_th", parsed["metadata"]["PackId"])
        self.assertEqual(16, len(parsed["zones"]["Main"]))
    ...
    def test_deck_code_round_trips_to_vanguard_deck_shape(self):
        ...
        self.assertEqual("Link Joker Legion Gold Paladin Fixture (m52_recipe_001)", deck["name"])
        ...
        self.assertEqual(16, len(deck["main"]))
    ...
    def test_scope_does_not_mutate_runtime_ui_bot_or_g_zone(self):
        ...
        self.assertTrue(scope["fifth_fixture_headless_load_smoke"])
    ...
    def test_m54_02_not_ready_blocks_smoke(self):
        report_input = copy.deepcopy(self.m54_02_report)
        report_input["summary"]["export_ready"] = False

        report = build_fifth_headless_fixture_load_smoke(self.fixture, report_input, self.deck_text)
        codes = [issue["code"] for issue in report["issues"]]

        self.assertFalse(report["summary"]["offline_load_ready"])
        self.assertFalse(report["summary"]["deck_code_created"])
        self.assertIn("m54_02_export_not_ready", codes)

    def test_non_empty_g_zone_blocks_smoke(self):
        deck_text = self.deck_text + "1 G-BT01-001TH\n"

        report = build_fifth_headless_fixture_load_smoke(self.fixture, self.m54_02_report, deck_text)
        codes = [issue["code"] for issue in report["issues"]]

        self.assertFalse(report["summary"]["offline_load_ready"])
        self.assertIn("g_zone_not_empty", codes)

    def test_unity_result_can_complete_m54_03_gate(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["summary"]["ready_for_m54_04"])

    def test_unity_rejected_result_blocks_m54_04(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(report["summary"]["ready_for_m54_04"])
    ...
    def test_output_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            json_path = out / "m54_03.json"
            md_path = out / "m54_03.md"
            ...
            self.assertEqual("M54-03", loaded["version"])
            self.assertNotIn("_deck_code", loaded)
            self.assertIn("M54-03 Fifth Fixture Headless Load Smoke", md_path.read_text(encoding="utf-8"))
