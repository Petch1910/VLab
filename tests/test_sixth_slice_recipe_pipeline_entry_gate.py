"""Tests for tools/deck/build_sixth_slice_recipe_pipeline_entry_gate.py."""
...
from tools.deck.build_sixth_slice_recipe_pipeline_entry_gate import (  # noqa: E402
    M55_02_READINESS,
    M55_03_PROBE,
    build_sixth_slice_recipe_pipeline_entry_gate,
    load_json,
    write_json,
    write_markdown,
)
...
class TestSixthSliceRecipePipelineEntryGate(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.readiness = load_json(M55_02_READINESS)
        cls.probe = load_json(M55_03_PROBE)
        cls.report = build_sixth_slice_recipe_pipeline_entry_gate(cls.readiness, cls.probe)

    def test_gate_allows_offline_m56_pipeline(self):
        summary = self.report["summary"]
        decision = self.report["decision"]

        self.assertEqual("M55-04", self.report["version"])
        ...
        self.assertTrue(summary["ready_for_m56"])
        self.assertTrue(decision["offline_recipe_pipeline_allowed"])
        self.assertEqual("M56 Sixth-slice offline recipe pipeline", decision["recommended_next_queue"])

    def test_evidence_matches_m55_02_and_m55_03_outputs(self):
        ...
        self.assertEqual(2069, evidence["pair_graph_edge_count"])
        self.assertEqual(70, evidence["candidate_edge_count"])
        self.assertEqual("requires_sixth_slice_fixture_scaffold", evidence["policy_reuse_decision"])

    def test_requires_fixture_scaffold_before_recipe_validation(self):
        decision = self.report["decision"]
        queue = self.report["proposed_m56_queue"]

        self.assertTrue(decision["fixture_scaffold_required_before_recipe_validation"])
        self.assertEqual("M56-01", queue[0]["milestone"])
        self.assertEqual("Sixth-slice fixture scaffold", queue[0]["task"])
        self.assertEqual("M56-closeout", queue[-1]["milestone"])
    ...
    def test_failed_fixture_readiness_blocks_gate(self):
        ...
        report = build_sixth_slice_recipe_pipeline_entry_gate(readiness, self.probe)
        codes = [issue["code"] for issue in report["issues"]]

        self.assertFalse(report["summary"]["ready_for_m56"])
    ...
    def test_failed_probe_blocks_gate(self):
        ...
        probe["readiness"]["ready_for_m55_04"] = False

        report = build_sixth_slice_recipe_pipeline_entry_gate(self.readiness, probe)
        codes = [issue["code"] for issue in report["issues"]]
    ...
    def test_next_target_is_m56_01(self):
        self.assertEqual("M56-01", self.report["next_target"]["milestone"])
        self.assertEqual("Sixth-slice fixture scaffold", self.report["next_target"]["task"])

    def test_output_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp)
            json_path = out / "m55_04.json"
            md_path = out / "m55_04.md"

            write_json(self.report, json_path)
            write_markdown(self.report, md_path)

            loaded = json.loads(json_path.read_text(encoding="utf-8"))
            self.assertEqual("M55-04", loaded["version"])
            self.assertIn("M55-04 Sixth-Slice Recipe", md_path.read_text(encoding="utf-8"))
