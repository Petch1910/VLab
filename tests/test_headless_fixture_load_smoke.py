"""Tests for tools/deck/build_headless_fixture_load_smoke.py (M39-03)."""
...
from tools.deck.build_headless_fixture_load_smoke import (  # noqa: E402
    DEFAULT_DECK_TEXT,
    DEFAULT_FIXTURE,
    DEFAULT_M39_02_REPORT,
    HEADLESS_RULESET,
    HEADLESS_SEED,
    build_headless_fixture_load_smoke,
    load_json,
...
class TestHeadlessFixtureLoadSmoke(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.fixture = load_json(DEFAULT_FIXTURE)
        cls.m39_02_report = load_json(DEFAULT_M39_02_REPORT)
        cls.deck_text = DEFAULT_DECK_TEXT.read_text(encoding="utf-8")
        cls.report = build_headless_fixture_load_smoke(cls.fixture, cls.m39_02_report, cls.deck_text)
    ...
    def test_offline_load_smoke_creates_deck_code(self):
        ...
        self.assertEqual("M39-03", self.report["version"])
        ...
        self.assertFalse(summary["ready_for_m39_04"])
    ...
    def test_count_line_parser_reads_main_ride_and_g_sections(self):
        ...
        self.assertEqual("Classic Core Nova Grappler Fixture (recipe_003)", parsed["metadata"]["Name"])
        self.assertEqual("classic_part1", parsed["metadata"]["Format"])
        self.assertEqual("vanguard_th", parsed["metadata"]["PackId"])
        self.assertEqual(17, len(parsed["zones"]["Main"]))
    ...
    def test_deck_code_round_trips_to_vanguard_deck_shape(self):
        ...
        self.assertEqual("Classic Core Nova Grappler Fixture (recipe_003)", deck["name"])
        ...
        self.assertEqual(17, len(deck["main"]))
    ...
    def test_m39_02_not_ready_blocks_smoke(self):
        report_input = copy.deepcopy(self.m39_02_report)
        report_input["summary"]["export_ready"] = False

        report = build_headless_fixture_load_smoke(self.fixture, report_input, self.deck_text)
        codes = [issue["code"] for issue in report["issues"]]

        self.assertFalse(report["summary"]["offline_load_ready"])
        self.assertFalse(report["summary"]["deck_code_created"])
        self.assertIn("m39_02_export_not_ready", codes)

    def test_unity_result_can_complete_m39_03_gate(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["summary"]["ready_for_m39_04"])

    def test_unity_rejected_result_blocks_m39_04(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(report["summary"]["ready_for_m39_04"])
    ...
    def test_output_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            json_path = out / "m39_03.json"
            md_path = out / "m39_03.md"
            ...
            self.assertEqual("M39-03", loaded["version"])
            self.assertNotIn("_deck_code", loaded)
            self.assertIn("M39-03 Headless Fixture Load Smoke", md_path.read_text(encoding="utf-8"))
