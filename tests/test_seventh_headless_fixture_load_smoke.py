"""Tests for tools/deck/build_seventh_headless_fixture_load_smoke.py."""
...
import tests.test_seventh_fixture_deck_text_export as deck_text_fixture  # noqa: E402
...
class TestSeventhHeadlessFixtureLoadSmoke(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        deck_text_fixture.TestSeventhFixtureDeckTextExport.setUpClass()
        cls.fixture = deck_text_fixture.TestSeventhFixtureDeckTextExport.fixture
        cls.m62_02_report = deck_text_fixture.TestSeventhFixtureDeckTextExport.report
        cls.deck_text = deck_text_fixture.TestSeventhFixtureDeckTextExport.deck_text
        cls.report = build_seventh_headless_fixture_load_smoke(cls.fixture, cls.m62_02_report, cls.deck_text)
    ...
    def test_offline_load_smoke_creates_deck_code(self):
        ...
        self.assertEqual("M62-03", self.report["version"])
        ...
        self.assertFalse(summary["ready_for_m62_04"])
    ...
    def test_count_line_parser_reads_main_ride_and_empty_g_sections(self):
        ...
        self.assertEqual("G Series Neo Nectar Fixture (m60_recipe_001)", parsed["metadata"]["Name"])
    ...
    def test_deck_code_round_trips_to_vanguard_deck_shape(self):
        ...
        self.assertEqual("G Series Neo Nectar Fixture (m60_recipe_001)", deck["name"])
    ...
    def test_scope_does_not_mutate_runtime_ui_bot_g_zone_or_bloom_token(self):
        scope = self.report["scope"]

        self.assertTrue(scope["offline_headless_fixture_load_smoke"])
        self.assertTrue(scope["seventh_fixture_headless_load_smoke"])
    ...
    def test_m62_02_not_ready_blocks_smoke(self):
        report_input = copy.deepcopy(self.m62_02_report)
        report_input["summary"]["export_ready"] = False

        report = build_seventh_headless_fixture_load_smoke(self.fixture, report_input, self.deck_text)
        codes = [issue["code"] for issue in report["issues"]]

        self.assertFalse(report["summary"]["offline_load_ready"])
        self.assertFalse(report["summary"]["deck_code_created"])
        self.assertIn("m62_02_export_not_ready", codes)

    def test_non_empty_g_zone_blocks_smoke(self):
        deck_text = self.deck_text + "1 G-BT01-001TH\n"

        report = build_seventh_headless_fixture_load_smoke(self.fixture, self.m62_02_report, deck_text)
        codes = [issue["code"] for issue in report["issues"]]

        self.assertFalse(report["summary"]["offline_load_ready"])
        self.assertIn("g_zone_not_empty", codes)

    def test_unity_result_can_complete_m62_03_gate(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["summary"]["ready_for_m62_04"])

    def test_unity_rejected_result_blocks_m62_04(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(report["summary"]["ready_for_m62_04"])
    ...
    def test_output_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            json_path = out / "m62_03.json"
            md_path = out / "m62_03.md"
            ...
            self.assertEqual("M62-03", loaded["version"])
            self.assertNotIn("_deck_code", loaded)
            self.assertIn("M62-03 Seventh Fixture Headless Load Smoke", md_path.read_text(encoding="utf-8"))
