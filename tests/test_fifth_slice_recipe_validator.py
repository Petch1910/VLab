"""Tests for tools/deck/validate_fifth_slice_recipe_drafts.py (M52-04)."""
...
class TestFifthSliceRecipeValidator(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.recipe_report = load_json(RECIPE_DRAFTS)
        cls.card_rows = load_card_rows(_all_card_ids(cls.recipe_report))
        cls.report = build_fifth_slice_validation_report(cls.recipe_report, cls.card_rows)

    def test_validator_runs_all_fifth_slice_drafts(self):
        summary = self.report["summary"]

        self.assertEqual("M52-04", self.report["version"])
        self.assertTrue(summary["ready_for_m52_05"])
    ...
    def test_human_selection_and_grade_review_gate_runtime_readiness(self):
        summary = self.report["summary"]
    ...
    def test_each_recipe_has_expected_counts_without_blockers(self):
    ...
    def test_next_target_is_m52_05(self):
        next_target = self.report["next_target"]

        self.assertEqual("M52-05", next_target["milestone"])
        self.assertEqual("Fifth-slice combo-to-recipe consistency", next_target["task"])

    def test_output_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp)
            json_path = out / "m52_04.json"
            md_path = out / "m52_04.md"

            write_json(self.report, json_path)
            write_markdown(self.report, md_path)

            loaded = json.loads(json_path.read_text(encoding="utf-8"))
            self.assertEqual("M52-04", loaded["version"])
            self.assertIn("M52-04 Fifth-Slice Recipe Validation Report", md_path.read_text(encoding="utf-8"))
