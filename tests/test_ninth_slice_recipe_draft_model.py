"""Tests for tools/deck/build_ninth_slice_recipe_draft_model.py (M68-03)."""
...
import tests.test_ninth_slice_review_packet as ninth_packet_fixture  # noqa: E402
from tools.deck.build_ninth_slice_recipe_draft_model import (  # noqa: E402
    build_ninth_slice_recipe_drafts,
    write_json,
    write_markdown,
)


class TestNinthSliceRecipeDraftModel(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ninth_packet_fixture.TestNinthSliceReviewPacket.setUpClass()
        cls.packet = ninth_packet_fixture.TestNinthSliceReviewPacket.report
        cls.scaffold = ninth_packet_fixture.TestNinthSliceReviewPacket.scaffold
        cls.report = build_ninth_slice_recipe_drafts(cls.packet, cls.scaffold)

    def test_recipe_drafts_are_ready_for_validator(self):
        summary = self.report["summary"]

        self.assertEqual("M68-03", self.report["version"])
        self.assertTrue(summary["ready_for_m68_04"])
        self.assertEqual(95, summary["candidate_edge_input_count"])
        self.assertEqual(1, summary["candidate_edges_skipped_for_trigger_grade4_or_missing"])
    ...
    def test_scope_is_offline_draft_only(self):
        ...
        self.assertTrue(policy["pair_anchored_from_m68_02_candidate_edges"])
        ...
        self.assertTrue(policy["aqua_force_battle_order_text_requires_manual_review"])
    ...
    def test_g_zone_stride_and_battle_order_gates_keep_drafts_out_of_runtime(self):
        for recipe in self.report["recipe_drafts"]:
            ...
            self.assertIn("m68_04_recipe_validator", recipe["review_blockers"])
            self.assertIn("m68_05_combo_recipe_consistency_check", recipe["review_blockers"])
            self.assertIn("g_zone_stride_manual_review_policy", recipe["review_blockers"])
            self.assertIn("aqua_force_battle_order_manual_review_policy", recipe["review_blockers"])
            self.assertIn("grade4_g_zone_support_decision", recipe["review_blockers"])
            ...
            self.assertTrue(metadata["stride_support_deferred"])
            self.assertTrue(metadata["aqua_force_battle_order_support_deferred"])
            self.assertTrue(metadata["draft_only"])
            self.assertTrue(metadata["requires_m68_04_validator"])
    ...
    def test_skipped_candidate_edges_explain_trigger_grade4_or_missing(self):
        self.assertEqual(1, len(self.report["skipped_candidate_edges"]))
        for item in self.report["skipped_candidate_edges"]:
    ...
    def test_next_target_is_m68_04(self):
        next_target = self.report["next_target"]

        self.assertEqual("M68-04", next_target["milestone"])
        self.assertEqual("Ninth-slice recipe validator", next_target["task"])

    def test_output_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp)
            json_path = out / "m68_03.json"
            md_path = out / "m68_03.md"

            write_json(self.report, json_path)
            write_markdown(self.report, md_path)

            loaded = json.loads(json_path.read_text(encoding="utf-8"))
            self.assertEqual("M68-03", loaded["version"])
            self.assertIn("M68-03 Ninth-Slice Recipe Draft Model", md_path.read_text(encoding="utf-8"))
