"""Tests for tools/deck/validate_second_runtime_fixture_schema.py."""
...
from tools.deck.validate_second_runtime_fixture_schema import (  # noqa: E402
    DEFAULT_FIXTURE,
    build_second_runtime_fixture_schema_validation_report,
    load_json,
    write_json,
    write_markdown,
)
...
class TestSecondRuntimeFixtureSchemaValidator(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.fixture = load_json(DEFAULT_FIXTURE)
        cls.report = build_second_runtime_fixture_schema_validation_report(cls.fixture)

    def test_valid_second_fixture_passes_schema_validation(self):
        summary = self.report["summary"]
        fixture = self.report["fixture_summary"]

        self.assertEqual("M42-01", self.report["version"])
        ...
        self.assertTrue(summary["ready_for_m42_02"])
    ...
    def test_scope_does_not_mutate_runtime_ui_or_bot(self):
        ...
        self.assertTrue(scope["second_fixture_schema_validator"])
    ...
    def test_invalid_runtime_boundary_blocks_fixture(self):
        ...
        report = build_second_runtime_fixture_schema_validation_report(fixture)
        codes = [issue["code"] for issue in report["issues"]]

        self.assertFalse(report["summary"]["schema_valid"])
        self.assertIn("runtime_boundary_violation", codes)
        self.assertFalse(report["summary"]["ready_for_m42_02"])
    ...
    def test_next_target_is_m42_02(self):
        next_target = self.report["next_target"]

        self.assertEqual("M42-02", next_target["milestone"])
        self.assertEqual("Second fixture deck text exporter", next_target["task"])

    def test_output_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp)
            json_path = out / "m42_01.json"
            md_path = out / "m42_01.md"

            write_json(self.report, json_path)
            write_markdown(self.report, md_path)

            loaded = json.loads(json_path.read_text(encoding="utf-8"))
            self.assertEqual("M42-01", loaded["version"])
            self.assertIn("M42-01 Second Runtime Fixture Schema Validation", md_path.read_text(encoding="utf-8"))
