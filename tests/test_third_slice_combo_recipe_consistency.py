"""Tests for tools/deck/check_third_slice_combo_recipe_consistency.py (M44-05)."""
...
from tools.deck.check_third_slice_combo_recipe_consistency import (  # noqa: E402
    M44_RECIPE_DRAFTS,
    M44_VALIDATION,
    build_third_slice_consistency_report,
    load_json,
    write_json,
    write_markdown,
)
...
class TestThirdSliceComboRecipeConsistency(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.recipes = load_json(M44_RECIPE_DRAFTS)
        cls.validation = load_json(M44_VALIDATION)
        cls.report = build_third_slice_consistency_report(cls.recipes, cls.validation)

    def test_consistency_checks_all_third_slice_drafts(self):
        summary = self.report["summary"]

        self.assertEqual("M44-05", self.report["version"])
        self.assertTrue(summary["ready_for_m44_06"])
    ...
    def test_next_target_is_m44_06(self):
        next_target = self.report["next_target"]

        self.assertEqual("M44-06", next_target["milestone"])
        self.assertEqual("Third-slice blocker repair candidates", next_target["task"])

    def test_output_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp)
            json_path = out / "m44_05.json"
            md_path = out / "m44_05.md"

            write_json(self.report, json_path)
            write_markdown(self.report, md_path)

            loaded = json.loads(json_path.read_text(encoding="utf-8"))
            self.assertEqual("M44-05", loaded["version"])
            self.assertIn("M44-05 Third-Slice Combo-To-Recipe Consistency", md_path.read_text(encoding="utf-8"))
