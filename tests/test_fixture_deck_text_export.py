"""Tests for tools/deck/export_fixture_deck_text.py (M39-02)."""
...
from tools.deck.export_fixture_deck_text import (  # noqa: E402
    DEFAULT_FIXTURE,
    DEFAULT_VALIDATION_REPORT,
    build_fixture_deck_text_export,
    load_json,
    write_deck_text,
    write_json,
    write_markdown,
...
class TestFixtureDeckTextExport(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.fixture = load_json(DEFAULT_FIXTURE)
        cls.validation_report = load_json(DEFAULT_VALIDATION_REPORT)
        cls.report = build_fixture_deck_text_export(cls.fixture, cls.validation_report)
        cls.deck_text = cls.report["_deck_text"]

    def test_export_is_ready_for_headless_fixture_smoke(self):
        summary = self.report["summary"]
        fixture = self.report["fixture_summary"]

        self.assertEqual("M39-02", self.report["version"])
        self.assertTrue(summary["export_ready"])
        self.assertEqual(0, summary["blocking_issue_count"])
        self.assertTrue(summary["ready_for_m39_03"])
    ...
    def test_text_uses_count_line_deck_codec_shape(self):
        ...
        self.assertIn("Name: Classic Core Nova Grappler Fixture (recipe_003)", deck_text)
    ...
    def test_card_lines_sum_to_fixture_main_deck_count(self):
        ...
        self.assertEqual("4 BT01-029TH", lines[0])
    ...
    def test_review_comments_do_not_change_importable_card_lines(self):
        ...
        self.assertTrue(any("บลูทัล แจ็ค" in line for line in comment_lines))
    ...
    def test_invalid_m39_01_validation_blocks_export(self):
        validation_report = copy.deepcopy(self.validation_report)
        validation_report["summary"]["schema_valid"] = False
        validation_report["summary"]["ready_for_m39_02"] = False

        report = build_fixture_deck_text_export(self.fixture, validation_report)
        codes = [issue["code"] for issue in report["issues"]]

        self.assertFalse(report["summary"]["export_ready"])
        self.assertFalse(report["summary"]["ready_for_m39_03"])
        self.assertEqual("", report["_deck_text"])
        self.assertIn("m39_01_validation_not_ready", codes)

    def test_output_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp)
            text_path = out / "fixture.txt"
            json_path = out / "fixture.json"
            md_path = out / "fixture.md"
            ...
            self.assertEqual("M39-02", loaded["version"])
            self.assertNotIn("_deck_text", loaded)
            self.assertIn("M39-02 Fixture Deck Text Export", md_path.read_text(encoding="utf-8"))
