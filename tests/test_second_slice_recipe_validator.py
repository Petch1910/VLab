"""Tests for tools/deck/validate_second_slice_recipe_drafts.py (M40-03)."""
...
from tools.deck.validate_second_slice_recipe_drafts import (  # noqa: E402
    RECIPE_DRAFTS,
    _all_card_ids,
    build_second_slice_validation_report,
    load_card_rows,
    load_json,
    write_json,
    write_markdown,
...
class TestSecondSliceRecipeValidator(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.recipe_report = load_json(RECIPE_DRAFTS)
        cls.card_rows = load_card_rows(_all_card_ids(cls.recipe_report))
        cls.report = build_second_slice_validation_report(cls.recipe_report, cls.card_rows)

    def test_validator_runs_all_second_slice_drafts(self):
        summary = self.report["summary"]

        self.assertEqual("M40-03", self.report["version"])
        self.assertTrue(summary["ready_for_m40_04"])
    ...
    def test_next_target_is_m40_04(self):
        next_target = self.report["next_target"]

        self.assertEqual("M40-04", next_target["milestone"])
        self.assertEqual("Second-slice combo-to-recipe consistency", next_target["task"])

    def test_output_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp)
            json_path = out / "m40_03.json"
            md_path = out / "m40_03.md"

            write_json(self.report, json_path)
            write_markdown(self.report, md_path)

            loaded = json.loads(json_path.read_text(encoding="utf-8"))
            self.assertEqual("M40-03", loaded["version"])
            self.assertIn("M40-03 Second-Slice Recipe Validation Report", md_path.read_text(encoding="utf-8"))
