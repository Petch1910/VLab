"""Tests for tools/deck/build_third_slice_recipe_draft_model.py (M44-03)."""
...
from tools.deck.build_third_slice_recipe_draft_model import (  # noqa: E402
    M44_REVIEW_PACKET,
    M44_SCAFFOLD,
    build_third_slice_recipe_drafts,
    load_json,
    write_json,
    write_markdown,
)
...
class TestThirdSliceRecipeDraftModel(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.packet = load_json(M44_REVIEW_PACKET)
        cls.scaffold = load_json(M44_SCAFFOLD)
        cls.report = build_third_slice_recipe_drafts(cls.packet, cls.scaffold)

    def test_recipe_drafts_are_ready_for_validator(self):
        summary = self.report["summary"]

        self.assertEqual("M44-03", self.report["version"])
        self.assertTrue(summary["ready_for_m44_04"])
        self.assertEqual(109, summary["candidate_edge_input_count"])
    ...
    def test_scope_is_offline_draft_only(self):
        ...
        self.assertTrue(policy["pair_anchored_from_m44_02_candidate_edges"])
    ...
    def test_manual_overlap_keeps_drafts_out_of_runtime(self):
        for recipe in self.report["recipe_drafts"]:
            ...
            self.assertIn("m44_04_recipe_validator", recipe["review_blockers"])
            self.assertIn("m44_05_combo_recipe_consistency_check", recipe["review_blockers"])
            ...
            self.assertTrue(metadata["requires_m44_04_validator"])
    ...
    def test_next_target_is_m44_04(self):
        next_target = self.report["next_target"]

        self.assertEqual("M44-04", next_target["milestone"])
        self.assertEqual("Third-slice recipe validator", next_target["task"])

    def test_output_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp)
            json_path = out / "m44_03.json"
            md_path = out / "m44_03.md"

            write_json(self.report, json_path)
            write_markdown(self.report, md_path)

            loaded = json.loads(json_path.read_text(encoding="utf-8"))
            self.assertEqual("M44-03", loaded["version"])
            self.assertIn("M44-03 Third-Slice Recipe Draft Model", md_path.read_text(encoding="utf-8"))
