"""Tests for tools/deck/export_fifth_fixture_deck_text.py."""
...
from tools.deck.export_fifth_fixture_deck_text import (  # noqa: E402
    DEFAULT_FIXTURE,
    DEFAULT_VALIDATION_REPORT,
    build_fifth_fixture_deck_text_export,
    load_json,
    write_deck_text,
    write_json,
    write_markdown,
...
class TestFifthFixtureDeckTextExport(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.fixture = load_json(DEFAULT_FIXTURE)
        cls.validation_report = load_json(DEFAULT_VALIDATION_REPORT)
        cls.report = build_fifth_fixture_deck_text_export(cls.fixture, cls.validation_report)
        cls.deck_text = cls.report["_deck_text"]

    def test_export_is_ready_for_fifth_headless_fixture_smoke(self):
        summary = self.report["summary"]
        fixture = self.report["fixture_summary"]

        self.assertEqual("M54-02", self.report["version"])
        self.assertTrue(summary["export_ready"])
        self.assertEqual(0, summary["blocking_issue_count"])
        self.assertTrue(summary["ready_for_m54_03"])
        self.assertEqual("runtime_fixture_m52_recipe_001_gold_paladin_m53_05", fixture["fixture_id"])
        self.assertEqual("m52_recipe_001", fixture["recipe_id"])
    ...
    def test_text_uses_count_line_deck_codec_shape(self):
        ...
        self.assertIn("Name: Link Joker Legion Gold Paladin Fixture (m52_recipe_001)", deck_text)
    ...
    def test_card_lines_sum_to_fixture_main_deck_count(self):
        ...
        self.assertEqual("3 BT10-011TH", lines[0])
    ...
    def test_scope_does_not_mutate_runtime_ui_or_bot(self):
        ...
        self.assertTrue(scope["fifth_fixture_deck_text_export"])
    ...
    def test_invalid_m54_01_validation_blocks_export(self):
        validation_report = copy.deepcopy(self.validation_report)
        validation_report["summary"]["schema_valid"] = False
        validation_report["summary"]["ready_for_m54_02"] = False

        report = build_fifth_fixture_deck_text_export(self.fixture, validation_report)
        codes = [issue["code"] for issue in report["issues"]]

        self.assertFalse(report["summary"]["export_ready"])
        self.assertFalse(report["summary"]["ready_for_m54_03"])
        self.assertEqual("", report["_deck_text"])
        self.assertIn("m54_01_validation_not_ready", codes)

    def test_output_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp)
            text_path = out / "fifth_fixture.txt"
            json_path = out / "fifth_fixture.json"
            md_path = out / "fifth_fixture.md"
            ...
            self.assertEqual("M54-02", loaded["version"])
            self.assertNotIn("_deck_text", loaded)
            self.assertIn("M54-02 Fifth Fixture Deck Text Export", md_path.read_text(encoding="utf-8"))
