"""Build M44-03 third-slice advisory recipe drafts."""
...
M44_REVIEW_PACKET = OUTPUT_DIR / "m44_02_third_slice_review_packet.json"
...
def _selected_cards(scaffold: dict[str, Any], sqlite_path: Path = CARDS_SQLITE) -> dict[str, dict[str, Any]]:
    ...
    query = (
        ...
        f"({placeholders}) and coalesce(deck_limit, 4) > 0"
    )
        ...
        rows.append(
            {
                ...
                "quantity_source": "m44_03_fixture_scaffold_plus_candidate_edge_pair",
            }
        )
    return rows
...
def _build_recipe(
    ...
    review_blockers = [
        ...
        "m44_04_recipe_validator",
        "m44_05_combo_recipe_consistency_check",
    ]
    if manual_overlap:
        review_blockers.append("manual_card_semantic_review")
    return {
        "recipe_id": f"m44_recipe_{index:03d}",
        "source_candidate_edge": edge["edge"],
        "source_edge_rank": edge["edge_rank"],
        "source_line_id": edge["edge"],
        "source_package_id": "m44_03_pair_anchored_fixture_scaffold",
        ...
        "validation_metadata": {
            "draft_only": True,
            "requires_m44_04_validator": True,
...
def build_third_slice_recipe_drafts(
    review_packet: dict[str, Any] | None = None,
    scaffold: dict[str, Any] | None = None,
    draft_limit: int = DEFAULT_DRAFT_LIMIT,
) -> dict[str, Any]:
    review_packet = review_packet or load_json(M44_REVIEW_PACKET)
    scaffold = scaffold or load_json(M44_SCAFFOLD)
    if not review_packet.get("summary", {}).get("ready_for_m44_03"):
        raise ValueError("M44-02 review packet is not ready for M44-03.")
    if not scaffold.get("summary", {}).get("ready_for_m44_02"):
        raise ValueError("M44-01 fixture scaffold is not ready for M44-03.")
    card_rows = _selected_cards(scaffold)
    ...
    return {
        "version": "M44-03",
        "description": "Third-slice advisory recipe draft model from M44-02 candidate edges",
        "selected_target": review_packet.get("selected_target", {}),
        "source_inputs": {
            "third_slice_review_packet": str(M44_REVIEW_PACKET.relative_to(ROOT)),
        ...
        "draft_policy": {
            "pair_anchored_from_m44_02_candidate_edges": True,
            "fixture_scaffold_source": "m44_01_third_slice_fixture_scaffold",
        ...
        "summary": {
            ...
            "ready_for_m44_04": bool(recipes) and quantity_complete_count == len(recipes),
        },
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M44-04: `{summary['ready_for_m44_04']}`",
        "",
        "## Drafts",
        "",
    ]
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M44-03 third-slice recipe draft model.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_third_slice_recipe_drafts(draft_limit=args.draft_limit)
    json_path = args.output_dir / "m44_03_third_slice_recipe_draft_model.json"
    md_path = args.output_dir / "m44_03_third_slice_recipe_draft_model.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M44-03 recipe draft model wrote {json_path}")
    print(f"M44-03 recipe draft model summary wrote {md_path}")
    print(
        "ready={ready} drafts={drafts} complete={complete}".format(
            ready=report["summary"]["ready_for_m44_04"],
            drafts=report["summary"]["recipe_draft_count"],
            complete=report["summary"]["quantity_complete_recipe_count"],
        )
    )
    return 0 if report["summary"]["ready_for_m44_04"] else 1
