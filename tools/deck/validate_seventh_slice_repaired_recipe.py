"""Validate the M61-03 seventh-slice repaired recipe preview for M61-05."""
...
M61_04_SYSTEM_DECISION = OUTPUT_DIR / "m61_04_seventh_slice_system_decision_artifact.json"
...
def _main_deck_only_boundary_applied(system_decision: dict[str, Any]) -> bool:
    ...
    return _g_zone_boundary_applied(system_decision) and _bloom_token_boundary_applied(system_decision) and (
        summary.get("main_deck_only_validation_allowed") is True
        and summary.get("ready_for_m61_05") is True
    )
...
def _bloom_token_boundary_applied(system_decision: dict[str, Any]) -> bool:
    summary = system_decision.get("summary", {})
    return (
        summary.get("selected_bloom_token_option_id") == "manual_semantic_review_only_no_runtime_promotion"
        and summary.get("bloom_token_runtime_enabled") is False
        and summary.get("bloom_token_decision_recorded") is True
    )
...
def _recipe_report_from_artifacts(
    ...
    bloom_token_boundary = _bloom_token_boundary_applied(system_decision)
    return {
        ...
        "recipe_drafts": [
            {
                ...
                "quantities": [
                    {
                        ...
                        "roles": ["m61_05_repaired_recipe_candidate"],
                        "quantity_source": "m61_03_accepted_repair_preview",
                    }
                ...
                "validation_metadata": {
                    ...
                    "m61_05_in_memory_repaired_validation": True,
                    "system_decision_item_id": system_decision.get("decision_item", {}).get(
                        "decision_item_id", ""
                    ),
                    "g_zone_support_deferred_resolved_by_m61_04": g_zone_boundary,
                    "bloom_token_support_deferred_resolved_by_m61_04": bloom_token_boundary,
                    "g_zone_support_deferred": not g_zone_boundary,
                    "bloom_token_support_deferred": not bloom_token_boundary,
                    "selected_g_zone_option_id": decision_summary.get("selected_g_zone_option_id", ""),
                    "selected_bloom_token_option_id": decision_summary.get("selected_bloom_token_option_id", ""),
...
def build_seventh_slice_repaired_validation_report(
    ...
    system_decision = system_decision or load_json(M61_04_SYSTEM_DECISION)
    accepted_summary = accepted_artifact.get("summary", {})
    decision_summary = system_decision.get("summary", {})
    main_deck_only_boundary = _main_deck_only_boundary_applied(system_decision)
    g_zone_boundary = _g_zone_boundary_applied(system_decision)
    ...
    validation_report = build_seventh_slice_validation_report(repaired_recipe_report, card_rows)
    validation = validation_report["recipe_validations"][0]
    consistency_report = build_seventh_slice_consistency_report(repaired_recipe_report, validation_report)
    consistency = consistency_report["consistency_checks"][0]
    ready_for_m61_06 = (
        bool(accepted_summary.get("human_acceptance_recorded"))
        and bool(accepted_summary.get("ready_for_m61_04"))
        and bool(decision_summary.get("g_zone_decision_recorded"))
        and bool(decision_summary.get("bloom_token_decision_recorded"))
    ...
    return {
        "version": "M61-05",
        "description": "Seventh-slice repaired recipe validation rerun",
        "selected_target": accepted_artifact.get("selected_target", {}),
        "source_inputs": {
            "m61_03_human_accepted_repair_artifact": str(M61_03_ACCEPTED_REPAIR.relative_to(ROOT)),
            "m61_04_system_decision_artifact": str(M61_04_SYSTEM_DECISION.relative_to(ROOT)),
        ...
        "validation_policy": {
            **validation_report.get("validation_policy", {}),
            "validated_input": "m61_03_repaired_quantity_preview",
            "human_acceptance_required": True,
            "system_boundary_required": True,
            "suppressed_by_m61_04_boundary_issue_codes": [
                code
                for code, resolved in [
                    ("g_zone_support_deferred", g_zone_boundary),
                    ("bloom_token_support_deferred", bloom_token_boundary),
                ]
                if resolved
            ],
        ...
        "accepted_context": {
            ...
            "accepted_bloom_token_package_id": accepted_summary.get("accepted_bloom_token_package_id", ""),
            ...
            "accepted_artifact_ready_for_system_decision": bool(accepted_summary.get("ready_for_m61_04")),
        },
        "system_decision_context": {
            "selected_g_zone_option_id": decision_summary.get("selected_g_zone_option_id", ""),
            "selected_bloom_token_option_id": decision_summary.get("selected_bloom_token_option_id", ""),
            "g_zone_decision_recorded": bool(decision_summary.get("g_zone_decision_recorded")),
            "bloom_token_decision_recorded": bool(decision_summary.get("bloom_token_decision_recorded")),
            "main_deck_only_boundary_applied": main_deck_only_boundary,
            "g_zone_boundary_applied": g_zone_boundary,
            "bloom_token_boundary_applied": bloom_token_boundary,
            "g_zone_runtime_enabled": False,
            "stride_runtime_enabled": False,
            "bloom_token_runtime_enabled": False,
        ...
        "repaired_recipe_consistency": {
            ...
            "bloom_token_support_deferred": consistency["bloom_token_support_deferred"],
        },
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        "# M61-05 Seventh-Slice Repaired Recipe Validation Rerun",
        "",
        "## Summary",
        "",
        ...
        f"- Ready for M61-06: `{summary['ready_for_m61_06']}`",
        "",
        "## Validation",
        "",
        ...
        f"- Bloom/token support deferred in consistency check: `{consistency['bloom_token_support_deferred']}`",
        "",
        "## Policy",
        "",
        "- Rerun is in-memory only.",
        "- Source recipe artifacts are not modified.",
        "- G Zone, Stride, Bloom/token runtime remain disabled.",
        "- Runtime fixture promotion remains disabled until M61-06.",
        "",
        "## Next",
        "",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate M61-03 seventh-slice repaired recipe preview.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_seventh_slice_repaired_validation_report()
    json_path = args.output_dir / "m61_05_seventh_slice_repaired_recipe_validation_report.json"
    md_path = args.output_dir / "m61_05_seventh_slice_repaired_recipe_validation_report.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M61-05 seventh-slice repaired recipe validation report wrote {json_path}")
    print(f"M61-05 seventh-slice repaired recipe validation summary wrote {md_path}")
    print(
        "ready_for_m61_06={ready} validation={validation} consistency={consistency}".format(
            ready=report["summary"]["ready_for_m61_06"],
            validation=report["summary"]["validator_passed_count"],
            consistency=report["summary"]["consistency_status"],
        )
    )
    return 0 if report["summary"]["ready_for_m61_06"] else 1
