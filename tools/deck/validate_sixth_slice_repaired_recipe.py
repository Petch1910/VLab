"""Validate the M57-03 sixth-slice repaired recipe preview for M57-05."""
...
M57_04_G_ZONE_DECISION = OUTPUT_DIR / "m57_04_sixth_slice_g_zone_stride_decision_artifact.json"

if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from tools.deck.check_sixth_slice_combo_recipe_consistency import (  # noqa: E402
    build_sixth_slice_consistency_report,
)
from tools.deck.validate_sixth_slice_recipe_drafts import (  # noqa: E402
    _all_card_ids,
    build_sixth_slice_validation_report,
    load_card_rows,
)
...
def _main_deck_only_boundary_applied(g_zone_decision: dict[str, Any]) -> bool:
    summary = g_zone_decision.get("summary", {})
    return (
        summary.get("selected_option_id") == "main_deck_only_review_no_runtime_promotion"
        ...
        and summary.get("ready_for_m57_05") is True
    )


def _recipe_report_from_artifacts(
    accepted_artifact: dict[str, Any],
    g_zone_decision: dict[str, Any],
) -> dict[str, Any]:
    repair = accepted_artifact["accepted_repair"]
    accepted_summary = accepted_artifact.get("summary", {})
    decision_summary = g_zone_decision.get("summary", {})
    main_deck_only_boundary = _main_deck_only_boundary_applied(g_zone_decision)
    return {
        ...
        "recipe_drafts": [
            {
                ...
                "quantities": [
                    {
                        ...
                        "roles": ["m57_05_repaired_recipe_candidate"],
                        "quantity_source": "m57_03_accepted_repair_preview",
                    }
                ...
                "validation_metadata": {
                    ...
                    "m57_05_in_memory_repaired_validation": True,
                    "g_zone_decision_item_id": g_zone_decision.get("decision_item", {}).get(
                        "decision_item_id", ""
                    ),
                    "g_zone_support_deferred_resolved_by_m57_04": main_deck_only_boundary,
                    "g_zone_support_deferred": not main_deck_only_boundary,
                    "selected_g_zone_option_id": decision_summary.get("selected_option_id", ""),
...
def build_sixth_slice_repaired_validation_report(
    ...
    g_zone_decision = g_zone_decision or load_json(M57_04_G_ZONE_DECISION)
    accepted_summary = accepted_artifact.get("summary", {})
    decision_summary = g_zone_decision.get("summary", {})
    main_deck_only_boundary = _main_deck_only_boundary_applied(g_zone_decision)
    repaired_recipe_report = _recipe_report_from_artifacts(accepted_artifact, g_zone_decision)
    card_rows = load_card_rows(_all_card_ids(repaired_recipe_report))
    validation_report = build_sixth_slice_validation_report(repaired_recipe_report, card_rows)
    validation = validation_report["recipe_validations"][0]
    consistency_report = build_sixth_slice_consistency_report(repaired_recipe_report, validation_report)
    consistency = consistency_report["consistency_checks"][0]
    ready_for_m57_06 = (
        bool(accepted_summary.get("human_acceptance_recorded"))
        and bool(accepted_summary.get("ready_for_m57_04"))
    ...
    return {
        "version": "M57-05",
        "description": "Sixth-slice repaired recipe validation rerun",
        "selected_target": accepted_artifact.get("selected_target", {}),
        "source_inputs": {
            "m57_03_human_accepted_repair_artifact": str(M57_03_ACCEPTED_REPAIR.relative_to(ROOT)),
            "m57_04_g_zone_stride_decision_artifact": str(M57_04_G_ZONE_DECISION.relative_to(ROOT)),
        ...
        "scope": {
            ...
            "changes_g_zone_decision_artifact": False,
        ...
        "validation_policy": {
            **validation_report.get("validation_policy", {}),
            "validated_input": "m57_03_repaired_quantity_preview",
            "human_acceptance_required": True,
            "g_zone_boundary_required": True,
            "suppressed_by_m57_04_boundary_issue_codes": (
                ["g_zone_support_deferred"] if main_deck_only_boundary else []
            ),
        ...
        "accepted_context": {
            ...
            "accepted_g_zone_package_id": accepted_summary.get("accepted_g_zone_package_id", ""),
            ...
            "accepted_artifact_ready_for_g_zone_decision": bool(accepted_summary.get("ready_for_m57_04")),
        },
        "g_zone_decision_context": {
            "selected_option_id": decision_summary.get("selected_option_id", ""),
            ...
            "g_zone_runtime_enabled": False,
            "stride_runtime_enabled": False,
            "source_decision_item_id": g_zone_decision.get("decision_item", {}).get("decision_item_id", ""),
        },
        ...
        "summary": {
            **validation_report["summary"],
            ...
            "g_zone_runtime_enabled": False,
            "stride_runtime_enabled": False,
            "source_g_zone_decision_item_id": g_zone_decision.get("decision_item", {}).get(
                "decision_item_id", ""
            ),
            "consistency_status": consistency["consistency_status"],
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M57-06: `{summary['ready_for_m57_06']}`",
        "",
        "## Validation",
        "",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate M57-03 sixth-slice repaired recipe preview.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_sixth_slice_repaired_validation_report()
    json_path = args.output_dir / "m57_05_sixth_slice_repaired_recipe_validation_report.json"
    md_path = args.output_dir / "m57_05_sixth_slice_repaired_recipe_validation_report.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M57-05 sixth-slice repaired recipe validation report wrote {json_path}")
    print(f"M57-05 sixth-slice repaired recipe validation summary wrote {md_path}")
    print(
        "ready_for_m57_06={ready} validation={validation} consistency={consistency}".format(
            ready=report["summary"]["ready_for_m57_06"],
            validation=report["summary"]["validator_passed_count"],
            consistency=report["summary"]["consistency_status"],
        )
    )
    return 0 if report["summary"]["ready_for_m57_06"] else 1
