"""Validate M60-03 seventh-slice recipe drafts for M60-04."""
...
RECIPE_DRAFTS = OUTPUT_DIR / "m60_03_seventh_slice_recipe_draft_model.json"
...
def _validate_recipe(recipe: dict[str, Any], card_rows: dict[str, dict[str, Any]], expected_clan: str) -> dict[str, Any]:
    ...
    if trigger_total != 16:
        issues.append(
            _issue(
                ...
                "blocker",
                "Classic/G-era trigger count must be 16 before recipe can be promoted.",
    ...
    if trigger_counts.get("Heal", 0) > 4:
        issues.append(
            _issue(
                ...
                "blocker",
                "Heal trigger count exceeds classic/G-era guardrail.",
    ...
    if manual_ids:
        issues.append(
            _issue(
                ...
                "blocker",
                "Recipe contains cards that M60-02 marked as manual-review required.",
    ...
    if metadata.get("bloom_token_support_deferred"):
        issues.append(
            _issue(
                "bloom_token_support_deferred",
                "review",
                "Bloom/token-like text remains manual-review only until dedicated rules support exists.",
                {},
            )
        )
...
def build_seventh_slice_validation_report(
    ...
    return {
        "version": "M60-04",
        "description": "Seventh-slice recipe validator for M60-03 advisory drafts",
        "selected_target": recipe_report.get("selected_target", {}),
        "source_inputs": {
            "seventh_slice_recipe_draft_model": str(RECIPE_DRAFTS.relative_to(ROOT)),
        ...
        "validation_policy": {
            ...
            "bloom_token_support_deferred_is_review_not_blocker": True,
        },
        "summary": {
            ...
            "bloom_token_deferred_recipe_count": sum(
                1 for item in validations if any(issue["code"] == "bloom_token_support_deferred" for issue in item["issues"])
            ),
            "issue_counts": dict(sorted(issue_counts.items())),
            "ready_for_m60_05": bool(validations),
        },
        "recipe_validations": validations,
        "next_target": {
            "milestone": "M60-05",
            "task": "Seventh-slice combo-to-recipe consistency",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        "# M60-04 Seventh-Slice Recipe Validation Report",
        "",
        "## Summary",
        "",
        ...
        f"- Ready for M60-05: `{summary['ready_for_m60_05']}`",
        "",
        "## Recipe Status",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "- Bloom/token deferred is review evidence, not a blocker.",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate M60-03 seventh-slice recipe drafts.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_seventh_slice_validation_report()
    json_path = args.output_dir / "m60_04_seventh_slice_recipe_validation_report.json"
    md_path = args.output_dir / "m60_04_seventh_slice_recipe_validation_report.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M60-04 seventh-slice recipe validation wrote {json_path}")
    print(f"M60-04 seventh-slice recipe validation summary wrote {md_path}")
    print(
        "ready_for_m60_05={ready} runtime_ready={runtime_ready} manual_blocked={manual_blocked} grade4={grade4}".format(
            ready=report["summary"]["ready_for_m60_05"],
    ...
    return 0 if report["summary"]["ready_for_m60_05"] else 1
