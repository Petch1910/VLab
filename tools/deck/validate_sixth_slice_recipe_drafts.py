"""Validate M56-03 sixth-slice recipe drafts for M56-04."""
...
RECIPE_DRAFTS = OUTPUT_DIR / "m56_03_sixth_slice_recipe_draft_model.json"
    ...
    if manual_ids:
        issues.append(
            _issue(
                ...
                "blocker",
                "Recipe contains cards that M56-02 marked as manual-review required.",
                {"card_ids": sorted(manual_ids)},
            )
        )

    if recipe.get("validation_metadata", {}).get("g_zone_support_deferred"):
        issues.append(
            _issue(
...
def build_sixth_slice_validation_report(
    ...
    return {
        "version": "M56-04",
        "description": "Sixth-slice recipe validator for M56-03 advisory drafts",
        "selected_target": recipe_report.get("selected_target", {}),
        "source_inputs": {
            "sixth_slice_recipe_draft_model": str(RECIPE_DRAFTS.relative_to(ROOT)),
        ...
        "summary": {
            ...
            "ready_for_m56_05": bool(validations),
        },
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M56-05: `{summary['ready_for_m56_05']}`",
        "",
        "## Recipe Status",
        "",
    ]
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate M56-03 sixth-slice recipe drafts.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_sixth_slice_validation_report()
    json_path = args.output_dir / "m56_04_sixth_slice_recipe_validation_report.json"
    md_path = args.output_dir / "m56_04_sixth_slice_recipe_validation_report.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M56-04 sixth-slice recipe validation wrote {json_path}")
    print(f"M56-04 sixth-slice recipe validation summary wrote {md_path}")
    print(
        "ready_for_m56_05={ready} runtime_ready={runtime_ready} manual_blocked={manual_blocked} grade4={grade4}".format(
            ready=report["summary"]["ready_for_m56_05"],
    ...
    return 0 if report["summary"]["ready_for_m56_05"] else 1
