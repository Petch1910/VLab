"""Build the M68-02 ninth-slice review packet."""
...
M67_01_SELECTION = OUTPUT_DIR / "m67_01_ninth_target_slice_selection.json"
...
JSON_OUTPUT = OUTPUT_DIR / "m68_02_ninth_slice_review_packet.json"
MD_OUTPUT = OUTPUT_DIR / "m68_02_ninth_slice_review_packet.md"
CSV_OUTPUT = OUTPUT_DIR / "m68_02_ninth_slice_review_packet.csv"
...
def _scaffold_note_item(scaffold_report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "item_id": "m68_01_fixture_scaffold",
        ...
        "required_trigger_types": scaffold.get("required_trigger_types", []),
        ...
        "g_zone_fixture_boundary_required": bool(scaffold.get("g_zone_fixture_boundary_required")),
        "mechanic_manual_review_required": bool(
            mechanic.get("stride_or_g_unit_text_requires_manual_review")
            or mechanic.get("generation_break_text_requires_manual_review")
            or mechanic.get("aqua_force_attack_count_text_requires_manual_review")
            or mechanic.get("grade4_cards_g_zone_advisory_only_until_format_support")
        ),
        "g_zone_runtime_enabled": bool(mechanic.get("runtime_g_zone_enabled")),
        "stride_runtime_enabled": bool(mechanic.get("runtime_stride_enabled")),
        "stride_or_g_unit_text_manual_review_required": bool(
            mechanic.get("stride_or_g_unit_text_requires_manual_review")
        ),
        "generation_break_text_manual_review_required": bool(
            mechanic.get("generation_break_text_requires_manual_review")
        ),
        "aqua_force_attack_count_text_manual_review_required": bool(
            mechanic.get("aqua_force_attack_count_text_requires_manual_review")
        ),
        "grade4_cards_g_zone_advisory_only": bool(
            mechanic.get("grade4_cards_g_zone_advisory_only_until_format_support")
        ),
        "review_reasons": [
            "ninth_slice_aquaforce_g_series_fixture_scaffold",
            "g_zone_stride_manual_review_boundary",
            "aqua_force_battle_order_manual_review_boundary",
            "must_review_before_recipe_validator",
        ],
        "blocked_until": [
            "fixture_scaffold_reviewed",
            "m68_04_recipe_validator_uses_scaffold",
            "g_zone_stride_policy_confirmed",
            "aqua_force_battle_order_policy_confirmed",
            "grade4_g_zone_support_decision",
        ],
...
def _candidate_edge_item(edge: dict[str, Any], rank: int) -> dict[str, Any]:
    return {
        ...
        "review_reasons": [
            "ninth_slice_candidate_compatibility_edge",
            "requires_human_recipe_selection_before_m68_03",
            "aqua_force_battle_order_review_boundary",
        ],
        "blocked_until": [
            "human_recipe_selection",
            "m68_04_recipe_validator",
            "combo_line_to_recipe_consistency_check",
        ],
        "recommended_next_action": "consider_for_m68_03_advisory_recipe_draft",
    }
...
def build_ninth_slice_review_packet(
    ...
    selection_report = selection_report or load_json(M67_01_SELECTION)
    readiness_report = readiness_report or load_json(M67_02_READINESS)
    probe_report = probe_report or load_json(M67_03_PROBE)
    ...
    gate_allows_offline = bool(gate_report.get("summary", {}).get("ready_for_m68"))
    scaffold_ready = bool(scaffold_report.get("summary", {}).get("ready_for_m68_02"))
    ...
    return {
        "version": "M68-02",
        "description": "Ninth-slice review packet before advisory deck recipe drafting",
        "selected_target": selection_report.get("selected_target", {}),
        "source_inputs": {
            "ninth_target_slice_selection": _rel(M67_01_SELECTION),
            "ninth_slice_fixture_readiness": _rel(M67_02_READINESS),
            "ninth_slice_semantic_compatibility_probe": _rel(M67_03_PROBE),
            "ninth_slice_recipe_pipeline_entry_gate": _rel(M67_04_GATE),
            "ninth_slice_fixture_scaffold": _rel(M68_01_SCAFFOLD),
        },
        "scope": {
        ...
        "review_policy": {
            ...
            "g_zone_and_stride_runtime_support_deferred": True,
            ...
            "grade4_g_zone_cards_review_only": True,
            "no_live_card_text_parsing": True,
            "no_direct_GameState_mutation": True,
        },
        "evidence_summary": {
            "m67_04_offline_recipe_pipeline_allowed": gate_allows_offline,
            "m67_04_runtime_promotion_closed": promotion_closed,
            "m68_01_scaffold_ready": scaffold_ready,
        ...
        "next_target": {
            "milestone": "M68-03",
            "task": "Ninth-slice recipe draft model",
            "blocked_until": [
                "M68-02 review packet is available",
                "human/team can inspect fixture scaffold, candidate edges, manual-review cards, and G Zone notes",
            ],
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M68-03: `{summary['ready_for_m68_03']}`",
        "",
        "## Fixture Scaffold",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "- G Zone and Stride runtime support remains deferred.",
            "- Stride, G-unit, Generation Break, and Aqua Force attack-count text remains manual-review only.",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M68-02 ninth-slice review packet.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"M68-02 review packet wrote {json_path}")
    print(f"M68-02 review packet summary wrote {md_path}")
    print(f"M68-02 review packet CSV wrote {csv_path}")
    print(
        "ready={ready} scaffold={scaffold} manual={manual} candidates={candidates}".format(
            ready=report["summary"]["ready_for_m68_03"],
    ...
    return 0 if report["summary"]["ready_for_m68_03"] else 1
