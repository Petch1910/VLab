"""Build and verify the M42-03 second fixture headless load smoke artifacts."""
...
DEFAULT_M42_02_REPORT = OUTPUT_DIR / "m42_02_second_fixture_deck_text_export.json"
DEFAULT_DECK_TEXT = OUTPUT_DIR / "m42_02_second_fixture_deck_text_export.txt"
CARDS_SQLITE = ROOT / "data" / "packs" / "vanguard_th" / "cards.sqlite"

DECK_CODE_OUTPUT = OUTPUT_DIR / "m42_03_second_fixture_deck_code.txt"
JSON_OUTPUT = OUTPUT_DIR / "m42_03_second_fixture_load_smoke.json"
MD_OUTPUT = OUTPUT_DIR / "m42_03_second_fixture_load_smoke.md"
DEFAULT_UNITY_RESULT = OUTPUT_DIR / "m42_03_second_fixture_unity_result.json"
DEFAULT_UNITY_REPLAY = OUTPUT_DIR / "m42_03_second_fixture_unity_replay.json"
...
def _build_deck_object(parsed: dict[str, Any]) -> dict[str, Any]:
    ...
    deck_id = "m4203" + hashlib.sha256(digest_source.encode("utf-8")).hexdigest()[:26]
    return {
        "deck_id": deck_id,
        "name": metadata["Name"],
...
def build_second_headless_fixture_load_smoke(
    fixture: dict[str, Any] | None = None,
    m42_02_report: dict[str, Any] | None = None,
    ...
    m42_02_report = m42_02_report or load_json(DEFAULT_M42_02_REPORT)
    deck_text = deck_text if deck_text is not None else DEFAULT_DECK_TEXT.read_text(encoding="utf-8")
    issues: list[dict[str, Any]] = []

    if not m42_02_report.get("summary", {}).get("export_ready"):
        issues.append(
            _issue(
                "m42_02_export_not_ready",
                "blocker",
                "M42-02 deck text export must be ready before headless load smoke.",
                {"summary": m42_02_report.get("summary", {})},
            )
        )
    ...
    ready_for_m42_04 = offline_load_ready and unity_smoke_passed

    return {
        "version": "M42-03",
        "description": "Second fixture headless load smoke",
        "source_inputs": {
            "runtime_fixture": str(DEFAULT_FIXTURE.relative_to(ROOT)),
            "m42_02_report": str(DEFAULT_M42_02_REPORT.relative_to(ROOT)),
        ...
        "summary": {
            ...
            "ready_for_m42_04": ready_for_m42_04,
        },
        "next_target": {
            "milestone": "M42-04",
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M42-04: `{summary['ready_for_m42_04']}`",
        "",
        "## Unity Headless",
        "",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M42-03 second fixture headless load smoke artifacts.")
    parser.add_argument("--fixture", type=Path, default=DEFAULT_FIXTURE)
    parser.add_argument("--m42-02-report", type=Path, default=DEFAULT_M42_02_REPORT)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    m42_02_report = load_json(args.m42_02_report)
    ...
    report["source_inputs"]["m42_02_report"] = str(args.m42_02_report)
    ...
    print(f"M42-03 second fixture headless load smoke wrote {json_path}")
    print(f"M42-03 second fixture headless load smoke summary wrote {md_path}")
    if report["summary"]["deck_code_created"]:
        print(f"M42-03 second fixture deck code wrote {deck_code_path}")
    print(
        "offline_ready={offline} unity_provided={provided} unity_passed={passed} blockers={blockers} next={next_ready}".format(
            ...
            next_ready=report["summary"]["ready_for_m42_04"],
        )
    )
