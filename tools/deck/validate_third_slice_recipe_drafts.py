"""Validate M44-03 third-slice recipe drafts for M44-04.
...
RECIPE_DRAFTS = OUTPUT_DIR / "m44_03_third_slice_recipe_draft_model.json"
    ...
    if manual_ids:
        issues.append(
            _issue(
                ...
                "blocker",
                "Recipe contains cards that M44-02 marked as manual-review required.",
...
def build_third_slice_validation_report(
    ...
    return {
        "version": "M44-04",
        "description": "Third-slice recipe validator for M44-03 advisory drafts",
        "selected_target": recipe_report.get("selected_target", {}),
        "source_inputs": {
            "third_slice_recipe_draft_model": str(RECIPE_DRAFTS.relative_to(ROOT)),
        ...
        "summary": {
            ...
            "ready_for_m44_05": bool(validations),
        },
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M44-05: `{summary['ready_for_m44_05']}`",
        "",
        "## Recipe Status",
        "",
    ]
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate M44-03 third-slice recipe drafts.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_third_slice_validation_report()
    json_path = args.output_dir / "m44_04_third_slice_recipe_validation_report.json"
    md_path = args.output_dir / "m44_04_third_slice_recipe_validation_report.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M44-04 third-slice recipe validation wrote {json_path}")
    print(f"M44-04 third-slice recipe validation summary wrote {md_path}")
    print(
        "ready_for_m44_05={ready} runtime_ready={runtime_ready} manual_blocked={manual_blocked}".format(
            ready=report["summary"]["ready_for_m44_05"],
            runtime_ready=report["summary"]["runtime_ready_recipe_count"],
            manual_blocked=report["summary"]["blocked_by_manual_review_count"],
        )
    )
    return 0 if report["summary"]["ready_for_m44_05"] else 1
