"""Export the fourth offline runtime fixture as count-line deck text (M50-02)."""
...
DEFAULT_VALIDATION_REPORT = OUTPUT_DIR / "m50_01_fourth_fixture_schema_validation.json"
CARDS_SQLITE = ROOT / "data" / "packs" / "vanguard_th" / "cards.sqlite"
PACK_MANIFEST = ROOT / "data" / "packs" / "vanguard_th" / "manifest.json"

TEXT_OUTPUT = OUTPUT_DIR / "m50_02_fourth_fixture_deck_text_export.txt"
JSON_OUTPUT = OUTPUT_DIR / "m50_02_fourth_fixture_deck_text_export.json"
MD_OUTPUT = OUTPUT_DIR / "m50_02_fourth_fixture_deck_text_export.md"
...
def _is_validation_ready(validation_report: dict[str, Any]) -> bool:
    ...
    return bool(summary.get("schema_valid")) and bool(summary.get("ready_for_m50_02"))
...
def _build_deck_text(
    ...
    era_preset = str(format_policy.get("era_preset") or "g_series_first")
    slice_name = str(format_policy.get("slice") or selected_target.get("slice") or "Fourth Offline Slice")
    group_name = str(format_policy.get("group") or selected_target.get("group") or "Royal Paladin")

    lines = [
        "# Vanguard Thai Sim Deck List",
        "# M50-02 review-only export from an offline runtime fixture.",
        f"# FixtureId: {fixture_id}",
        f"# RecipeId: {recipe_id}",
        f"# Slice: {slice_name}",
        f"# Group: {group_name}",
        f"# GZoneBoundary: {format_policy.get('g_zone_boundary') or ''}",
        "# Scope: review_only_offline_fixture",
        "Name: G Series Royal Paladin Fixture (" + recipe_id + ")",
        "Format: " + era_preset,
...
def build_fourth_fixture_deck_text_export(
    ...
    if not _is_validation_ready(validation_report):
        issues.append(
            _issue(
                "m50_01_validation_not_ready",
                "blocker",
                "M50-01 validation report must be schema-valid and ready before deck text export.",
                {"summary": validation_report.get("summary", {})},
            )
        )
    ...
    return {
        "version": "M50-02",
        "description": "Fourth fixture deck text exporter",
        "source_inputs": {
            "runtime_fixture": str(DEFAULT_FIXTURE.relative_to(ROOT)),
            "m50_01_validation_report": str(DEFAULT_VALIDATION_REPORT.relative_to(ROOT)),
        ...
        "scope": {
            ...
            "fourth_fixture_deck_text_export": True,
        ...
        "summary": {
            ...
            "ready_for_m50_03": export_ready,
        },
        "next_target": {
            "milestone": "M50-03",
            "task": "Fourth fixture headless load smoke",
        },
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        "# M50-02 Fourth Fixture Deck Text Export",
        "",
        "## Summary",
        "",
        ...
        f"- Ready for M50-03: `{summary['ready_for_m50_03']}`",
        "",
        "## Runtime Boundary",
        "",
        "- Review text only.",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export M50-02 fourth fixture count-line deck text.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_fourth_fixture_deck_text_export(fixture, validation_report)
    report["source_inputs"]["runtime_fixture"] = str(args.fixture)
    report["source_inputs"]["m50_01_validation_report"] = str(args.validation_report)
    ...
    print(f"M50-02 fourth fixture deck text export wrote {json_path}")
    print(f"M50-02 fourth fixture deck text summary wrote {md_path}")
    if report["summary"]["export_ready"]:
        print(f"M50-02 fourth fixture deck text wrote {text_path}")
    print(
        "export_ready={ready} blockers={blockers} card_lines={lines} next={next_ready}".format(
            ...
            next_ready=report["summary"]["ready_for_m50_03"],
        )
    )
