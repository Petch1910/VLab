"""Export the eighth offline runtime fixture as count-line deck text (M66-02)."""
...
DEFAULT_VALIDATION_REPORT = OUTPUT_DIR / "m66_01_eighth_fixture_schema_validation.json"
CARDS_SQLITE = ROOT / "data" / "packs" / "vanguard_th" / "cards.sqlite"
PACK_MANIFEST = ROOT / "data" / "packs" / "vanguard_th" / "manifest.json"

TEXT_OUTPUT = OUTPUT_DIR / "m66_02_eighth_fixture_deck_text_export.txt"
JSON_OUTPUT = OUTPUT_DIR / "m66_02_eighth_fixture_deck_text_export.json"
MD_OUTPUT = OUTPUT_DIR / "m66_02_eighth_fixture_deck_text_export.md"
...
def _is_validation_ready(validation_report: dict[str, Any]) -> bool:
    ...
    return bool(summary.get("schema_valid")) and bool(summary.get("ready_for_m66_02"))
...
def _build_deck_text(
    ...
    era_preset = str(format_policy.get("era_preset") or selected_target.get("era_preset") or "link_joker_legion_mate")
    slice_name = str(format_policy.get("slice") or selected_target.get("slice") or "Eighth Offline Slice")
    group_name = str(format_policy.get("group") or selected_target.get("group") or "Kagero")

    lines = [
        "# Vanguard Thai Sim Deck List",
        "# M66-02 review-only export from an offline runtime fixture.",
        f"# FixtureId: {fixture_id}",
        f"# RecipeId: {recipe_id}",
        f"# Slice: {slice_name}",
        f"# Group: {group_name}",
        f"# LockBoundary: {system_boundaries.get('selected_lock_option_id') or ''}",
        f"# LegionBoundary: {system_boundaries.get('selected_legion_option_id') or ''}",
        "# Scope: review_only_offline_fixture",
        "Name: Link Joker/Legion Mate Kagero Fixture (" + recipe_id + ")",
        "Format: " + era_preset,
    ...
    lines.extend(["", "[Ride]", "", "[G]"])
    if system_boundaries.get("lock_runtime_enabled") is False and system_boundaries.get("unlock_runtime_enabled") is False:
        lines.append("# Note: Lock/Unlock runtime is disabled; Lock text remains review-only.")
    if system_boundaries.get("legion_runtime_enabled") is False and system_boundaries.get("mate_identity_check_enabled") is False:
        lines.append("# Note: Legion/Mate runtime is disabled; Legion text remains review-only.")
    lines.append("")
    return "\n".join(lines).rstrip() + "\n"


def build_eighth_fixture_deck_text_export(
    ...
    if not _is_validation_ready(validation_report):
        issues.append(
            _issue(
                "m66_01_validation_not_ready",
                "blocker",
                "M66-01 validation report must be schema-valid and ready before deck text export.",
                {"summary": validation_report.get("summary", {})},
            )
        )
    ...
    return {
        "version": "M66-02",
        "description": "Eighth fixture deck text exporter",
        "source_inputs": {
            "runtime_fixture": str(DEFAULT_FIXTURE.relative_to(ROOT)),
            "m66_01_validation_report": str(DEFAULT_VALIDATION_REPORT.relative_to(ROOT)),
        ...
        "scope": {
            ...
            "eighth_fixture_deck_text_export": True,
        ...
        "deck_text_policy": {
            ...
            "lock_legion_notice_is_comment_only": True,
        },
        "fixture_summary": {
        ...
        "summary": {
            ...
            "ready_for_m66_03": export_ready,
        },
        "next_target": {
            "milestone": "M66-03",
            "task": "Eighth fixture headless load smoke",
        },
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        "# M66-02 Eighth Fixture Deck Text Export",
        "",
        "## Summary",
        "",
        ...
        f"- Ready for M66-03: `{summary['ready_for_m66_03']}`",
        "",
        "## Runtime Boundary",
        "",
        "- Review text only.",
        ...
        "- Does not enable Lock or Unlock runtime.",
        "- Does not enable Legion or Mate runtime.",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export M66-02 eighth fixture count-line deck text.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_eighth_fixture_deck_text_export(fixture, validation_report)
    report["source_inputs"]["runtime_fixture"] = str(args.fixture)
    report["source_inputs"]["m66_01_validation_report"] = str(args.validation_report)
    ...
    print(f"M66-02 eighth fixture deck text export wrote {json_path}")
    print(f"M66-02 eighth fixture deck text summary wrote {md_path}")
    if report["summary"]["export_ready"]:
        print(f"M66-02 eighth fixture deck text wrote {text_path}")
    print(
        "export_ready={ready} blockers={blockers} card_lines={lines} next={next_ready}".format(
            ...
            next_ready=report["summary"]["ready_for_m66_03"],
        )
    )
