"""Export the ninth offline runtime fixture as count-line deck text (M70-02)."""
...
DEFAULT_VALIDATION_REPORT = OUTPUT_DIR / "m70_01_ninth_fixture_schema_validation.json"
CARDS_SQLITE = ROOT / "data" / "packs" / "vanguard_th" / "cards.sqlite"
PACK_MANIFEST = ROOT / "data" / "packs" / "vanguard_th" / "manifest.json"

TEXT_OUTPUT = OUTPUT_DIR / "m70_02_ninth_fixture_deck_text_export.txt"
JSON_OUTPUT = OUTPUT_DIR / "m70_02_ninth_fixture_deck_text_export.json"
MD_OUTPUT = OUTPUT_DIR / "m70_02_ninth_fixture_deck_text_export.md"
...
def _is_validation_ready(validation_report: dict[str, Any]) -> bool:
    ...
    return bool(summary.get("schema_valid")) and bool(summary.get("ready_for_m70_02"))
...
def _build_deck_text(
    ...
    era_preset = str(format_policy.get("era_preset") or selected_target.get("era_preset") or "g_series_first")
    slice_name = str(format_policy.get("slice") or selected_target.get("slice") or "Ninth Offline Slice")
    group_name = str(format_policy.get("group") or selected_target.get("group") or "Aqua Force")

    lines = [
        "# Vanguard Thai Sim Deck List",
        "# M70-02 review-only export from an offline runtime fixture.",
        f"# FixtureId: {fixture_id}",
        f"# RecipeId: {recipe_id}",
        f"# Slice: {slice_name}",
        f"# Group: {group_name}",
        f"# GZoneBoundary: {system_boundaries.get('selected_g_zone_option_id') or ''}",
        f"# StrideBoundary: {system_boundaries.get('selected_stride_option_id') or ''}",
        f"# AquaForceBoundary: {system_boundaries.get('selected_aqua_force_option_id') or ''}",
        "# Scope: review_only_offline_fixture",
        "Name: G Series Aqua Force Fixture (" + recipe_id + ")",
        "Format: " + era_preset,
    ...
    lines.extend(["", "[Ride]", "", "[G]"])
    if system_boundaries.get("g_zone_runtime_enabled") is False:
        lines.append("# Note: G Zone runtime is disabled; G-unit and G-zone cards remain review-only.")
    if system_boundaries.get("stride_runtime_enabled") is False:
        lines.append("# Note: Stride runtime is disabled; stride text remains review-only.")
    if system_boundaries.get("aqua_force_battle_order_runtime_enabled") is False:
        lines.append(
...
def build_ninth_fixture_deck_text_export(
    ...
    if not _is_validation_ready(validation_report):
        issues.append(
            _issue(
                "m70_01_validation_not_ready",
                "blocker",
                "M70-01 validation report must be schema-valid and ready before deck text export.",
                {"summary": validation_report.get("summary", {})},
            )
        )
    ...
    return {
        "version": "M70-02",
        "description": "Ninth fixture deck text exporter",
        "source_inputs": {
            "runtime_fixture": str(DEFAULT_FIXTURE.relative_to(ROOT)),
            "m70_01_validation_report": str(DEFAULT_VALIDATION_REPORT.relative_to(ROOT)),
        ...
        "scope": {
            ...
            "ninth_fixture_deck_text_export": True,
        ...
        "deck_text_policy": {
            ...
            "g_zone_stride_aqua_notice_is_comment_only": True,
        },
        "fixture_summary": {
        ...
        "summary": {
            ...
            "ready_for_m70_03": export_ready,
        },
        "next_target": {
            "milestone": "M70-03",
            "task": "Ninth fixture headless load smoke",
        },
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        "# M70-02 Ninth Fixture Deck Text Export",
        "",
        "## Summary",
        "",
        ...
        f"- Ready for M70-03: `{summary['ready_for_m70_03']}`",
        "",
        "## Runtime Boundary",
        "",
        "- Review text only.",
        ...
        "- Does not enable G Zone runtime.",
        "- Does not enable Stride runtime.",
        "- Does not enable Aqua Force battle-order runtime.",
        "- Does not parse live card text.",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export M70-02 ninth fixture count-line deck text.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_ninth_fixture_deck_text_export(fixture, validation_report)
    report["source_inputs"]["runtime_fixture"] = str(args.fixture)
    report["source_inputs"]["m70_01_validation_report"] = str(args.validation_report)
    ...
    print(f"M70-02 ninth fixture deck text export wrote {json_path}")
    print(f"M70-02 ninth fixture deck text summary wrote {md_path}")
    if report["summary"]["export_ready"]:
        print(f"M70-02 ninth fixture deck text wrote {text_path}")
    print(
        "export_ready={ready} blockers={blockers} card_lines={lines} next={next_ready}".format(
            ...
            next_ready=report["summary"]["ready_for_m70_03"],
        )
    )
