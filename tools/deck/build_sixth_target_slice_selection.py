"""Build the M55-01 sixth target slice selection artifact."""
...
M54_04_DECISION = OUTPUT_DIR / "m54_04_five_fixture_scale_decision.json"
...
def build_sixth_target_slice_selection(m54_decision: dict[str, Any] | None = None) -> dict[str, Any]:
    m54_decision = m54_decision or load_json(M54_04_DECISION)
    summary = m54_decision.get("summary", {})
    candidates = list(m54_decision.get("candidate_queue", []))
    selection_allowed = bool(summary.get("sixth_slice_offline_pipeline_allowed")) and bool(candidates)
    selected = candidates[0] if selection_allowed else {}
    selected_target = (
        {
            "slice": "Sixth Offline Slice",
    ...
    return {
        "version": "M55-01",
        "description": "Sixth target slice selection",
        "source_inputs": {
            "five_fixture_scale_decision": str(M54_04_DECISION.relative_to(ROOT)),
        },
        "scope": {
        ...
        "selection_policy": {
            "requires_m54_scale_allowed": True,
        ...
        "decision": {
            ...
            "sixth_slice_selected": bool(selected_target),
        ...
        "summary": {
            ...
            "ready_for_m55_02": bool(selected_target),
        },
        "next_target": {
            "milestone": "M55-02" if selected_target else "M54-repair",
            "task": "Sixth-slice fixture/format readiness" if selected_target else "Repair five-fixture scale decision",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        "## Summary",
        "",
        f"- Sixth slice selected: `{summary['sixth_slice_selected']}`",
        ...
        f"- Ready for M55-02: `{summary['ready_for_m55_02']}`",
        "",
        "## Selected Target",
        "",
    ]
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M55-01 sixth target slice selection.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_sixth_target_slice_selection()
    json_path = args.output_dir / "m55_01_sixth_target_slice_selection.json"
    md_path = args.output_dir / "m55_01_sixth_target_slice_selection.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M55-01 sixth target slice selection wrote {json_path}")
    print(f"M55-01 sixth target slice selection summary wrote {md_path}")
    print(
        "selected={selected} group={group} next={next_ready}".format(
            selected=report["summary"]["sixth_slice_selected"],
            group=report["summary"]["selected_group"],
            next_ready=report["summary"]["ready_for_m55_02"],
        )
    )
    return 0 if report["summary"]["ready_for_m55_02"] else 1
