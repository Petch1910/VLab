"""Build ninth-slice fixture/format readiness evidence for M67-02."""
...
JSON_OUTPUT = OUTPUT_DIR / "m67_02_ninth_slice_fixture_readiness.json"
MD_OUTPUT = OUTPUT_DIR / "m67_02_ninth_slice_fixture_readiness.md"
...
def build_ninth_slice_fixture_readiness(selection_report: dict[str, Any] | None = None) -> dict[str, Any]:
    selection_report = selection_report or load_json(M67_01_SELECTION)
    selected = selection_report.get("selected_target", {})
    selection_ready = bool(selection_report.get("summary", {}).get("ready_for_m67_02")) and bool(selected)
    ...
    return {
        "version": "M67-02",
        "description": "Ninth-slice fixture/format readiness",
        "source_inputs": {
            "ninth_target_slice_selection": str(M67_01_SELECTION.relative_to(ROOT)),
        ...
        "format_scope": {
            ...
            "policy_reuse_decision": "requires_ninth_slice_fixture_scaffold"
            if fixture_expectations_met
            else "requires_ninth_slice_readiness_repair",
        },
        "card_pool_summary": {
        ...
        "next_target": {
            "milestone": "M67-03" if fixture_expectations_met else "M67-repair",
            "task": "Ninth-slice semantic/compatibility probe"
            if fixture_expectations_met
            else "Repair ninth-slice readiness blockers",
        },
        "summary": {
            ...
            "all_fixture_expectations_met": fixture_expectations_met,
            "semantic_probe_ready": fixture_expectations_met,
            "ready_for_m67_03": fixture_expectations_met,
            "repair_required": not fixture_expectations_met,
            "repair_reasons": repair_reasons,
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        "# M67-02 Ninth-Slice Fixture/Format Readiness",
        "",
        "## Selected Target",
        "",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M67-02 ninth-slice fixture readiness report.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"M67-02 ninth-slice fixture readiness wrote {json_path}")
    print(f"M67-02 ninth-slice fixture readiness summary wrote {md_path}")
    print(
    ...
    return 0 if report["summary"]["ready_for_m67_03"] else 1
