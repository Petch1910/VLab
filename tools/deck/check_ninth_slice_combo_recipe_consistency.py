"""Check M68-05 ninth-slice combo-to-recipe consistency."""
...
def _issue_details(validation: dict[str, Any], code: str) -> Any:
...
def _manual_overlap_from_validation(validation: dict[str, Any]) -> list[str]:
    details = _issue_details(validation, "manual_review_card_overlap")
    return sorted(details.get("card_ids", [])) if isinstance(details, dict) else []


def _has_issue(validation: dict[str, Any], code: str) -> bool:
    return any(issue.get("code") == code for issue in validation.get("issues", []))
...
def build_ninth_slice_consistency_report(
    ...
    for recipe in recipe_report.get("recipe_drafts", []):
        ...
        g_zone_deferred = _has_issue(validation, "g_zone_support_deferred")
        stride_deferred = _has_issue(validation, "stride_support_deferred")
        aqua_force_battle_order_deferred = _has_issue(
            validation,
            "aqua_force_battle_order_support_deferred",
        )
        grade_profile_review = _has_issue(validation, "grade_profile_review")
        checks.append(
            {
                "recipe_id": recipe_id,
                ...
                "g_zone_support_deferred": g_zone_deferred,
                "stride_support_deferred": stride_deferred,
                "aqua_force_battle_order_support_deferred": aqua_force_battle_order_deferred,
                "grade_profile_review": grade_profile_review,
    ...
    return {
        "version": "M68-05",
        "description": "Ninth-slice candidate edge to recipe consistency check",
        ...
        "summary": {
            ...
            "g_zone_deferred_check_count": sum(1 for item in checks if item["g_zone_support_deferred"]),
            "stride_deferred_check_count": sum(1 for item in checks if item["stride_support_deferred"]),
            "aqua_force_battle_order_deferred_check_count": sum(
                1 for item in checks if item["aqua_force_battle_order_support_deferred"]
            ),
            "grade_profile_review_check_count": sum(1 for item in checks if item["grade_profile_review"]),
            ...
            "ready_for_m68_06": bool(checks),
        },
        "consistency_checks": checks,
        "next_target": {
            "milestone": "M68-06",
            "task": "Ninth-slice blocker repair candidates",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- G Zone deferred checks: `{summary['g_zone_deferred_check_count']}`",
        f"- Stride deferred checks: `{summary['stride_deferred_check_count']}`",
        f"- Aqua Force battle-order deferred checks: `{summary['aqua_force_battle_order_deferred_check_count']}`",
        f"- Grade-profile review checks: `{summary['grade_profile_review_check_count']}`",
        f"- Promotion allowed: `{summary['promotion_allowed_count']}`",
        f"- Runtime-ready consistent: `{summary['runtime_ready_consistent_count']}`",
        f"- Ready for M68-06: `{summary['ready_for_m68_06']}`",
        "",
        "## Status Counts",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "- Stride/G-unit deferred remains visible as a review dependency.",
            "- Aqua Force battle-order deferred remains visible as a review dependency.",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check M68-05 ninth-slice combo-to-recipe consistency.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_ninth_slice_consistency_report()
    json_path = args.output_dir / "m68_05_ninth_slice_combo_recipe_consistency_report.json"
    md_path = args.output_dir / "m68_05_ninth_slice_combo_recipe_consistency_report.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M68-05 ninth-slice combo recipe consistency wrote {json_path}")
    print(f"M68-05 ninth-slice combo recipe consistency summary wrote {md_path}")
    print(
        "ready_for_m68_06={ready} checked={checked} promotion_allowed={allowed}".format(
            ready=report["summary"]["ready_for_m68_06"],
            checked=report["summary"]["consistency_check_count"],
            allowed=report["summary"]["promotion_allowed_count"],
        )
    )
    return 0 if report["summary"]["ready_for_m68_06"] else 1
