"""Build M68-06 ninth-slice blocker repair candidates.

This creates advisory repair candidates for blockers reported by M68-04 and
carried through M68-05. It does not mutate M68-03 drafts and does not create
...
M68_RECIPE_DRAFTS = OUTPUT_DIR / "m68_03_ninth_slice_recipe_draft_model.json"
M68_VALIDATION = OUTPUT_DIR / "m68_04_ninth_slice_recipe_validation_report.json"
M68_CONSISTENCY = OUTPUT_DIR / "m68_05_ninth_slice_combo_recipe_consistency_report.json"
M68_SCAFFOLD = OUTPUT_DIR / "m68_01_ninth_slice_fixture_scaffold.json"
    ...
    rows = [
        {
            ...
            "source": "m68_03_ninth_slice_recipe_draft_model",
        }
        for card_id, quantity in quantities.items()
...
def _build_g_zone_package(recipe_id: str, validation: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        ...
        "advisory_only": True,
        "can_be_repaired_in_m68_06": False,
        "requires_future_system_work": [
            ...
            "Generation Break runtime support",
        ],
...
def _build_stride_package(recipe_id: str, validation: dict[str, Any]) -> dict[str, Any] | None:
    if "stride_support_deferred" not in _issues_by_code(validation):
        return None
    return {
        "package_id": f"{recipe_id}_stride_deferred_pkg_001",
        "repair_type": "stride_support_deferred_decision",
        "advisory_only": True,
        "can_be_repaired_in_m68_06": False,
        "requires_future_system_work": [
            "Stride declaration timing",
            "stride-cost payment validation",
            "heart-card and G unit state model",
            "end-phase G unit return handling",
        ],
        "main_deck_validation_only": True,
        "runtime_promotion_allowed": False,
    }
...
def _build_aqua_force_battle_order_package(recipe_id: str, validation: dict[str, Any]) -> dict[str, Any] | None:
    if "aqua_force_battle_order_support_deferred" not in _issues_by_code(validation):
        return None
    return {
        "package_id": f"{recipe_id}_aqua_force_battle_order_pkg_001",
        "repair_type": "aqua_force_battle_order_deferred_decision",
        "advisory_only": True,
        "can_be_repaired_in_m68_06": False,
        "requires_future_system_work": [
            "battle-count tracker",
            "attack-order timing predicates",
            "re-stand and multi-attack action labels",
            "battle-phase trigger window integration",
        ],
...
def build_ninth_slice_blocker_repair_candidates(
    recipe_report: dict[str, Any] | None = None,
    validation_report: dict[str, Any] | None = None,
    consistency_report: dict[str, Any] | None = None,
    scaffold: dict[str, Any] | None = None,
) -> dict[str, Any]:
    recipe_report = recipe_report or load_json(M68_RECIPE_DRAFTS)
    validation_report = validation_report or load_json(M68_VALIDATION)
    consistency_report = consistency_report or load_json(M68_CONSISTENCY)
    ...
    for recipe in recipe_report.get("recipe_drafts", []):
        ...
        stride_package = _build_stride_package(recipe_id, validation)
        aqua_force_package = _build_aqua_force_battle_order_package(recipe_id, validation)
        ...
        ready_for_human_repair_review = (
            ...
            and bool(consistency_item.get("pair_cards_present", False))
        )
        repair_items.append(
            {
                "recipe_id": recipe_id,
                ...
                "stride_deferred_package": stride_package,
                "aqua_force_battle_order_deferred_package": aqua_force_package,
                ...
                "blocked_by_stride_deferred": bool(stride_package),
                "blocked_by_aqua_force_battle_order_deferred": bool(aqua_force_package),
                "runtime_promotion_allowed": False,
            }
        )
    ...
    return {
        "version": "M68-06",
        "description": "Ninth-slice blocker repair candidates",
        "selected_target": recipe_report.get("selected_target", {}),
        "source_inputs": {
            "ninth_slice_recipe_draft_model": str(M68_RECIPE_DRAFTS.relative_to(ROOT)),
            "ninth_slice_recipe_validation_report": str(M68_VALIDATION.relative_to(ROOT)),
            "ninth_slice_combo_recipe_consistency_report": str(M68_CONSISTENCY.relative_to(ROOT)),
        ...
        "repair_policy": {
            ...
            "series_scope_from_m68_01_fixture_scaffold": series_scope,
            ...
            "stride_support_is_deferred_system_work": True,
            "aqua_force_battle_order_support_is_deferred_system_work": True,
            "runtime_promotion_allowed": False,
        },
        ...
        "summary": {
            ...
            "stride_deferred_recipe_count": sum(1 for item in repair_items if item["stride_deferred_package"]),
            "aqua_force_battle_order_deferred_recipe_count": sum(
                1 for item in repair_items if item["aqua_force_battle_order_deferred_package"]
            ),
            ...
            "runtime_promotion_allowed": False,
            "ready_for_m68_closeout": bool(repair_items) and unexpected_structural_blocker_count == 0,
        },
        "next_target": {
            "milestone": "M68-closeout",
            "task": "Ninth-slice runtime readiness decision",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M68-closeout: `{summary['ready_for_m68_closeout']}`",
        "",
        "## Repair Items",
        "",
    ]
    for item in report["repair_items"]:
        ...
        stride_package = item["stride_deferred_package"] or {}
        aqua_force_package = item["aqua_force_battle_order_deferred_package"] or {}
        lines.append(
            ...
            f"stride_deferred=`{bool(stride_package)}` "
            f"aqua_force_battle_order_deferred=`{bool(aqua_force_package)}` "
    ...
    lines.extend(
        [
            ...
            "- M68-03 draft files are not modified.",
            "- Manual-review cards are not resolved by this milestone.",
            "- G Zone, Stride, and Aqua Force battle-order support remain future system work.",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M68-06 ninth-slice blocker repair candidates.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_ninth_slice_blocker_repair_candidates()
    json_path = args.output_dir / "m68_06_ninth_slice_blocker_repair_candidates.json"
    md_path = args.output_dir / "m68_06_ninth_slice_blocker_repair_candidates.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M68-06 ninth-slice blocker repair candidates wrote {json_path}")
    print(f"M68-06 ninth-slice blocker repair candidates summary wrote {md_path}")
    print(
        "ready_for_m68_closeout={ready} recipes={recipes} human_review_ready={ready_count} "
        "g_zone_deferred={g_zone} stride_deferred={stride} aqua_force_deferred={aqua_force}".format(
            ready=report["summary"]["ready_for_m68_closeout"],
            ...
            stride=report["summary"]["stride_deferred_recipe_count"],
            aqua_force=report["summary"]["aqua_force_battle_order_deferred_recipe_count"],
        )
    )
    return 0 if report["summary"]["ready_for_m68_closeout"] else 1
