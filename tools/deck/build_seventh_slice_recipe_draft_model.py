"""Build M60-03 seventh-slice advisory recipe drafts."""
...
M60_REVIEW_PACKET = OUTPUT_DIR / "m60_02_seventh_slice_review_packet.json"
        ...
        rows.append(
            {
                ...
                "quantity_source": "m60_03_fixture_scaffold_plus_candidate_edge_pair",
            }
        )
    return rows
...
def _build_recipe(
    ...
    return {
        ...
        "validation_metadata": {
            "draft_only": True,
            "requires_m60_04_validator": True,
            ...
            "bloom_token_support_deferred": True,
...
def build_seventh_slice_recipe_drafts(
    review_packet: dict[str, Any] | None = None,
    scaffold: dict[str, Any] | None = None,
    draft_limit: int = DEFAULT_DRAFT_LIMIT,
) -> dict[str, Any]:
    review_packet = review_packet or load_json(M60_REVIEW_PACKET)
    scaffold = scaffold or load_json(M60_SCAFFOLD)
    if not review_packet.get("summary", {}).get("ready_for_m60_03"):
        raise ValueError("M60-02 review packet is not ready for M60-03.")
    if not scaffold.get("summary", {}).get("ready_for_m60_02"):
        raise ValueError("M60-01 fixture scaffold is not ready for M60-03.")
    ...
    return {
        "version": "M60-03",
        "description": "Seventh-slice advisory recipe draft model from M60-02 candidate edges",
        "selected_target": review_packet.get("selected_target", {}),
        "source_inputs": {
            "seventh_slice_review_packet": str(M60_REVIEW_PACKET.relative_to(ROOT)),
        ...
        "draft_policy": {
            "pair_anchored_from_m60_02_candidate_edges": True,
            "fixture_scaffold_source": "m60_01_seventh_slice_fixture_scaffold",
        ...
        "summary": {
            ...
            "ready_for_m60_04": bool(recipes) and quantity_complete_count == len(recipes),
        },
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M60-04: `{summary['ready_for_m60_04']}`",
        "",
        "## Drafts",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "- M60-04 must validate deck legality and manual-review blockers.",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M60-03 seventh-slice recipe draft model.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_seventh_slice_recipe_drafts(draft_limit=args.draft_limit)
    json_path = args.output_dir / "m60_03_seventh_slice_recipe_draft_model.json"
    md_path = args.output_dir / "m60_03_seventh_slice_recipe_draft_model.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M60-03 recipe draft model wrote {json_path}")
    print(f"M60-03 recipe draft model summary wrote {md_path}")
    print(
        "ready={ready} drafts={drafts} complete={complete} skipped={skipped}".format(
            ready=report["summary"]["ready_for_m60_04"],
    ...
    return 0 if report["summary"]["ready_for_m60_04"] else 1
