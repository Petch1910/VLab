"""Build and verify the M50-03 fourth fixture headless load smoke artifacts."""
...
DEFAULT_M50_02_REPORT = OUTPUT_DIR / "m50_02_fourth_fixture_deck_text_export.json"
DEFAULT_DECK_TEXT = OUTPUT_DIR / "m50_02_fourth_fixture_deck_text_export.txt"
CARDS_SQLITE = ROOT / "data" / "packs" / "vanguard_th" / "cards.sqlite"

DECK_CODE_OUTPUT = OUTPUT_DIR / "m50_03_fourth_fixture_deck_code.txt"
JSON_OUTPUT = OUTPUT_DIR / "m50_03_fourth_fixture_load_smoke.json"
MD_OUTPUT = OUTPUT_DIR / "m50_03_fourth_fixture_load_smoke.md"
DEFAULT_UNITY_RESULT = OUTPUT_DIR / "m50_03_fourth_fixture_unity_result.json"
DEFAULT_UNITY_REPLAY = OUTPUT_DIR / "m50_03_fourth_fixture_unity_replay.json"
...
def _build_deck_object(parsed: dict[str, Any]) -> dict[str, Any]:
    ...
    deck_id = "m5003" + hashlib.sha256(digest_source.encode("utf-8")).hexdigest()[:26]
    return {
        "deck_id": deck_id,
        "name": metadata["Name"],
...
def build_fourth_headless_fixture_load_smoke(
    fixture: dict[str, Any] | None = None,
    m50_02_report: dict[str, Any] | None = None,
    ...
    m50_02_report = m50_02_report or load_json(DEFAULT_M50_02_REPORT)
    deck_text = deck_text if deck_text is not None else DEFAULT_DECK_TEXT.read_text(encoding="utf-8")
    issues: list[dict[str, Any]] = []

    if not m50_02_report.get("summary", {}).get("export_ready"):
        issues.append(
            _issue(
                "m50_02_export_not_ready",
                "blocker",
                "M50-02 deck text export must be ready before headless load smoke.",
                {"summary": m50_02_report.get("summary", {})},
            )
        )
    ...
    if parsed["zones"]["G"]:
        issues.append(_issue("g_zone_not_empty", "blocker", "Fourth fixture G section must stay empty.", {}))
    ...
    ready_for_m50_04 = offline_load_ready and unity_smoke_passed

    return {
        "version": "M50-03",
        "description": "Fourth fixture headless load smoke",
        "source_inputs": {
            "runtime_fixture": str(DEFAULT_FIXTURE.relative_to(ROOT)),
            "m50_02_report": str(DEFAULT_M50_02_REPORT.relative_to(ROOT)),
        ...
        "scope": {
            ...
            "fourth_fixture_headless_load_smoke": True,
        ...
        "summary": {
            ...
            "ready_for_m50_04": ready_for_m50_04,
        },
        "next_target": {
            "milestone": "M50-04",
            "task": "Four-fixture scale decision",
        },
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M50-04: `{summary['ready_for_m50_04']}`",
        "",
        "## Unity Headless",
        "",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M50-03 fourth fixture headless load smoke artifacts.")
    parser.add_argument("--fixture", type=Path, default=DEFAULT_FIXTURE)
    parser.add_argument("--m50-02-report", type=Path, default=DEFAULT_M50_02_REPORT)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    m50_02_report = load_json(args.m50_02_report)
    ...
    report["source_inputs"]["m50_02_report"] = str(args.m50_02_report)
    ...
    print(f"M50-03 fourth fixture headless load smoke wrote {json_path}")
    print(f"M50-03 fourth fixture headless load smoke summary wrote {md_path}")
    if report["summary"]["deck_code_created"]:
        print(f"M50-03 fourth fixture deck code wrote {deck_code_path}")
    print(
        "offline_ready={offline} unity_provided={provided} unity_passed={passed} blockers={blockers} next={next_ready}".format(
            ...
            next_ready=report["summary"]["ready_for_m50_04"],
        )
    )
