"""Build and verify the M54-03 fifth fixture headless load smoke artifacts."""
...
from tools.deck.build_fourth_headless_fixture_load_smoke import (
...
DEFAULT_M54_02_REPORT = OUTPUT_DIR / "m54_02_fifth_fixture_deck_text_export.json"
DEFAULT_DECK_TEXT = OUTPUT_DIR / "m54_02_fifth_fixture_deck_text_export.txt"
CARDS_SQLITE = ROOT / "data" / "packs" / "vanguard_th" / "cards.sqlite"

DECK_CODE_OUTPUT = OUTPUT_DIR / "m54_03_fifth_fixture_deck_code.txt"
JSON_OUTPUT = OUTPUT_DIR / "m54_03_fifth_fixture_load_smoke.json"
MD_OUTPUT = OUTPUT_DIR / "m54_03_fifth_fixture_load_smoke.md"
DEFAULT_UNITY_RESULT = OUTPUT_DIR / "m54_03_fifth_fixture_unity_result.json"
DEFAULT_UNITY_REPLAY = OUTPUT_DIR / "m54_03_fifth_fixture_unity_replay.json"
...
def _build_deck_object(parsed: dict[str, Any]) -> dict[str, Any]:
    ...
    deck_id = "m5403" + hashlib.sha256(digest_source.encode("utf-8")).hexdigest()[:26]
    return {
        "deck_id": deck_id,
        "name": metadata["Name"],
...
def build_fifth_headless_fixture_load_smoke(
    fixture: dict[str, Any] | None = None,
    m54_02_report: dict[str, Any] | None = None,
    ...
    m54_02_report = m54_02_report or load_json(DEFAULT_M54_02_REPORT)
    deck_text = deck_text if deck_text is not None else DEFAULT_DECK_TEXT.read_text(encoding="utf-8")
    issues: list[dict[str, Any]] = []

    if not m54_02_report.get("summary", {}).get("export_ready"):
        issues.append(
            _issue(
                "m54_02_export_not_ready",
                "blocker",
                "M54-02 deck text export must be ready before headless load smoke.",
                {"summary": m54_02_report.get("summary", {})},
            )
        )
    ...
    if parsed["zones"]["G"]:
        issues.append(_issue("g_zone_not_empty", "blocker", "Fifth fixture G section must stay empty.", {}))
    ...
    ready_for_m54_04 = offline_load_ready and unity_smoke_passed

    return {
        "version": "M54-03",
        "description": "Fifth fixture headless load smoke",
        "source_inputs": {
            "runtime_fixture": str(DEFAULT_FIXTURE.relative_to(ROOT)),
            "m54_02_report": str(DEFAULT_M54_02_REPORT.relative_to(ROOT)),
        ...
        "scope": {
            ...
            "fifth_fixture_headless_load_smoke": True,
        ...
        "summary": {
            ...
            "ready_for_m54_04": ready_for_m54_04,
        },
        "next_target": {
            "milestone": "M54-04",
            "task": "Multi-fixture scale decision",
        },
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M54-04: `{summary['ready_for_m54_04']}`",
        "",
        "## Unity Headless",
        "",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M54-03 fifth fixture headless load smoke artifacts.")
    parser.add_argument("--fixture", type=Path, default=DEFAULT_FIXTURE)
    parser.add_argument("--m54-02-report", type=Path, default=DEFAULT_M54_02_REPORT)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    m54_02_report = load_json(args.m54_02_report)
    deck_text = args.deck_text.read_text(encoding="utf-8")
    report = build_fifth_headless_fixture_load_smoke(
        fixture,
        m54_02_report,
        deck_text,
        args.unity_result,
    ...
    report["source_inputs"]["m54_02_report"] = str(args.m54_02_report)
    ...
    print(f"M54-03 fifth fixture headless load smoke wrote {json_path}")
    print(f"M54-03 fifth fixture headless load smoke summary wrote {md_path}")
    if report["summary"]["deck_code_created"]:
        print(f"M54-03 fifth fixture deck code wrote {deck_code_path}")
    print(
        "offline_ready={offline} unity_provided={provided} unity_passed={passed} blockers={blockers} next={next_ready}".format(
            ...
            next_ready=report["summary"]["ready_for_m54_04"],
        )
    )
