"""Build the M57-06 sixth-slice runtime fixture promotion gate."""
...
M57_03_ACCEPTED_REPAIR = OUTPUT_DIR / "m57_03_sixth_slice_human_accepted_repair_artifact.json"
M57_05_VALIDATION = OUTPUT_DIR / "m57_05_sixth_slice_repaired_recipe_validation_report.json"
...
def _fixture_filename(recipe_id: str, selected_target: dict[str, Any]) -> str:
    return f"{recipe_id}_{_group_slug(str(selected_target.get('group', '')))}_m57_06.json"


def _fixture_id(recipe_id: str, selected_target: dict[str, Any]) -> str:
    return f"runtime_fixture_{recipe_id}_{_group_slug(str(selected_target.get('group', '')))}_m57_06"
...
def build_runtime_fixture(
    ...
    context = validation_report["accepted_context"]
    g_zone = validation_report["g_zone_decision_context"]
    ...
    return {
        ...
        "recipe_id": recipe_id,
        "source_artifacts": {
            "m57_03_human_accepted_repair_artifact": str(M57_03_ACCEPTED_REPAIR.relative_to(ROOT)),
            "m57_05_repaired_recipe_validation_report": str(M57_05_VALIDATION.relative_to(ROOT)),
        },
        ...
        "source_packages": {
            "selected_review_item_id": context.get("accepted_review_item_id", ""),
            "combined_repair_package_id": context.get("accepted_combined_repair_package_id", ""),
            "g_zone_package_id": context.get("accepted_g_zone_package_id", ""),
            "g_zone_decision_item_id": g_zone.get("source_decision_item_id", ""),
            "selected_g_zone_option_id": g_zone.get("selected_option_id", ""),
        },
        "g_zone_boundary": {
            "selected_option_id": g_zone.get("selected_option_id", ""),
            "main_deck_only_validation_allowed": bool(g_zone.get("main_deck_only_boundary_applied")),
...
def build_sixth_slice_runtime_fixture_promotion_gate(
    accepted_artifact: dict[str, Any] | None = None,
    validation_report: dict[str, Any] | None = None,
) -> dict[str, Any]:
    accepted_artifact = accepted_artifact or load_json(M57_03_ACCEPTED_REPAIR)
    validation_report = validation_report or load_json(M57_05_VALIDATION)
    ...
    checks = [
        _check(
            ...
            bool(context.get("human_selection_recorded"))
            and bool(context.get("human_acceptance_recorded"))
            and bool(accepted_summary.get("human_selection_recorded"))
            and bool(accepted_summary.get("human_acceptance_recorded")),
            {
                "validation_human_selection_recorded": context.get("human_selection_recorded"),
                "validation_human_acceptance_recorded": context.get("human_acceptance_recorded"),
        ...
        _check(
            "g_zone_boundary",
            g_zone.get("selected_option_id") == "main_deck_only_review_no_runtime_promotion"
            and bool(g_zone.get("main_deck_only_boundary_applied"))
            and not bool(g_zone.get("g_zone_runtime_enabled"))
            and not bool(g_zone.get("stride_runtime_enabled")),
            {
                "selected_option_id": g_zone.get("selected_option_id", ""),
                "main_deck_only_boundary_applied": g_zone.get("main_deck_only_boundary_applied"),
                "g_zone_runtime_enabled": g_zone.get("g_zone_runtime_enabled"),
                "stride_runtime_enabled": g_zone.get("stride_runtime_enabled"),
            },
        ),
        _check(
            "validation",
        ...
        _check(
            "rerun_ready",
            bool(summary.get("ready_for_m57_06")),
            {"ready_for_m57_06": summary.get("ready_for_m57_06")},
        ),
        _check(
            "runtime_boundary",
            ...
            {
                "m57_05_runtime_fixture_created": summary.get("runtime_fixture_created"),
                "m57_05_runtime_promotion_allowed": summary.get("runtime_promotion_allowed"),
                "m57_05_creates_runtime_fixture": validation_report["scope"].get("creates_runtime_fixture"),
                "m57_05_saved_deck_injection": validation_report["scope"].get("saved_deck_injection"),
                "m57_05_ui_deck_list_publication": validation_report["scope"].get("ui_deck_list_publication"),
                "m57_05_bot_playbook": validation_report["scope"].get("bot_playbook"),
                "m57_05_direct_GameState_mutation": validation_report["scope"].get("direct_GameState_mutation"),
                "m57_05_g_zone_runtime_enabled": validation_report["scope"].get("g_zone_runtime_enabled"),
                "m57_05_stride_runtime_enabled": validation_report["scope"].get("stride_runtime_enabled"),
            },
        ),
    ]
    ...
    return {
        "version": "M57-06",
        "description": "Sixth-slice runtime fixture promotion gate",
        "source_inputs": {
            "m57_03_human_accepted_repair_artifact": str(M57_03_ACCEPTED_REPAIR.relative_to(ROOT)),
            "m57_05_repaired_recipe_validation_report": str(M57_05_VALIDATION.relative_to(ROOT)),
        },
        ...
        "promotion_decision": {
            ...
            "g_zone_runtime_enabled": False,
            "stride_runtime_enabled": False,
            "next_action": "M57-closeout" if promotion_allowed else "repair_failed_gate_checks",
        },
        ...
        "summary": {
            ...
            "g_zone_runtime_enabled": False,
            "stride_runtime_enabled": False,
            "ready_for_m57_closeout": promotion_allowed,
        },
        "next_target": {
            "milestone": "M57-closeout",
            "task": "Sixth-slice fixture closeout",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M57-closeout: `{summary['ready_for_m57_closeout']}`",
        "",
        "## Gate Checks",
        "",
    ]
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M57-06 sixth-slice runtime fixture promotion gate.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_sixth_slice_runtime_fixture_promotion_gate()
    json_path = args.output_dir / "m57_06_sixth_slice_runtime_fixture_promotion_gate.json"
    md_path = args.output_dir / "m57_06_sixth_slice_runtime_fixture_promotion_gate.md"
    ...
    print(f"M57-06 sixth-slice runtime fixture promotion gate wrote {json_path}")
    print(f"M57-06 sixth-slice runtime fixture promotion gate summary wrote {md_path}")
    if fixture:
        print(f"M57-06 sixth-slice runtime fixture wrote {fixture_path}")
    print(
