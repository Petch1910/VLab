"""Check M64-05 eighth-slice combo-to-recipe consistency."""
...
def build_eighth_slice_consistency_report(
    ...
    for recipe in recipe_report.get("recipe_drafts", []):
        ...
        lock_deferred = _has_issue(validation, "lock_runtime_support_deferred")
        legion_deferred = _has_issue(validation, "legion_runtime_support_deferred")
        checks.append(
            {
                "recipe_id": recipe_id,
                ...
                "lock_runtime_support_deferred": lock_deferred,
                "legion_runtime_support_deferred": legion_deferred,
    ...
    return {
        "version": "M64-05",
        "description": "Eighth-slice candidate edge to recipe consistency check",
        ...
        "summary": {
            ...
            "lock_deferred_check_count": sum(1 for item in checks if item["lock_runtime_support_deferred"]),
            "legion_deferred_check_count": sum(1 for item in checks if item["legion_runtime_support_deferred"]),
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Lock deferred checks: `{summary['lock_deferred_check_count']}`",
        f"- Legion deferred checks: `{summary['legion_deferred_check_count']}`",
        f"- Promotion allowed: `{summary['promotion_allowed_count']}`",
        f"- Runtime-ready consistent: `{summary['runtime_ready_consistent_count']}`",
        f"- Ready for M64-06: `{summary['ready_for_m64_06']}`",
        "",
        "## Status Counts",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "- Lock deferred remains visible as a review dependency.",
            "- Legion deferred remains visible as a review dependency.",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check M64-05 eighth-slice combo-to-recipe consistency.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_eighth_slice_consistency_report()
    json_path = args.output_dir / "m64_05_eighth_slice_combo_recipe_consistency_report.json"
    md_path = args.output_dir / "m64_05_eighth_slice_combo_recipe_consistency_report.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M64-05 eighth-slice combo recipe consistency wrote {json_path}")
    print(f"M64-05 eighth-slice combo recipe consistency summary wrote {md_path}")
    print(
        "ready_for_m64_06={ready} checked={checked} promotion_allowed={allowed}".format(
            ready=report["summary"]["ready_for_m64_06"],
            checked=report["summary"]["consistency_check_count"],
            allowed=report["summary"]["promotion_allowed_count"],
        )
    )
    return 0 if report["summary"]["ready_for_m64_06"] else 1
