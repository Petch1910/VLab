"""Check M56-05 sixth-slice combo-to-recipe consistency."""
...
def build_sixth_slice_consistency_report(
    ...
    return {
        "version": "M56-05",
        "description": "Sixth-slice candidate edge to recipe consistency check",
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M56-06: `{summary['ready_for_m56_06']}`",
        "",
        "## Status Counts",
        "",
    ]
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check M56-05 sixth-slice combo-to-recipe consistency.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_sixth_slice_consistency_report()
    json_path = args.output_dir / "m56_05_sixth_slice_combo_recipe_consistency_report.json"
    md_path = args.output_dir / "m56_05_sixth_slice_combo_recipe_consistency_report.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M56-05 sixth-slice combo recipe consistency wrote {json_path}")
    print(f"M56-05 sixth-slice combo recipe consistency summary wrote {md_path}")
    print(
        "ready_for_m56_06={ready} checked={checked} promotion_allowed={allowed}".format(
            ready=report["summary"]["ready_for_m56_06"],
            checked=report["summary"]["consistency_check_count"],
            allowed=report["summary"]["promotion_allowed_count"],
        )
    )
    return 0 if report["summary"]["ready_for_m56_06"] else 1
