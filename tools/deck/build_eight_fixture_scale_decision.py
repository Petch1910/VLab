"""Build the M66-04 eight-fixture scale decision artifact."""
...
M66_03_SMOKE = OUTPUT_DIR / "m66_03_eighth_fixture_load_smoke.json"
JSON_OUTPUT = OUTPUT_DIR / "m66_04_eight_fixture_scale_decision.json"
MD_OUTPUT = OUTPUT_DIR / "m66_04_eight_fixture_scale_decision.md"
...
def build_eight_fixture_scale_decision(
    ...
    scale_allowed = not failed and len(fixture_evidence) == 8 and bool(candidates)

    return {
        "version": "M66-04",
        "description": "Eight-fixture scale decision",
        "source_inputs": {
        ...
        "decision": {
            "ninth_slice_offline_pipeline_allowed": scale_allowed,
            "ninth_slice_selected_now": False,
            "recommended_next_action": "M67-01 ninth target slice selection" if scale_allowed else "repair_failed_fixture_evidence",
            ...
            "mate_identity_check_enabled": False,
            "decision_notes": [
                "Eight independent fixtures now pass offline and Unity headless load smoke.",
                "The next step may define and select a ninth slice for offline analysis only.",
                "Do not publish fixture decks to UI or bot runtime without a later explicit gate.",
                "G Zone, Stride, Bloom/token, Lock, Unlock, Legion, and Mate runtime remain disabled.",
            ]
            if scale_allowed
            else [
        ...
        "summary": {
            ...
            "ninth_slice_offline_pipeline_allowed": scale_allowed,
            "ready_for_m67": scale_allowed,
        },
        "next_target": {
            "milestone": "M67-01" if scale_allowed else "M66-repair",
            "task": "Ninth target slice selection" if scale_allowed else "Repair failed fixture evidence",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ninth-slice offline pipeline allowed: `{summary['ninth_slice_offline_pipeline_allowed']}`",
        f"- Ready for M67: `{summary['ready_for_m67']}`",
        "",
        "## Fixture Evidence",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "## Decision",
            "",
            f"- Ninth slice selected now: `{decision['ninth_slice_selected_now']}`",
...
def parse_args(argv: Any = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M66-04 eight-fixture scale decision.")
...
def main(argv: Any = None) -> int:
    ...
    print(f"M66-04 eight-fixture scale decision wrote {json_path}")
    print(f"M66-04 eight-fixture scale decision summary wrote {md_path}")
    print(
        "ready_for_m67={ready} fixtures_passed={passed} candidates={candidates}".format(
            ready=report["summary"]["ready_for_m67"],
            passed=report["summary"]["passed_fixture_count"],
            candidates=report["summary"]["candidate_count"],
        )
    )
    return 0 if report["summary"]["ready_for_m67"] else 1
