"""Build M68-03 ninth-slice advisory recipe drafts."""
...
M68_REVIEW_PACKET = OUTPUT_DIR / "m68_02_ninth_slice_review_packet.json"
        ...
        rows.append(
            {
                ...
                "quantity_source": "m68_03_fixture_scaffold_plus_candidate_edge_pair",
            }
        )
    return rows
...
def _build_recipe(
    ...
    review_blockers = [
        ...
        "m68_05_combo_recipe_consistency_check",
        "g_zone_stride_manual_review_policy",
        "aqua_force_battle_order_manual_review_policy",
        "grade4_g_zone_support_decision",
    ]
    if manual_overlap:
        review_blockers.append("manual_card_semantic_review")
    return {
        "recipe_id": f"m68_recipe_{index:03d}",
        "source_candidate_edge": edge["edge"],
        "source_edge_rank": edge["edge_rank"],
        "source_line_id": edge["edge"],
        "source_package_id": "m68_03_pair_anchored_fixture_scaffold",
        ...
        "validation_metadata": {
            "draft_only": True,
            "requires_m68_04_validator": True,
            ...
            "g_zone_support_deferred": True,
            "stride_support_deferred": True,
            "aqua_force_battle_order_support_deferred": True,
...
def build_ninth_slice_recipe_drafts(
    review_packet: dict[str, Any] | None = None,
    scaffold: dict[str, Any] | None = None,
    draft_limit: int = DEFAULT_DRAFT_LIMIT,
) -> dict[str, Any]:
    review_packet = review_packet or load_json(M68_REVIEW_PACKET)
    scaffold = scaffold or load_json(M68_SCAFFOLD)
    if not review_packet.get("summary", {}).get("ready_for_m68_03"):
        raise ValueError("M68-02 review packet is not ready for M68-03.")
    if not scaffold.get("summary", {}).get("ready_for_m68_02"):
        raise ValueError("M68-01 fixture scaffold is not ready for M68-03.")
    ...
    return {
        "version": "M68-03",
        "description": "Ninth-slice advisory recipe draft model from M68-02 candidate edges",
        "selected_target": review_packet.get("selected_target", {}),
        "source_inputs": {
            "ninth_slice_review_packet": str(M68_REVIEW_PACKET.relative_to(ROOT)),
        ...
        "summary": {
            ...
            "ready_for_m68_04": bool(recipes) and quantity_complete_count == len(recipes),
        },
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M68-04: `{summary['ready_for_m68_04']}`",
        "",
        "## Drafts",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "- M68-04 must validate deck legality and manual-review blockers.",
            "- Grade 4 / G Zone support remains deferred.",
            "- Stride, G-unit, Generation Break, and Aqua Force battle-order text remains manual-review only.",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M68-03 ninth-slice recipe draft model.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_ninth_slice_recipe_drafts(draft_limit=args.draft_limit)
    json_path = args.output_dir / "m68_03_ninth_slice_recipe_draft_model.json"
    md_path = args.output_dir / "m68_03_ninth_slice_recipe_draft_model.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M68-03 recipe draft model wrote {json_path}")
    print(f"M68-03 recipe draft model summary wrote {md_path}")
    print(
        "ready={ready} drafts={drafts} complete={complete} skipped={skipped}".format(
            ready=report["summary"]["ready_for_m68_04"],
    ...
    return 0 if report["summary"]["ready_for_m68_04"] else 1
