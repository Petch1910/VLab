"""Validate M52-03 fifth-slice recipe drafts for M52-04."""
...
RECIPE_DRAFTS = OUTPUT_DIR / "m52_03_fifth_slice_recipe_draft_model.json"
    ...
    if manual_ids:
        issues.append(
            _issue(
                ...
                "blocker",
                "Recipe contains cards that M52-02 marked as manual-review required.",
...
def build_fifth_slice_validation_report(
    ...
    return {
        "version": "M52-04",
        "description": "Fifth-slice recipe validator for M52-03 advisory drafts",
        "selected_target": recipe_report.get("selected_target", {}),
        "source_inputs": {
            "fifth_slice_recipe_draft_model": str(RECIPE_DRAFTS.relative_to(ROOT)),
        ...
        "summary": {
            ...
            "ready_for_m52_05": bool(validations),
        },
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M52-05: `{summary['ready_for_m52_05']}`",
        "",
        "## Recipe Status",
        "",
    ]
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate M52-03 fifth-slice recipe drafts.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_fifth_slice_validation_report()
    json_path = args.output_dir / "m52_04_fifth_slice_recipe_validation_report.json"
    md_path = args.output_dir / "m52_04_fifth_slice_recipe_validation_report.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M52-04 fifth-slice recipe validation wrote {json_path}")
    print(f"M52-04 fifth-slice recipe validation summary wrote {md_path}")
    print(
        "ready_for_m52_05={ready} runtime_ready={runtime_ready} manual_blocked={manual_blocked}".format(
            ready=report["summary"]["ready_for_m52_05"],
            runtime_ready=report["summary"]["runtime_ready_recipe_count"],
            manual_blocked=report["summary"]["blocked_by_manual_review_count"],
        )
    )
    return 0 if report["summary"]["ready_for_m52_05"] else 1
