"""Build the M43-01 third target slice selection artifact."""
...
M42_04_DECISION = OUTPUT_DIR / "m42_04_multi_fixture_scale_decision.json"
...
def build_third_target_slice_selection(m42_decision: dict[str, Any] | None = None) -> dict[str, Any]:
    m42_decision = m42_decision or load_json(M42_04_DECISION)
    summary = m42_decision.get("summary", {})
    candidates = list(m42_decision.get("candidate_queue", []))
    selection_allowed = bool(summary.get("third_slice_offline_pipeline_allowed")) and bool(candidates)
    selected = candidates[0] if selection_allowed else {}
    selected_target = (
        {
            "slice": "Third Offline Slice",
    ...
    return {
        "version": "M43-01",
        "description": "Third target slice selection",
        "source_inputs": {
            "multi_fixture_scale_decision": str(M42_04_DECISION.relative_to(ROOT)),
        },
        "scope": {
        ...
        "selection_policy": {
            "requires_m42_scale_allowed": True,
        ...
        "decision": {
            ...
            "third_slice_selected": bool(selected_target),
        ...
        "summary": {
            ...
            "ready_for_m43_02": bool(selected_target),
        },
        "next_target": {
            "milestone": "M43-02" if selected_target else "M42-repair",
            "task": "Third-slice fixture/format readiness" if selected_target else "Repair multi-fixture scale decision",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        "## Summary",
        "",
        f"- Third slice selected: `{summary['third_slice_selected']}`",
        ...
        f"- Ready for M43-02: `{summary['ready_for_m43_02']}`",
        "",
        "## Selected Target",
        "",
    ]
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M43-01 third target slice selection.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_third_target_slice_selection()
    json_path = args.output_dir / "m43_01_third_target_slice_selection.json"
    md_path = args.output_dir / "m43_01_third_target_slice_selection.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M43-01 third target slice selection wrote {json_path}")
    print(f"M43-01 third target slice selection summary wrote {md_path}")
    print(
        "selected={selected} group={group} next={next_ready}".format(
            selected=report["summary"]["third_slice_selected"],
            group=report["summary"]["selected_group"],
            next_ready=report["summary"]["ready_for_m43_02"],
        )
    )
    return 0 if report["summary"]["ready_for_m43_02"] else 1
