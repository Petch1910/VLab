"""Build the M65-06 eighth-slice runtime fixture promotion gate."""
...
M65_05_VALIDATION = OUTPUT_DIR / "m65_05_eighth_slice_repaired_recipe_validation_report.json"
...
def _fixture_filename(recipe_id: str, selected_target: dict[str, Any]) -> str:
    return f"{recipe_id}_{_group_slug(str(selected_target.get('group', '')))}_m65_06.json"


def _fixture_id(recipe_id: str, selected_target: dict[str, Any]) -> str:
    return f"runtime_fixture_{recipe_id}_{_group_slug(str(selected_target.get('group', '')))}_m65_06"
...
def build_runtime_fixture(
    ...
    return {
        ...
        "source_artifacts": {
            ...
            "m65_05_repaired_recipe_validation_report": str(M65_05_VALIDATION.relative_to(ROOT)),
        },
        ...
        "source_packages": {
            ...
            "accepted_grade_repair_package_id": accepted_context.get("accepted_grade_repair_package_id", ""),
            "lock_package_id": accepted_context.get("selected_lock_package_id", ""),
            "legion_package_id": accepted_context.get("selected_legion_package_id", ""),
            "system_decision_item_id": system_context.get("source_decision_item_id", ""),
            "selected_lock_option_id": system_context.get("selected_lock_option_id", ""),
            "selected_legion_option_id": system_context.get("selected_legion_option_id", ""),
        },
        "system_boundaries": {
            "selected_lock_option_id": system_context.get("selected_lock_option_id", ""),
            "selected_legion_option_id": system_context.get("selected_legion_option_id", ""),
            "main_deck_only_validation_allowed": bool(system_context.get("main_deck_only_boundary_applied")),
            "lock_boundary_applied": bool(system_context.get("lock_boundary_applied")),
            "legion_boundary_applied": bool(system_context.get("legion_boundary_applied")),
...
def build_eighth_slice_runtime_fixture_promotion_gate(
    accepted_artifact: dict[str, Any] | None = None,
    validation_report: dict[str, Any] | None = None,
) -> dict[str, Any]:
    accepted_artifact = accepted_artifact or load_json(M65_03_ACCEPTED_REPAIR)
    validation_report = validation_report or load_json(M65_05_VALIDATION)
    ...
    checks = [
        _check(
            ...
            and bool(accepted_context.get("grade_repair_accepted"))
            ...
            and bool(accepted_summary.get("grade_repair_accepted")),
            {
                ...
                "validation_grade_repair_accepted": accepted_context.get("grade_repair_accepted"),
                ...
                "accepted_grade_repair_accepted": accepted_summary.get("grade_repair_accepted"),
                "selected_review_item_id": accepted_context.get("selected_review_item_id", ""),
            },
        ),
        _check(
            "lock_legion_boundary",
            system_context.get("selected_lock_option_id") == "main_deck_only_review_no_runtime_promotion"
            and system_context.get("selected_legion_option_id") == "main_deck_only_review_no_runtime_promotion"
            and bool(system_context.get("lock_decision_recorded"))
            and bool(system_context.get("legion_decision_recorded"))
            and bool(system_context.get("main_deck_only_boundary_applied"))
            and bool(system_context.get("lock_boundary_applied"))
            and bool(system_context.get("legion_boundary_applied"))
            and not bool(system_context.get("lock_runtime_enabled"))
            and not bool(system_context.get("unlock_runtime_enabled"))
            and not bool(system_context.get("legion_runtime_enabled"))
            and not bool(system_context.get("mate_identity_check_enabled")),
            {
                "selected_lock_option_id": system_context.get("selected_lock_option_id", ""),
                "selected_legion_option_id": system_context.get("selected_legion_option_id", ""),
                "lock_decision_recorded": system_context.get("lock_decision_recorded"),
                "legion_decision_recorded": system_context.get("legion_decision_recorded"),
                "main_deck_only_boundary_applied": system_context.get("main_deck_only_boundary_applied"),
                "lock_boundary_applied": system_context.get("lock_boundary_applied"),
                "legion_boundary_applied": system_context.get("legion_boundary_applied"),
                "lock_runtime_enabled": system_context.get("lock_runtime_enabled"),
                "unlock_runtime_enabled": system_context.get("unlock_runtime_enabled"),
                "legion_runtime_enabled": system_context.get("legion_runtime_enabled"),
                "mate_identity_check_enabled": system_context.get("mate_identity_check_enabled"),
            },
        ),
        _check(
            "validation",
        ...
        _check(
            ...
            and not bool(consistency.get("lock_runtime_support_deferred"))
            and not bool(consistency.get("legion_runtime_support_deferred")),
            {
                ...
                "lock_runtime_support_deferred": consistency.get("lock_runtime_support_deferred"),
                "legion_runtime_support_deferred": consistency.get("legion_runtime_support_deferred"),
            },
        ),
        _check(
        ...
        _check(
            "rerun_ready",
            bool(summary.get("ready_for_m65_06")),
            {"ready_for_m65_06": summary.get("ready_for_m65_06")},
        ),
        _check(
            "runtime_boundary",
            ...
            and not bool(validation_report["scope"].get("lock_runtime_enabled"))
            and not bool(validation_report["scope"].get("unlock_runtime_enabled"))
            and not bool(validation_report["scope"].get("legion_runtime_enabled"))
            and not bool(validation_report["scope"].get("mate_identity_check_enabled")),
            {
                "m65_05_runtime_fixture_created": summary.get("runtime_fixture_created"),
                "m65_05_runtime_promotion_allowed": summary.get("runtime_promotion_allowed"),
                "m65_05_creates_runtime_fixture": validation_report["scope"].get("creates_runtime_fixture"),
                "m65_05_saved_deck_injection": validation_report["scope"].get("saved_deck_injection"),
                "m65_05_ui_deck_list_publication": validation_report["scope"].get("ui_deck_list_publication"),
                "m65_05_bot_playbook": validation_report["scope"].get("bot_playbook"),
                "m65_05_direct_GameState_mutation": validation_report["scope"].get("direct_GameState_mutation"),
                "m65_05_lock_runtime_enabled": validation_report["scope"].get("lock_runtime_enabled"),
                "m65_05_unlock_runtime_enabled": validation_report["scope"].get("unlock_runtime_enabled"),
                "m65_05_legion_runtime_enabled": validation_report["scope"].get("legion_runtime_enabled"),
                "m65_05_mate_identity_check_enabled": validation_report["scope"].get(
    ...
    return {
        "version": "M65-06",
        "description": "Eighth-slice runtime fixture promotion gate",
        "source_inputs": {
            "m65_03_human_accepted_grade_repair_artifact": str(M65_03_ACCEPTED_REPAIR.relative_to(ROOT)),
            "m65_05_repaired_recipe_validation_report": str(M65_05_VALIDATION.relative_to(ROOT)),
        },
        ...
        "gate_policy": {
            ...
            "requires_grade_repair_acceptance": True,
            "requires_main_deck_only_lock_boundary": True,
            "requires_main_deck_only_legion_boundary": True,
            "requires_zero_blockers": True,
            "requires_combo_consistency": True,
            "requires_m65_05_ready": True,
        ...
        "promotion_decision": {
            ...
            "legion_runtime_enabled": False,
            "mate_identity_check_enabled": False,
            "next_action": "M65-closeout" if promotion_allowed else "repair_failed_gate_checks",
        },
        ...
        "summary": {
            ...
            "legion_runtime_enabled": False,
            "mate_identity_check_enabled": False,
            "ready_for_m65_closeout": promotion_allowed,
        },
        "next_target": {
            "milestone": "M65-closeout",
            "task": "Eighth-slice fixture closeout",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M65-closeout: `{summary['ready_for_m65_closeout']}`",
        "",
        "## Gate Checks",
        "",
    ]
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M65-06 eighth-slice runtime fixture promotion gate.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_eighth_slice_runtime_fixture_promotion_gate()
    json_path = args.output_dir / "m65_06_eighth_slice_runtime_fixture_promotion_gate.json"
    md_path = args.output_dir / "m65_06_eighth_slice_runtime_fixture_promotion_gate.md"
    ...
    print(f"M65-06 eighth-slice runtime fixture promotion gate wrote {json_path}")
    print(f"M65-06 eighth-slice runtime fixture promotion gate summary wrote {md_path}")
    if fixture:
        print(f"M65-06 eighth-slice runtime fixture wrote {fixture_path}")
    print(
