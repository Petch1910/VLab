"""Build the M67-04 ninth-slice recipe pipeline entry gate artifact."""
...
JSON_OUTPUT = OUTPUT_DIR / "m67_04_ninth_slice_recipe_pipeline_entry_gate.json"
MD_OUTPUT = OUTPUT_DIR / "m67_04_ninth_slice_recipe_pipeline_entry_gate.md"
...
def build_ninth_slice_recipe_pipeline_entry_gate(
    ...
    if not readiness.get("all_fixture_expectations_met"):
        issues.append(
            _issue(
                ...
                "blocker",
                "M67-02 fixture/format readiness must pass before recipe pipeline entry.",
                {"readiness": readiness},
            )
        )
    if not readiness.get("semantic_probe_ready"):
        issues.append(
            _issue(
                ...
                "blocker",
                "M67-02 did not mark the selected slice ready for semantic probe.",
                {"readiness": readiness},
            )
        )
    if not probe_readiness.get("ready_for_m67_04"):
        issues.append(
            _issue(
                "semantic_compatibility_probe_not_ready",
                "blocker",
                "M67-03 semantic/compatibility probe must pass before recipe pipeline entry.",
    ...
    if probe_summary.get("candidate_edge_count", 0) <= 0:
        issues.append(
            _issue(
                "no_candidate_edges",
                "blocker",
                "Ninth slice needs at least one candidate edge before recipe work.",
    ...
    if readiness.get("runtime_or_bot_promotion_allowed") or probe_readiness.get("runtime_or_bot_promotion_allowed"):
        issues.append(
            _issue(
                ...
                {"m67_02": readiness, "m67_03": probe_readiness},
            )
        )
    ...
    return {
        "version": "M67-04",
        "description": "Ninth-slice recipe pipeline entry gate",
        "source_inputs": {
        ...
        "evidence_summary": {
            ...
            "semantic_probe_passed": bool(probe_readiness.get("ninth_slice_semantic_compatibility_probe_passed")),
        ...
        "decision": {
            ...
            "recommended_next_queue": "M68 Ninth-slice offline recipe pipeline"
            if offline_recipe_pipeline_allowed
            else "Hold ninth-slice recipe pipeline",
            "reason": (
                "Ninth slice has source-backed readiness plus a passed semantic/compatibility probe; offline recipe work may start with a new fixture scaffold."
                if offline_recipe_pipeline_allowed
                else "Ninth-slice recipe work is blocked until readiness/probe issues are repaired."
            ),
        },
        "proposed_m68_queue": [
            {
                "milestone": "M68-01",
                "task": "Ninth-slice fixture scaffold",
                "scope": "Define source-backed fixture policy for the Aquaforce G-series slice before validator work.",
            },
            {
                "milestone": "M68-02",
                "task": "Ninth-slice review packet",
                "scope": "Export candidate edges, manual-review cards, G Zone boundary notes, and format notes for human review.",
            },
            {
                "milestone": "M68-03",
            ...
            {
                "milestone": "M68-04",
                "task": "Ninth-slice recipe validator",
                "scope": "Validate count, trigger, grade, identity, copy limits, missing cards, G Zone boundary, and fixture scaffold constraints.",
            },
            {
                "milestone": "M68-05",
        ...
        "summary": {
            ...
            "offline_recipe_pipeline_allowed": offline_recipe_pipeline_allowed,
            "ready_for_m68": offline_recipe_pipeline_allowed,
        },
        "next_target": {
            "milestone": "M68-01" if offline_recipe_pipeline_allowed else "M67-repair-semantic",
            "task": "Ninth-slice fixture scaffold"
            if offline_recipe_pipeline_allowed
            else "Repair ninth-slice recipe pipeline entry blockers",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        "# M67-04 Ninth-Slice Recipe Pipeline Entry Gate",
        "",
        "## Selected Target",
        "",
    ...
    for item in report["proposed_m68_queue"]:
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M67-04 ninth-slice recipe pipeline entry gate.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_ninth_slice_recipe_pipeline_entry_gate()
    json_path = args.output_dir / JSON_OUTPUT.name
    md_path = args.output_dir / MD_OUTPUT.name
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M67-04 ninth-slice recipe gate wrote {json_path}")
    print(f"M67-04 ninth-slice recipe gate summary wrote {md_path}")
    print(
        "ready={ready} blockers={blockers} next={next_milestone}".format(
            ready=report["summary"]["ready_for_m68"],
            blockers=report["summary"]["blocking_issue_count"],
            next_milestone=report["next_target"]["milestone"],
        )
    )
    return 0 if report["summary"]["ready_for_m68"] else 1
