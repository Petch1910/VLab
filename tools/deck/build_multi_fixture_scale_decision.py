"""Build the M42-04 multi-fixture scale decision artifact."""
...
def build_multi_fixture_scale_decision(
    ...
    first_fixture = _load_fixture_from_smoke(first_smoke)
    second_fixture = _load_fixture_from_smoke(second_smoke)
    fixture_evidence = [
    ...
    completed_groups = _completed_groups([first_fixture, second_fixture])
    candidates = _candidate_queue(ranking, completed_groups)
    scale_allowed = not failed and len(fixture_evidence) == 2 and bool(candidates)

    return {
        "version": "M42-04",
        "description": "Multi-fixture scale decision",
        "source_inputs": {
        ...
        "decision": {
            "third_slice_offline_pipeline_allowed": scale_allowed,
            "third_slice_selected_now": False,
            "recommended_next_action": "M43-01 third target slice selection" if scale_allowed else "repair_failed_fixture_evidence",
            "live_runtime_deck_enabled": False,
            "saved_deck_enabled": False,
            "ui_deck_list_enabled": False,
            "bot_playbook_enabled": False,
            "decision_notes": [
                "Two independent fixtures now pass offline and Unity headless load smoke.",
                "The next step may select a third slice for offline analysis only.",
        ...
        "summary": {
            ...
            "third_slice_offline_pipeline_allowed": scale_allowed,
            "ready_for_m43": scale_allowed,
        },
        "next_target": {
            "milestone": "M43-01" if scale_allowed else "M42-repair",
            "task": "Third target slice selection" if scale_allowed else "Repair failed fixture evidence",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Third-slice offline pipeline allowed: `{summary['third_slice_offline_pipeline_allowed']}`",
        f"- Ready for M43: `{summary['ready_for_m43']}`",
        "",
        "## Fixture Evidence",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "## Decision",
            "",
            f"- Third slice selected now: `{decision['third_slice_selected_now']}`",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M42-04 multi-fixture scale decision.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_multi_fixture_scale_decision()
    json_path = args.output_dir / "m42_04_multi_fixture_scale_decision.json"
    md_path = args.output_dir / "m42_04_multi_fixture_scale_decision.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M42-04 multi-fixture scale decision wrote {json_path}")
    print(f"M42-04 multi-fixture scale decision summary wrote {md_path}")
    print(
        "ready_for_m43={ready} fixtures_passed={passed} candidates={candidates}".format(
            ready=report["summary"]["ready_for_m43"],
            passed=report["summary"]["passed_fixture_count"],
            candidates=report["summary"]["candidate_count"],
        )
    )
    return 0 if report["summary"]["ready_for_m43"] else 1
