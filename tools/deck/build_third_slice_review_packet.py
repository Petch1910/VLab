"""Build the M44-02 third-slice review packet."""
...
def _scaffold_note_item(scaffold_report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "item_id": "m44_01_fixture_scaffold",
        ...
        "mechanic_manual_review_required": bool(mechanic.get("legion_or_lock_text_requires_manual_review")),
        "review_reasons": [
        ...
        "blocked_until": [
            ...
            "m44_04_recipe_validator_uses_scaffold",
        ],
...
def _candidate_edge_item(edge: dict[str, Any], rank: int) -> dict[str, Any]:
    return {
        ...
        "review_reasons": [
            "third_slice_candidate_compatibility_edge",
            "requires_human_recipe_selection_before_m44_03",
        ],
        "blocked_until": [
        ...
        "recommended_next_action": "consider_for_m44_03_advisory_recipe_draft",
    }
...
def build_third_slice_review_packet(
    ...
    probe_report = probe_report or load_json(M43_03_PROBE)
    ...
    gate_allows_offline = bool(gate_report.get("summary", {}).get("ready_for_m44"))
    scaffold_ready = bool(scaffold_report.get("summary", {}).get("ready_for_m44_02"))
    ...
    return {
        "version": "M44-02",
        "description": "Third-slice review packet before advisory deck recipe drafting",
        "selected_target": selection_report.get("selected_target", {}),
        "source_inputs": {
            "third_target_slice_selection": _rel(M43_01_SELECTION),
            "third_slice_fixture_readiness": _rel(M43_02_READINESS),
            "third_slice_semantic_compatibility_probe": _rel(M43_03_PROBE),
            "third_slice_recipe_pipeline_entry_gate": _rel(M43_04_GATE),
            "third_slice_fixture_scaffold": _rel(M44_01_SCAFFOLD),
        },
        "scope": {
        ...
        "evidence_summary": {
            "m43_04_offline_recipe_pipeline_allowed": gate_allows_offline,
            "m43_04_runtime_promotion_closed": promotion_closed,
            "m44_01_scaffold_ready": scaffold_ready,
        ...
        "next_target": {
            ...
            "blocked_until": [
                "M44-02 review packet is available",
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M44-03: `{summary['ready_for_m44_03']}`",
        "",
        "## Fixture Scaffold",
        "",
    ]
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M44-02 third-slice review packet.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    json_path = args.output_dir / "m44_02_third_slice_review_packet.json"
    md_path = args.output_dir / "m44_02_third_slice_review_packet.md"
    csv_path = args.output_dir / "m44_02_third_slice_review_packet.csv"
    write_json(report, json_path)
    write_markdown(report, md_path)
    write_csv(report, csv_path)
    print(f"M44-02 review packet wrote {json_path}")
    print(f"M44-02 review packet summary wrote {md_path}")
    print(f"M44-02 review packet CSV wrote {csv_path}")
    print(
        "ready={ready} scaffold={scaffold} manual={manual} candidates={candidates}".format(
            ready=report["summary"]["ready_for_m44_03"],
    ...
    return 0 if report["summary"]["ready_for_m44_03"] else 1
