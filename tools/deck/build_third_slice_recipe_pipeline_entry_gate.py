"""Build the M43-04 third-slice recipe pipeline entry gate artifact."""
...
def build_third_slice_recipe_pipeline_entry_gate(
    ...
    if not readiness.get("all_fixture_expectations_met"):
        issues.append(
            _issue(
                ...
                "blocker",
                "M43-02 fixture/format readiness must pass before recipe pipeline entry.",
                {"readiness": readiness},
            )
        )
    if not readiness.get("semantic_probe_ready"):
        issues.append(
            _issue(
                ...
                "blocker",
                "M43-02 did not mark the selected slice ready for semantic probe.",
                {"readiness": readiness},
            )
        )
    if not probe_readiness.get("ready_for_m43_04"):
        issues.append(
            _issue(
                "semantic_compatibility_probe_not_ready",
                "blocker",
                "M43-03 semantic/compatibility probe must pass before recipe pipeline entry.",
    ...
    if probe_summary.get("candidate_edge_count", 0) <= 0:
        issues.append(
            _issue(
                "no_candidate_edges",
                "blocker",
                "Third slice needs at least one candidate edge before recipe work.",
    ...
    if readiness.get("runtime_or_bot_promotion_allowed") or probe_readiness.get("runtime_or_bot_promotion_allowed"):
        issues.append(
            _issue(
                ...
                {"m43_02": readiness, "m43_03": probe_readiness},
            )
        )
    ...
    return {
        "version": "M43-04",
        "description": "Third-slice recipe pipeline entry gate",
        "source_inputs": {
        ...
        "evidence_summary": {
            ...
            "semantic_probe_passed": bool(probe_readiness.get("third_slice_semantic_compatibility_probe_passed")),
        ...
        "decision": {
            ...
            "recommended_next_queue": "M44 Third-slice offline recipe pipeline"
            if offline_recipe_pipeline_allowed
            else "Hold third-slice recipe pipeline",
            "reason": (
                "Third slice has source-backed readiness plus a passed semantic/compatibility probe; offline recipe work may start with a new fixture scaffold."
                if offline_recipe_pipeline_allowed
                else "Third-slice recipe work is blocked until readiness/probe issues are repaired."
            ),
        },
        "proposed_m44_queue": [
            {
                "milestone": "M44-01",
                "task": "Third-slice fixture scaffold",
                "scope": "Define source-backed fixture policy for the Link Joker/Legion Mate slice before validator work.",
            },
            {
                "milestone": "M44-02",
        ...
        "summary": {
            ...
            "offline_recipe_pipeline_allowed": offline_recipe_pipeline_allowed,
            "ready_for_m44": offline_recipe_pipeline_allowed,
        },
        "next_target": {
            "milestone": "M44-01" if offline_recipe_pipeline_allowed else "M43-repair",
            "task": "Third-slice fixture scaffold"
            if offline_recipe_pipeline_allowed
            else "Repair third-slice recipe pipeline entry blockers",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    for item in report["proposed_m44_queue"]:
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M43-04 third-slice recipe pipeline entry gate.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_third_slice_recipe_pipeline_entry_gate()
    json_path = args.output_dir / "m43_04_third_slice_recipe_pipeline_entry_gate.json"
    md_path = args.output_dir / "m43_04_third_slice_recipe_pipeline_entry_gate.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M43-04 third-slice recipe gate wrote {json_path}")
    print(f"M43-04 third-slice recipe gate summary wrote {md_path}")
    print(
        "ready={ready} blockers={blockers} next={next_milestone}".format(
            ready=report["summary"]["ready_for_m44"],
            blockers=report["summary"]["blocking_issue_count"],
            next_milestone=report["next_target"]["milestone"],
        )
    )
    return 0 if report["summary"]["ready_for_m44"] else 1
