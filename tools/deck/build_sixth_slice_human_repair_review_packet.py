"""Build the M57-01 sixth-slice human repair review packet."""
...
M56_CLOSEOUT = OUTPUT_DIR / "m56_closeout_sixth_slice_runtime_readiness.json"
M56_REPAIRS = OUTPUT_DIR / "m56_06_sixth_slice_blocker_repair_candidates.json"
M56_DRAFTS = OUTPUT_DIR / "m56_03_sixth_slice_recipe_draft_model.json"
...
def _decision_options() -> list[dict[str, str]]:
    return [
        {
            "option_id": "accept_recipe_repairs_and_keep_g_zone_deferred_for_validation_rerun",
            "label": "Accept recipe repairs and keep G Zone deferred for validation rerun",
            "effect": "Allows M57-02/M57-03 to record explicit recipe/repair choice, but runtime promotion remains blocked by G Zone decision.",
        },
        {
        ...
        {
            "option_id": "request_different_repair_or_g_zone_scope",
            "label": "Request different repair or G Zone scope",
            "effect": "Keeps the recipe advisory and asks for another repair candidate or explicit G Zone scope decision.",
        },
        {
...
def _review_item(repair_item: dict[str, Any], draft: dict[str, Any] | None) -> dict[str, Any]:
    ...
    return {
        "review_item_id": f"m57_01_{repair_item['recipe_id']}_repair_review",
        "item_type": "sixth_slice_human_repair_and_g_zone_review",
        ...
        "g_zone_can_be_repaired_in_m57_01": False,
        ...
        "recommended_reviewer_action": "select_one_recipe_review_manual_grade_repairs_and_record_g_zone_decision_before_m57_02",
    }


def build_sixth_slice_human_repair_review_packet(
    closeout: dict[str, Any] | None = None,
    repairs: dict[str, Any] | None = None,
    drafts: dict[str, Any] | None = None,
) -> dict[str, Any]:
    closeout = closeout or load_json(M56_CLOSEOUT)
    repairs = repairs or load_json(M56_REPAIRS)
    drafts = drafts or load_json(M56_DRAFTS)
    ...
    return {
        "version": "M57-01",
        "description": "Sixth-slice human repair review packet",
        "selected_target": repairs.get("selected_target", closeout.get("key_results", {}).get("selected_target", {})),
        "source_inputs": {
            "m56_closeout": str(M56_CLOSEOUT.relative_to(ROOT)),
            "m56_repair_candidates": str(M56_REPAIRS.relative_to(ROOT)),
            "m56_recipe_drafts": str(M56_DRAFTS.relative_to(ROOT)),
        },
        "scope": {
        ...
        "review_policy": {
            ...
            "runtime_promotion_allowed": False,
            "m57_02_must_record_explicit_selection": True,
            "m57_03_must_record_explicit_acceptance": True,
            "m57_04_must_record_g_zone_decision": True,
            "m57_05_must_rerun_validation": True,
        },
        "summary": {
            ...
            "ready_for_m57_02": ready_for_m57_02,
        },
        "review_items": review_items,
        "next_target": {
            "milestone": "M57-02",
            "task": "Sixth-slice human-selected recipe artifact",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M57-02: `{summary['ready_for_m57_02']}`",
        "",
        "## Review Items",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "## Policy",
            "",
            "- This packet is not human selection, acceptance, or G Zone decision.",
            "- Runtime promotion remains disabled.",
            "- M57-02 must record exactly one selected recipe.",
            "- M57-03 must record explicit acceptance or rejection.",
            "- M57-04 must record explicit G Zone/Stride decision.",
            "- M57-05 must rerun repaired validation before any fixture gate.",
            "",
            "## Next",
            "",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M57-01 sixth-slice human repair review packet.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_sixth_slice_human_repair_review_packet()
    json_path = args.output_dir / "m57_01_sixth_slice_human_repair_review_packet.json"
    md_path = args.output_dir / "m57_01_sixth_slice_human_repair_review_packet.md"
    csv_path = args.output_dir / "m57_01_sixth_slice_human_repair_review_packet.csv"
    write_json(report, json_path)
    write_markdown(report, md_path)
    write_csv(report, csv_path)
    print(f"M57-01 sixth-slice human repair review packet wrote {json_path}")
    print(f"M57-01 sixth-slice human repair review packet summary wrote {md_path}")
    print(f"M57-01 sixth-slice human repair review packet CSV wrote {csv_path}")
    print(
        "ready_for_m57_02={ready} review_items={items}".format(
            ready=report["summary"]["ready_for_m57_02"],
            items=report["summary"]["review_item_count"],
        )
    )
    return 0 if report["summary"]["ready_for_m57_02"] else 1
