"""Build the M56-01 sixth-slice fixture scaffold artifact."""
...
def build_sixth_slice_fixture_scaffold(
    ...
    if not gate_report.get("summary", {}).get("ready_for_m56"):
        issues.append(
            _issue(
                "entry_gate_not_ready",
                "blocker",
                "M55-04 must allow the offline M56 pipeline before scaffolding fixtures.",
    ...
    if not evidence.get("new_format_or_mechanic_fixtures_required"):
        issues.append(
            _issue(
                ...
                "warning",
                "M56-01 is intended for a sixth slice that requires a new fixture scaffold.",
                {"evidence": evidence},
            )
        )
    ...
    return {
        "version": "M56-01",
        "description": "Sixth-slice fixture scaffold",
        "source_inputs": {
            "sixth_slice_recipe_pipeline_entry_gate": str(M55_04_GATE.relative_to(ROOT)),
        ...
        "fixture_scaffold": {
            "policy_level": "sixth_slice_g_next_z_fixture_scaffold_not_full_official_legality",
            ...
            "trial_deck_scope": format_scope.get("trial_decks", []),
            "booster_scope": format_scope.get("booster_packs", []),
            "clan_booster_scope": format_scope.get("clan_boosters", []),
            "character_booster_scope": format_scope.get("character_boosters", []),
            ...
            "deferred_source_backed_special_limits": [
                ...
                "G NEXT/Z card-specific exceptions not represented in local source data",
            ],
        },
        "mechanic_scope": {
            ...
            "notes": [
                ...
                "G units, Stride, Generation Break, Ritual, and retire-heavy text stay manual-review until dedicated rules modules exist.",
            ],
        },
        "validator_contract_for_m56_04": {
        ...
        "summary": {
            ...
            "ready_for_m56_02": scaffold_ready,
        },
        "next_target": {
            "milestone": "M56-02" if scaffold_ready else "M56-repair",
            "task": "Sixth-slice review packet"
            if scaffold_ready
            else "Repair sixth-slice fixture scaffold blockers",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines.extend(
        [
            ...
            f"- Ready for M56-02: `{summary['ready_for_m56_02']}`",
            "",
            "## Boundary",
            "",
        ]
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M56-01 sixth-slice fixture scaffold.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_sixth_slice_fixture_scaffold()
    json_path = args.output_dir / "m56_01_sixth_slice_fixture_scaffold.json"
    md_path = args.output_dir / "m56_01_sixth_slice_fixture_scaffold.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M56-01 sixth-slice fixture scaffold wrote {json_path}")
    print(f"M56-01 sixth-slice fixture scaffold summary wrote {md_path}")
    print(
        "ready={ready} blockers={blockers} next={next_milestone}".format(
            ready=report["summary"]["ready_for_m56_02"],
            blockers=report["summary"]["blocking_issue_count"],
            next_milestone=report["next_target"]["milestone"],
        )
    )
    return 0 if report["summary"]["ready_for_m56_02"] else 1
