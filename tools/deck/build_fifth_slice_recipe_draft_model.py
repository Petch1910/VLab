"""Build M52-03 fifth-slice advisory recipe drafts."""
...
M52_REVIEW_PACKET = OUTPUT_DIR / "m52_02_fifth_slice_review_packet.json"
            ...
            skipped.append(
                {
                    ...
                    "reason": "trigger_or_missing_from_main_deck_pool",
        ...
        rows.append(
            {
                ...
                "quantity_source": "m52_03_fixture_scaffold_plus_candidate_edge_pair",
            }
        )
    return rows
...
def _build_recipe(
    ...
    return {
        ...
        "recipe_status": (
            ...
            if slot_summary["explicit_card_count"] == target_total
...
def build_fifth_slice_recipe_drafts(
    review_packet: dict[str, Any] | None = None,
    scaffold: dict[str, Any] | None = None,
    draft_limit: int = DEFAULT_DRAFT_LIMIT,
) -> dict[str, Any]:
    review_packet = review_packet or load_json(M52_REVIEW_PACKET)
    scaffold = scaffold or load_json(M52_SCAFFOLD)
    if not review_packet.get("summary", {}).get("ready_for_m52_03"):
        raise ValueError("M52-02 review packet is not ready for M52-03.")
    if not scaffold.get("summary", {}).get("ready_for_m52_02"):
        raise ValueError("M52-01 fixture scaffold is not ready for M52-03.")
    ...
    return {
        "version": "M52-03",
        "description": "Fifth-slice advisory recipe draft model from M52-02 candidate edges",
        "selected_target": review_packet.get("selected_target", {}),
        "source_inputs": {
            "fifth_slice_review_packet": str(M52_REVIEW_PACKET.relative_to(ROOT)),
        ...
        "draft_policy": {
            "pair_anchored_from_m52_02_candidate_edges": True,
            "fixture_scaffold_source": "m52_01_fifth_slice_fixture_scaffold",
            "draft_limit": draft_limit,
            "card_quantities_are_advisory": True,
            "copy_limit_from_sqlite_deck_limit": True,
            "human_selection_required_before_runtime": True,
            "trigger_pair_edges_excluded_to_preserve_trigger_profile": True,
        ...
        "summary": {
            ...
            "candidate_edges_skipped_for_trigger_or_missing": len(skipped_edges),
            ...
            "ready_for_m52_04": bool(recipes) and quantity_complete_count == len(recipes),
        },
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Candidate edges skipped for trigger/missing: `{summary['candidate_edges_skipped_for_trigger_or_missing']}`",
        ...
        f"- Ready for M52-04: `{summary['ready_for_m52_04']}`",
        "",
        "## Drafts",
        "",
    ]
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M52-03 fifth-slice recipe draft model.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_fifth_slice_recipe_drafts(draft_limit=args.draft_limit)
    json_path = args.output_dir / "m52_03_fifth_slice_recipe_draft_model.json"
    md_path = args.output_dir / "m52_03_fifth_slice_recipe_draft_model.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M52-03 recipe draft model wrote {json_path}")
    print(f"M52-03 recipe draft model summary wrote {md_path}")
    print(
        "ready={ready} drafts={drafts} complete={complete} skipped={skipped}".format(
            ready=report["summary"]["ready_for_m52_04"],
            drafts=report["summary"]["recipe_draft_count"],
            complete=report["summary"]["quantity_complete_recipe_count"],
            skipped=report["summary"]["candidate_edges_skipped_for_trigger_or_missing"],
        )
    )
    return 0 if report["summary"]["ready_for_m52_04"] else 1
