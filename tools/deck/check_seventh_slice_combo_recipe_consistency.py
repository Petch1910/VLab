"""Check M60-05 seventh-slice combo-to-recipe consistency."""
...
def build_seventh_slice_consistency_report(
    ...
    for recipe in recipe_report.get("recipe_drafts", []):
        ...
        bloom_token_deferred = _has_issue(validation, "bloom_token_support_deferred")
        checks.append(
            {
                "recipe_id": recipe_id,
                ...
                "bloom_token_support_deferred": bloom_token_deferred,
    ...
    return {
        "version": "M60-05",
        "description": "Seventh-slice candidate edge to recipe consistency check",
        ...
        "summary": {
            ...
            "bloom_token_deferred_check_count": sum(1 for item in checks if item["bloom_token_support_deferred"]),
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines.extend(
        [
            ...
            "- Bloom/token deferred remains visible as a review dependency.",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check M60-05 seventh-slice combo-to-recipe consistency.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_seventh_slice_consistency_report()
    json_path = args.output_dir / "m60_05_seventh_slice_combo_recipe_consistency_report.json"
    md_path = args.output_dir / "m60_05_seventh_slice_combo_recipe_consistency_report.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M60-05 seventh-slice combo recipe consistency wrote {json_path}")
    print(f"M60-05 seventh-slice combo recipe consistency summary wrote {md_path}")
    print(
        "ready_for_m60_06={ready} checked={checked} promotion_allowed={allowed}".format(
            ready=report["summary"]["ready_for_m60_06"],
            checked=report["summary"]["consistency_check_count"],
            allowed=report["summary"]["promotion_allowed_count"],
        )
    )
    return 0 if report["summary"]["ready_for_m60_06"] else 1
