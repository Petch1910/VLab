"""Build the M59-04 seventh-slice recipe pipeline entry gate artifact."""
...
JSON_OUTPUT = OUTPUT_DIR / "m59_04_seventh_slice_recipe_pipeline_entry_gate.json"
MD_OUTPUT = OUTPUT_DIR / "m59_04_seventh_slice_recipe_pipeline_entry_gate.md"
...
def build_seventh_slice_recipe_pipeline_entry_gate(
    ...
    if not readiness.get("all_fixture_expectations_met"):
        issues.append(
            _issue(
                ...
                "blocker",
                "M59-02 fixture/format readiness must pass before recipe pipeline entry.",
                {"readiness": readiness},
            )
        )
    if not readiness.get("semantic_probe_ready"):
        issues.append(
            _issue(
                ...
                "blocker",
                "M59-02 did not mark the selected slice ready for semantic probe.",
                {"readiness": readiness},
            )
        )
    if not probe_readiness.get("ready_for_m59_04"):
        issues.append(
            _issue(
                "semantic_compatibility_probe_not_ready",
                "blocker",
                "M59-03 semantic/compatibility probe must pass before recipe pipeline entry.",
    ...
    if probe_summary.get("candidate_edge_count", 0) <= 0:
        issues.append(
            _issue(
                "no_candidate_edges",
                "blocker",
                "Seventh slice needs at least one candidate edge before recipe work.",
    ...
    if readiness.get("runtime_or_bot_promotion_allowed") or probe_readiness.get("runtime_or_bot_promotion_allowed"):
        issues.append(
            _issue(
                ...
                {"m59_02": readiness, "m59_03": probe_readiness},
            )
        )
    ...
    return {
        "version": "M59-04",
        "description": "Seventh-slice recipe pipeline entry gate",
        "source_inputs": {
            "seventh_slice_fixture_readiness": str(M59_02_READINESS.relative_to(ROOT)),
            "seventh_slice_semantic_compatibility_probe": str(M59_03_PROBE.relative_to(ROOT)),
        },
        ...
        "evidence_summary": {
            ...
            "semantic_probe_passed": bool(probe_readiness.get("seventh_slice_semantic_compatibility_probe_passed")),
        ...
        "decision": {
            ...
            "recommended_next_queue": "M60 Seventh-slice offline recipe pipeline"
            if offline_recipe_pipeline_allowed
            else "Hold seventh-slice recipe pipeline",
            "reason": (
                "Seventh slice has source-backed readiness plus a passed semantic/compatibility probe; offline recipe work may start with a new fixture scaffold."
                if offline_recipe_pipeline_allowed
                else "Seventh-slice recipe work is blocked until readiness/probe issues are repaired."
            ),
        },
        "proposed_m60_queue": [
            {
                "milestone": "M60-01",
                "task": "Seventh-slice fixture scaffold",
                "scope": "Define source-backed fixture policy for the Neo Nectar G-series slice before validator work.",
            },
            {
                "milestone": "M60-02",
                "task": "Seventh-slice review packet",
            ...
            {
                "milestone": "M60-04",
                "task": "Seventh-slice recipe validator",
            ...
            {
                "milestone": "M60-06",
                "task": "Seventh-slice blocker repair candidates",
        ...
        "summary": {
            ...
            "offline_recipe_pipeline_allowed": offline_recipe_pipeline_allowed,
            "ready_for_m60": offline_recipe_pipeline_allowed,
        },
        "next_target": {
            "milestone": "M60-01" if offline_recipe_pipeline_allowed else "M59-repair-semantic",
            "task": "Seventh-slice fixture scaffold"
            if offline_recipe_pipeline_allowed
            else "Repair seventh-slice recipe pipeline entry blockers",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        "# M59-04 Seventh-Slice Recipe Pipeline Entry Gate",
        "",
        "## Selected Target",
        "",
    ...
    for item in report["proposed_m60_queue"]:
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M59-04 seventh-slice recipe pipeline entry gate.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_seventh_slice_recipe_pipeline_entry_gate()
    json_path = args.output_dir / JSON_OUTPUT.name
    md_path = args.output_dir / MD_OUTPUT.name
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M59-04 seventh-slice recipe gate wrote {json_path}")
    print(f"M59-04 seventh-slice recipe gate summary wrote {md_path}")
    print(
        "ready={ready} blockers={blockers} next={next_milestone}".format(
            ready=report["summary"]["ready_for_m60"],
            blockers=report["summary"]["blocking_issue_count"],
            next_milestone=report["next_target"]["milestone"],
        )
    )
    return 0 if report["summary"]["ready_for_m60"] else 1
