"""Build the M60-01 seventh-slice fixture scaffold artifact."""
...
JSON_OUTPUT = OUTPUT_DIR / "m60_01_seventh_slice_fixture_scaffold.json"
MD_OUTPUT = OUTPUT_DIR / "m60_01_seventh_slice_fixture_scaffold.md"
...
def build_seventh_slice_fixture_scaffold(
    ...
    if not gate_report.get("summary", {}).get("ready_for_m60"):
        issues.append(
            _issue(
                "entry_gate_not_ready",
                "blocker",
                "M59-04 must allow the offline M60 pipeline before scaffolding fixtures.",
    ...
    if not evidence.get("new_format_or_mechanic_fixtures_required"):
        issues.append(
            _issue(
                ...
                "warning",
                "M60-01 is intended for a seventh slice that requires a new fixture scaffold.",
                {"evidence": evidence},
            )
        )
    ...
    if missing_triggers:
        issues.append(
            _issue(
                ...
                "blocker",
                "The scaffold requires all four classic/G-era trigger types to be present.",
    ...
    return {
        "version": "M60-01",
        "description": "Seventh-slice fixture scaffold",
        "source_inputs": {
            "seventh_slice_recipe_pipeline_entry_gate": str(M59_04_GATE.relative_to(ROOT)),
        ...
        "fixture_scaffold": {
            "policy_level": "seventh_slice_g_series_first_fixture_scaffold_not_full_official_legality",
            ...
            "deferred_source_backed_special_limits": [
                ...
                "G-series card-specific exceptions not represented in local source data",
            ],
        },
        "mechanic_scope": {
            ...
            "bloom_or_token_like_text_requires_manual_review": True,
            ...
            "notes": [
                ...
                "G units, Stride, Generation Break, Bloom-like text, and token-like text stay manual-review until dedicated rules modules exist.",
            ],
        },
        "validator_contract_for_m60_04": {
        ...
        "summary": {
            ...
            "ready_for_m60_02": scaffold_ready,
        },
        "next_target": {
            "milestone": "M60-02" if scaffold_ready else "M60-repair",
            "task": "Seventh-slice review packet"
            if scaffold_ready
            else "Repair seventh-slice fixture scaffold blockers",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines.extend(
        [
            ...
            f"- Ready for M60-02: `{summary['ready_for_m60_02']}`",
            "",
            "## Boundary",
            "",
        ]
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M60-01 seventh-slice fixture scaffold.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_seventh_slice_fixture_scaffold()
    json_path = args.output_dir / JSON_OUTPUT.name
    md_path = args.output_dir / MD_OUTPUT.name
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M60-01 seventh-slice fixture scaffold wrote {json_path}")
    print(f"M60-01 seventh-slice fixture scaffold summary wrote {md_path}")
    print(
        "ready={ready} blockers={blockers} next={next_milestone}".format(
            ready=report["summary"]["ready_for_m60_02"],
            blockers=report["summary"]["blocking_issue_count"],
            next_milestone=report["next_target"]["milestone"],
        )
    )
    return 0 if report["summary"]["ready_for_m60_02"] else 1
