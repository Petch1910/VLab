"""Export the seventh offline runtime fixture as count-line deck text (M62-02)."""
...
DEFAULT_VALIDATION_REPORT = OUTPUT_DIR / "m62_01_seventh_fixture_schema_validation.json"
CARDS_SQLITE = ROOT / "data" / "packs" / "vanguard_th" / "cards.sqlite"
PACK_MANIFEST = ROOT / "data" / "packs" / "vanguard_th" / "manifest.json"

TEXT_OUTPUT = OUTPUT_DIR / "m62_02_seventh_fixture_deck_text_export.txt"
JSON_OUTPUT = OUTPUT_DIR / "m62_02_seventh_fixture_deck_text_export.json"
MD_OUTPUT = OUTPUT_DIR / "m62_02_seventh_fixture_deck_text_export.md"
...
def _is_validation_ready(validation_report: dict[str, Any]) -> bool:
    ...
    return bool(summary.get("schema_valid")) and bool(summary.get("ready_for_m62_02"))
...
def _build_deck_text(
    ...
    slice_name = str(format_policy.get("slice") or selected_target.get("slice") or "Seventh Offline Slice")
    group_name = str(format_policy.get("group") or selected_target.get("group") or "Neo Nectar")

    lines = [
        "# Vanguard Thai Sim Deck List",
        "# M62-02 review-only export from an offline runtime fixture.",
        f"# FixtureId: {fixture_id}",
        f"# RecipeId: {recipe_id}",
        f"# Slice: {slice_name}",
        f"# Group: {group_name}",
        f"# GZoneBoundary: {system_boundaries.get('selected_g_zone_option_id') or ''}",
        f"# BloomTokenBoundary: {system_boundaries.get('selected_bloom_token_option_id') or ''}",
        "# Scope: review_only_offline_fixture",
        "Name: G Series Neo Nectar Fixture (" + recipe_id + ")",
        "Format: " + era_preset,
    ...
    lines.extend(["", "[Ride]", "", "[G]"])
    if system_boundaries.get("g_zone_runtime_enabled") is False and system_boundaries.get("stride_runtime_enabled") is False:
        lines.append("# Empty: G Zone and Stride runtime are disabled for this fixture.")
    if system_boundaries.get("bloom_token_runtime_enabled") is False:
        lines.append("# Note: Bloom/token runtime is disabled; Bloom/token text remains review-only.")
    lines.append("")
    return "\n".join(lines).rstrip() + "\n"


def build_seventh_fixture_deck_text_export(
    ...
    if not _is_validation_ready(validation_report):
        issues.append(
            _issue(
                "m62_01_validation_not_ready",
                "blocker",
                "M62-01 validation report must be schema-valid and ready before deck text export.",
                {"summary": validation_report.get("summary", {})},
            )
        )
    ...
    return {
        "version": "M62-02",
        "description": "Seventh fixture deck text exporter",
        "source_inputs": {
            "runtime_fixture": str(DEFAULT_FIXTURE.relative_to(ROOT)),
            "m62_01_validation_report": str(DEFAULT_VALIDATION_REPORT.relative_to(ROOT)),
        ...
        "scope": {
            ...
            "seventh_fixture_deck_text_export": True,
        ...
        "deck_text_policy": {
            ...
            "g_section_is_comment_only": True,
            "bloom_token_notice_is_comment_only": True,
        },
        "fixture_summary": {
        ...
        "summary": {
            ...
            "ready_for_m62_03": export_ready,
        },
        "next_target": {
            "milestone": "M62-03",
            "task": "Seventh fixture headless load smoke",
        },
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        "# M62-02 Seventh Fixture Deck Text Export",
        "",
        "## Summary",
        "",
        ...
        f"- Ready for M62-03: `{summary['ready_for_m62_03']}`",
        "",
        "## Runtime Boundary",
        "",
        "- Review text only.",
        ...
        "- Does not enable Bloom/token runtime.",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export M62-02 seventh fixture count-line deck text.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_seventh_fixture_deck_text_export(fixture, validation_report)
    report["source_inputs"]["runtime_fixture"] = str(args.fixture)
    report["source_inputs"]["m62_01_validation_report"] = str(args.validation_report)
    ...
    print(f"M62-02 seventh fixture deck text export wrote {json_path}")
    print(f"M62-02 seventh fixture deck text summary wrote {md_path}")
    if report["summary"]["export_ready"]:
        print(f"M62-02 seventh fixture deck text wrote {text_path}")
    print(
        "export_ready={ready} blockers={blockers} card_lines={lines} next={next_ready}".format(
            ...
            next_ready=report["summary"]["ready_for_m62_03"],
        )
    )
