"""Build M44-06 third-slice blocker repair candidates.

This creates advisory, source-backed repair candidates for blockers reported by
M44-04 and carried through M44-05. It does not mutate M44-03 drafts and does
...
M44_RECIPE_DRAFTS = OUTPUT_DIR / "m44_03_third_slice_recipe_draft_model.json"
M44_VALIDATION = OUTPUT_DIR / "m44_04_third_slice_recipe_validation_report.json"
M44_CONSISTENCY = OUTPUT_DIR / "m44_05_third_slice_combo_recipe_consistency_report.json"
M44_SCAFFOLD = OUTPUT_DIR / "m44_01_third_slice_fixture_scaffold.json"
    ...
    rows = [
        {
            ...
            "source": "m44_03_third_slice_recipe_draft_model",
        }
        for card_id, quantity in quantities.items()
...
def build_third_slice_blocker_repair_candidates(
    recipe_report: dict[str, Any] | None = None,
    validation_report: dict[str, Any] | None = None,
    consistency_report: dict[str, Any] | None = None,
    scaffold: dict[str, Any] | None = None,
) -> dict[str, Any]:
    recipe_report = recipe_report or load_json(M44_RECIPE_DRAFTS)
    validation_report = validation_report or load_json(M44_VALIDATION)
    consistency_report = consistency_report or load_json(M44_CONSISTENCY)
    ...
    for recipe in recipe_report.get("recipe_drafts", []):
        ...
        manual_package = (
            _build_manual_package(recipe_id, manual_ids, quantities, cards, series_scope, series_order)
            if manual_ids
            else None
        )
    ...
    return {
        "version": "M44-06",
        "description": "Third-slice blocker repair candidates",
        "selected_target": recipe_report.get("selected_target", {}),
        "source_inputs": {
            "third_slice_recipe_draft_model": str(M44_RECIPE_DRAFTS.relative_to(ROOT)),
            "third_slice_recipe_validation_report": str(M44_VALIDATION.relative_to(ROOT)),
            "third_slice_combo_recipe_consistency_report": str(M44_CONSISTENCY.relative_to(ROOT)),
        ...
        "repair_policy": {
            ...
            "series_scope_from_m44_01_fixture_scaffold": series_scope,
        ...
        "summary": {
            ...
            "runtime_promotion_allowed": False,
            "ready_for_m44_closeout": bool(repair_items),
        },
        "next_target": {
            "milestone": "M44-closeout",
            "task": "Third-slice runtime readiness decision",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M44-closeout: `{summary['ready_for_m44_closeout']}`",
        "",
        "## Repair Items",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "- M44-03 draft files are not modified.",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M44-06 third-slice blocker repair candidates.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_third_slice_blocker_repair_candidates()
    json_path = args.output_dir / "m44_06_third_slice_blocker_repair_candidates.json"
    md_path = args.output_dir / "m44_06_third_slice_blocker_repair_candidates.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M44-06 third-slice blocker repair candidates wrote {json_path}")
    print(f"M44-06 third-slice blocker repair candidates summary wrote {md_path}")
    print(
        "ready_for_m44_closeout={ready} recipes={recipes} human_review_ready={ready_count}".format(
            ready=report["summary"]["ready_for_m44_closeout"],
            recipes=report["summary"]["recipe_count"],
            ready_count=report["summary"]["ready_for_human_repair_review_count"],
        )
    )
    return 0 if report["summary"]["ready_for_m44_closeout"] else 1
