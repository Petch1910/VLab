"""Build the M61-02 seventh-slice human-selected recipe artifact.

This tool intentionally has no default selected review item. A caller must pass
an explicit M61-01 review item id and non-empty selection text.
"""
...
M61_01_REVIEW = OUTPUT_DIR / "m61_01_seventh_slice_human_repair_review_packet.json"
...
def _find_review_item(review_packet: dict[str, Any], review_item_id: str) -> dict[str, Any]:
    if not review_item_id.strip():
        raise ValueError("M61-02 requires an explicit non-empty review_item_id.")
    matches = [
        item
    ...
    if not matches:
        raise ValueError(f"M61-01 review item not found: {review_item_id}")
    if len(matches) > 1:
        raise ValueError(f"M61-01 review item id is not unique: {review_item_id}")
    return matches[0]
...
def _require_selection_text(selection_text: str) -> str:
    ...
    if not normalized:
        raise ValueError("M61-02 requires non-empty selection_text.")
    return normalized
...
def build_seventh_slice_human_selected_recipe_artifact(
    ...
    *,
    ...
    review_packet = review_packet or load_json(M61_01_REVIEW)
    ...
    ready_for_m61_03 = (
        bool(review_packet.get("summary", {}).get("ready_for_m61_02"))
    ...
    return {
        "version": "M61-02",
        "description": "Seventh-slice human-selected recipe artifact",
        "selected_target": review_packet.get("selected_target", {}),
        "source_inputs": {
            "m61_01_human_repair_review_packet": str(M61_01_REVIEW.relative_to(ROOT)),
        },
        "scope": {
        ...
        "selection_policy": {
            ...
            "selection_is_not_bloom_token_decision": True,
        ...
        "selection": {
            ...
            "selected_action": "select_recipe_for_m61_03_acceptance_review",
            ...
            "g_zone_can_be_repaired_in_m61_01": bool(selected_item.get("g_zone_can_be_repaired_in_m61_01")),
            "g_zone_future_system_work": selected_item.get("g_zone_future_system_work", []),
            "bloom_token_package_id": selected_item.get("bloom_token_package_id", ""),
            "bloom_token_deferred": bool(selected_item.get("bloom_token_deferred")),
            "bloom_token_can_be_repaired_in_m61_01": bool(
                selected_item.get("bloom_token_can_be_repaired_in_m61_01")
            ),
            "bloom_token_future_system_work": selected_item.get("bloom_token_future_system_work", []),
            "decision_options": selected_item.get("decision_options", []),
            "g_zone_decision_options": selected_item.get("g_zone_decision_options", []),
            "bloom_token_decision_options": selected_item.get("bloom_token_decision_options", []),
        ...
        "summary": {
            ...
            "selected_bloom_token_package_id": selected_item.get("bloom_token_package_id", ""),
            "records_human_selection": True,
            "records_human_acceptance": False,
            "records_g_zone_decision": False,
            "records_bloom_token_decision": False,
            ...
            "g_zone_deferred": bool(selected_item.get("g_zone_deferred")),
            "bloom_token_deferred": bool(selected_item.get("bloom_token_deferred")),
            "ready_for_m61_03": ready_for_m61_03,
        },
        "next_target": {
            "milestone": "M61-03",
            "task": "Seventh-slice human-accepted repair artifact",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Selected Bloom/token package: `{summary['selected_bloom_token_package_id']}`",
        ...
        f"- Records Bloom/token decision: `{summary['records_bloom_token_decision']}`",
        ...
        f"- Ready for M61-03: `{summary['ready_for_m61_03']}`",
        "",
        "## Selection",
        "",
        ...
        f"- Bloom/token deferred package: `{selection['bloom_token_package_id']}`",
        "",
        "## Policy",
        "",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M61-02 seventh-slice human-selected recipe artifact.")
    parser.add_argument("--review-item-id", required=True, help="Explicit M61-01 review_item_id selected by the user.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    json_path = args.output_dir / "m61_02_seventh_slice_human_selected_recipe_artifact.json"
    md_path = args.output_dir / "m61_02_seventh_slice_human_selected_recipe_artifact.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M61-02 seventh-slice human-selected recipe artifact wrote {json_path}")
    print(f"M61-02 seventh-slice human-selected recipe artifact summary wrote {md_path}")
    print(
        "ready_for_m61_03={ready} selected_recipe={recipe}".format(
            ready=report["summary"]["ready_for_m61_03"],
            recipe=report["summary"]["selected_recipe_id"],
        )
    )
    return 0 if report["summary"]["ready_for_m61_03"] else 1
