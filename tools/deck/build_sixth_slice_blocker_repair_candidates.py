"""Build M56-06 sixth-slice blocker repair candidates.

This creates advisory repair candidates for blockers reported by M56-04 and
carried through M56-05. It does not mutate M56-03 drafts and does not create
...
M56_RECIPE_DRAFTS = OUTPUT_DIR / "m56_03_sixth_slice_recipe_draft_model.json"
M56_VALIDATION = OUTPUT_DIR / "m56_04_sixth_slice_recipe_validation_report.json"
M56_CONSISTENCY = OUTPUT_DIR / "m56_05_sixth_slice_combo_recipe_consistency_report.json"
M56_SCAFFOLD = OUTPUT_DIR / "m56_01_sixth_slice_fixture_scaffold.json"
...
def _pick_grade_additions(
    ...
    remaining = needed
    pool = _sort_candidates(
        _load_candidate_pool(clan, int(grade), "", "Normal Unit", manual_ids, series_scope),
        series_order,
    )
    for card in pool:
        if remaining <= 0:
    ...
    rows = [
        {
            ...
            "source": "m56_03_sixth_slice_recipe_draft_model",
        }
        for card_id, quantity in quantities.items()
...
def _build_grade_package(
    ...
    for grade, values in issue.items():
        ...
        if actual < expected:
            ...
            additions.extend(
                _pick_grade_additions(clan, grade, expected - actual, quantities, manual_ids, series_scope, series_order)
            )
...
def _build_g_zone_package(recipe_id: str, validation: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        ...
        "advisory_only": True,
        "can_be_repaired_in_m56_06": False,
...
def build_sixth_slice_blocker_repair_candidates(
    recipe_report: dict[str, Any] | None = None,
    validation_report: dict[str, Any] | None = None,
    consistency_report: dict[str, Any] | None = None,
    scaffold: dict[str, Any] | None = None,
) -> dict[str, Any]:
    recipe_report = recipe_report or load_json(M56_RECIPE_DRAFTS)
    validation_report = validation_report or load_json(M56_VALIDATION)
    consistency_report = consistency_report or load_json(M56_CONSISTENCY)
    scaffold = scaffold or load_json(M56_SCAFFOLD)
    ...
    return {
        "version": "M56-06",
        "description": "Sixth-slice blocker repair candidates",
        "selected_target": recipe_report.get("selected_target", {}),
        "source_inputs": {
            "sixth_slice_recipe_draft_model": str(M56_RECIPE_DRAFTS.relative_to(ROOT)),
            "sixth_slice_recipe_validation_report": str(M56_VALIDATION.relative_to(ROOT)),
            "sixth_slice_combo_recipe_consistency_report": str(M56_CONSISTENCY.relative_to(ROOT)),
            "sixth_slice_fixture_scaffold": str(M56_SCAFFOLD.relative_to(ROOT)),
        ...
        "repair_policy": {
            ...
            "series_scope_from_m56_01_fixture_scaffold": series_scope,
        ...
        "summary": {
            ...
            "runtime_promotion_allowed": False,
            "ready_for_m56_closeout": bool(repair_items) and unexpected_structural_blocker_count == 0,
        },
        "next_target": {
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M56-closeout: `{summary['ready_for_m56_closeout']}`",
        "",
        "## Repair Items",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "- M56-03 draft files are not modified.",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M56-06 sixth-slice blocker repair candidates.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_sixth_slice_blocker_repair_candidates()
    json_path = args.output_dir / "m56_06_sixth_slice_blocker_repair_candidates.json"
    md_path = args.output_dir / "m56_06_sixth_slice_blocker_repair_candidates.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M56-06 sixth-slice blocker repair candidates wrote {json_path}")
    print(f"M56-06 sixth-slice blocker repair candidates summary wrote {md_path}")
    print(
        "ready_for_m56_closeout={ready} recipes={recipes} human_review_ready={ready_count} g_zone_deferred={g_zone}".format(
            ready=report["summary"]["ready_for_m56_closeout"],
    ...
    return 0 if report["summary"]["ready_for_m56_closeout"] else 1
