"""Build the M63-01 eighth target slice selection scaffold artifact."""
...
M62_04_DECISION = OUTPUT_DIR / "m62_04_seven_fixture_scale_decision.json"
JSON_OUTPUT = OUTPUT_DIR / "m63_01_eighth_target_slice_selection.json"
MD_OUTPUT = OUTPUT_DIR / "m63_01_eighth_target_slice_selection.md"
...
def build_eighth_target_slice_selection(m62_decision: dict[str, Any] | None = None) -> dict[str, Any]:
    m62_decision = m62_decision or load_json(M62_04_DECISION)
    summary = m62_decision.get("summary", {})
    decision = m62_decision.get("decision", {})
    candidates = list(m62_decision.get("candidate_queue", []))
    selection_allowed = (
        bool(summary.get("ready_for_m63"))
        and bool(summary.get("eighth_slice_offline_pipeline_allowed"))
        and bool(decision.get("eighth_slice_offline_pipeline_allowed"))
        and bool(candidates)
    )
    ...
    return {
        "version": "M63-01",
        "description": "Eighth target slice selection",
        "source_inputs": {
            "seven_fixture_scale_decision": str(M62_04_DECISION.relative_to(ROOT)),
        },
        "scope": {
        ...
        "selection_policy": {
            "requires_m62_scale_allowed": True,
            "requires_ready_for_m63": True,
        ...
        "decision": {
            ...
            "eighth_slice_selected": bool(selected_target),
        ...
        "summary": {
            ...
            "ready_for_m63_02": bool(selected_target),
        },
        "next_target": {
            "milestone": "M63-02" if selected_target else "M62-repair",
            "task": "Eighth-slice fixture/format readiness" if selected_target else "Repair seven-fixture scale decision",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        "# M63-01 Eighth Target Slice Selection",
        "",
        "## Summary",
        "",
        f"- Eighth slice selected: `{summary['eighth_slice_selected']}`",
        ...
        f"- Ready for M63-02: `{summary['ready_for_m63_02']}`",
        "",
        "## Selected Target",
        "",
    ]
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M63-01 eighth target slice selection.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_eighth_target_slice_selection()
    json_path = args.output_dir / JSON_OUTPUT.name
    md_path = args.output_dir / MD_OUTPUT.name
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M63-01 eighth target slice selection wrote {json_path}")
    print(f"M63-01 eighth target slice selection summary wrote {md_path}")
    print(
        "selected={selected} group={group} next={next_ready}".format(
            selected=report["summary"]["eighth_slice_selected"],
            group=report["summary"]["selected_group"],
            next_ready=report["summary"]["ready_for_m63_02"],
        )
    )
    return 0 if report["summary"]["ready_for_m63_02"] else 1
