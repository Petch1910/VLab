"""Validate M64-03 eighth-slice recipe drafts for M64-04."""
...
RECIPE_DRAFTS = OUTPUT_DIR / "m64_03_eighth_slice_recipe_draft_model.json"
    ...
    if grade_counts.get("4", 0) > 0:
        issues.append(
            _issue(
                ...
                "blocker",
                "Grade 4 cards must not enter main deck drafts until dedicated format support exists.",
    ...
    if manual_ids:
        issues.append(
            _issue(
                ...
                "blocker",
                "Recipe contains cards that M64-02 marked as manual-review required.",
                {"card_ids": sorted(manual_ids)},
            )
        )

    metadata = recipe.get("validation_metadata", {})
    if metadata.get("lock_runtime_support_deferred"):
        issues.append(
            _issue(
                "lock_runtime_support_deferred",
                "review",
                "Lock/Unlock runtime support is deferred; this draft can only validate main-deck quantities.",
                {},
            )
        )
    if metadata.get("legion_runtime_support_deferred"):
        issues.append(
            _issue(
                "legion_runtime_support_deferred",
                "review",
                "Legion/Mate runtime support is deferred; this draft can only validate main-deck quantities.",
                {},
            )
        )
...
def build_eighth_slice_validation_report(
    ...
    return {
        "version": "M64-04",
        "description": "Eighth-slice recipe validator for M64-03 advisory drafts",
        "selected_target": recipe_report.get("selected_target", {}),
        "source_inputs": {
            "eighth_slice_recipe_draft_model": str(RECIPE_DRAFTS.relative_to(ROOT)),
        ...
        "validation_policy": {
            ...
            "heal_trigger_max": 4,
            "grade4_main_deck_max_until_format_support": 0,
            ...
            "lock_runtime_deferred_is_review_not_blocker": True,
            "legion_runtime_deferred_is_review_not_blocker": True,
        },
        "summary": {
            ...
            "lock_deferred_recipe_count": sum(
                1 for item in validations if any(issue["code"] == "lock_runtime_support_deferred" for issue in item["issues"])
            ),
            "legion_deferred_recipe_count": sum(
                1 for item in validations if any(issue["code"] == "legion_runtime_support_deferred" for issue in item["issues"])
            ),
            "issue_counts": dict(sorted(issue_counts.items())),
            "ready_for_m64_05": bool(validations),
        },
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        "# M64-04 Eighth-Slice Recipe Validation Report",
        "",
        "## Summary",
        "",
        ...
        f"- Ready for M64-05: `{summary['ready_for_m64_05']}`",
        "",
        "## Recipe Status",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "- Grade 4 main-deck count must stay 0 until dedicated format support exists.",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate M64-03 eighth-slice recipe drafts.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_eighth_slice_validation_report()
    json_path = args.output_dir / "m64_04_eighth_slice_recipe_validation_report.json"
    md_path = args.output_dir / "m64_04_eighth_slice_recipe_validation_report.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M64-04 eighth-slice recipe validation wrote {json_path}")
    print(f"M64-04 eighth-slice recipe validation summary wrote {md_path}")
    print(
        "ready_for_m64_05={ready} runtime_ready={runtime_ready} pending={pending} grade4={grade4}".format(
            ready=report["summary"]["ready_for_m64_05"],
            runtime_ready=report["summary"]["runtime_ready_recipe_count"],
            pending=report["summary"]["validator_passed_pending_human_selection_count"],
            grade4=report["summary"]["grade4_main_deck_recipe_count"],
        )
    )
    return 0 if report["summary"]["ready_for_m64_05"] else 1
