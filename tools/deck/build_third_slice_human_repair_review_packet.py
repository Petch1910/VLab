"""Build the M45-01 third-slice human repair review packet."""
...
M44_CLOSEOUT = OUTPUT_DIR / "m44_closeout_third_slice_runtime_readiness.json"
M44_REPAIRS = OUTPUT_DIR / "m44_06_third_slice_blocker_repair_candidates.json"
...
def _decision_options() -> list[dict[str, str]]:
    return [
        {
            "option_id": "accept_combined_manual_and_grade_repair_for_validation_rerun",
            "label": "Accept combined manual and grade repair for validation rerun",
            "effect": "Allows M45-02 to record explicit acceptance and M45-03 to rerun validation; does not promote runtime.",
        },
        {
...
def _review_item(repair_item: dict[str, Any], draft: dict[str, Any] | None) -> dict[str, Any]:
    ...
    return {
        "review_item_id": f"m45_01_{repair_item['recipe_id']}_repair_review",
        "item_type": "third_slice_repair_review",
        ...
        "recommended_reviewer_action": "review_combined_manual_and_grade_packages_before_m45_02_acceptance",
    }


def build_third_slice_human_repair_review_packet(
    closeout: dict[str, Any] | None = None,
    repairs: dict[str, Any] | None = None,
    drafts: dict[str, Any] | None = None,
) -> dict[str, Any]:
    closeout = closeout or load_json(M44_CLOSEOUT)
    repairs = repairs or load_json(M44_REPAIRS)
    ...
    ready_for_m45_02 = human_review_allowed and len(ready_items) == len(review_items) and len(review_items) > 0
    return {
        "version": "M45-01",
        "description": "Third-slice human repair review packet",
        "selected_target": repairs.get("selected_target", closeout.get("key_results", {}).get("selected_target", {})),
        "source_inputs": {
            "m44_closeout": str(M44_CLOSEOUT.relative_to(ROOT)),
            "m44_repair_candidates": str(M44_REPAIRS.relative_to(ROOT)),
        ...
        "review_policy": {
            ...
            "runtime_promotion_allowed": False,
            "m45_02_must_record_explicit_acceptance": True,
            "m45_03_must_rerun_validation": True,
        },
        "summary": {
            ...
            "ready_for_m45_02": ready_for_m45_02,
        },
        "review_items": review_items,
        "next_target": {
            "milestone": "M45-02",
            "task": "Third-slice human-accepted repair artifact",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M45-02: `{summary['ready_for_m45_02']}`",
        "",
        "## Review Items",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "- M45-02 must record explicit acceptance or rejection.",
            "- M45-03 must rerun repaired validation before any fixture gate.",
            "",
            "## Next",
            "",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M45-01 third-slice human repair review packet.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    json_path = args.output_dir / "m45_01_third_slice_human_repair_review_packet.json"
    md_path = args.output_dir / "m45_01_third_slice_human_repair_review_packet.md"
    csv_path = args.output_dir / "m45_01_third_slice_human_repair_review_packet.csv"
    write_json(report, json_path)
    write_markdown(report, md_path)
    write_csv(report, csv_path)
    print(f"M45-01 third-slice human repair review packet wrote {json_path}")
    print(f"M45-01 third-slice human repair review packet summary wrote {md_path}")
    print(f"M45-01 third-slice human repair review packet CSV wrote {csv_path}")
    print(
        "ready_for_m45_02={ready} review_items={items}".format(
            ready=report["summary"]["ready_for_m45_02"],
            items=report["summary"]["review_item_count"],
        )
    )
    return 0 if report["summary"]["ready_for_m45_02"] else 1
