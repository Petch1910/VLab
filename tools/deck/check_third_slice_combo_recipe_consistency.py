"""Check M44-05 third-slice combo-to-recipe consistency.

For the third slice, the combo unit is the M44-02 candidate edge used by each
M44-03 recipe draft. This checker verifies that the candidate source/target
cards are present in the recipe and carries forward M44-04 validator blockers.
...
def build_third_slice_consistency_report(
    ...
    return {
        "version": "M44-05",
        "description": "Third-slice candidate edge to recipe consistency check",
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M44-06: `{summary['ready_for_m44_06']}`",
        "",
        "## Status Counts",
        "",
    ]
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check M44-05 third-slice combo-to-recipe consistency.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_third_slice_consistency_report()
    json_path = args.output_dir / "m44_05_third_slice_combo_recipe_consistency_report.json"
    md_path = args.output_dir / "m44_05_third_slice_combo_recipe_consistency_report.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M44-05 third-slice combo recipe consistency wrote {json_path}")
    print(f"M44-05 third-slice combo recipe consistency summary wrote {md_path}")
    print(
        "ready_for_m44_06={ready} checked={checked} promotion_allowed={allowed}".format(
            ready=report["summary"]["ready_for_m44_06"],
            checked=report["summary"]["consistency_check_count"],
            allowed=report["summary"]["promotion_allowed_count"],
        )
    )
    return 0 if report["summary"]["ready_for_m44_06"] else 1
