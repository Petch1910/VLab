"""Run the eighth-slice semantic/compatibility probe for M63-03.

This reuses the generalized M35 B/C probe in memory after normalizing the
M63 selection/readiness reports. The probe remains advisory and offline.
"""
...
JSON_OUTPUT = OUTPUT_DIR / "m63_03_eighth_slice_semantic_compatibility_probe.json"
MD_OUTPUT = OUTPUT_DIR / "m63_03_eighth_slice_semantic_compatibility_probe.md"
...
def _normalize_readiness(readiness_report: dict[str, Any]) -> dict[str, Any]:
    ...
    normalized["fixture_policy"] = {
        "policy_level": "eighth_slice_source_backed_fixture_readiness_only",
...
def build_eighth_slice_semantic_compatibility_probe(
    selection_report: dict[str, Any] | None = None,
    readiness_report: dict[str, Any] | None = None,
    selection_path: Path = M63_01_SELECTION,
    readiness_path: Path = M63_02_READINESS,
) -> dict[str, Any]:
    ...
    return {
        "version": "M63-03",
        "description": "Eighth-slice semantic/compatibility probe",
        "selected_target": normalized_selection["selected_target"],
        "source_inputs": {
            "eighth_target_slice_selection": str(selection_path.relative_to(ROOT)),
            "eighth_slice_fixture_readiness": str(readiness_path.relative_to(ROOT)),
        },
        "normalization": {
            "uses_m35_b_c_probe_in_memory": True,
            "adds_format_policy_for_contract": True,
            "adds_fixture_policy_for_contract": True,
            "maps_m63_readiness_to_semantic_scaleout_ready": True,
        ...
        "readiness": {
            "eighth_slice_semantic_compatibility_probe_passed": all_ready,
            "ready_for_m63_04": all_ready,
        ...
        "next_target": {
            "milestone": "M63-04" if all_ready else "M63-repair-semantic",
            "task": "Eighth-slice recipe pipeline entry gate"
            if all_ready
            else "Repair eighth-slice semantic/compatibility blockers",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        "# M63-03 Eighth-Slice Semantic / Compatibility Probe",
        "",
        "## Selected Target",
        "",
    ...
    lines.extend(
        [
            ...
            "- Does not enable G Zone, Stride, or Bloom/token runtime.",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M63-03 eighth-slice semantic/compatibility probe.")
    parser.add_argument("--selection-report", type=Path, default=M63_01_SELECTION)
    parser.add_argument("--readiness-report", type=Path, default=M63_02_READINESS)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_eighth_slice_semantic_compatibility_probe(
        selection_report,
        readiness_report,
    ...
    print(f"M63-03 eighth-slice semantic probe wrote {json_path}")
    print(f"M63-03 eighth-slice semantic probe summary wrote {md_path}")
    print(
        "ready={ready} cards={cards} edges={edges} candidates={candidates} next={next_target}".format(
            ready=report["readiness"]["ready_for_m63_04"],
    ...
    return 0 if report["readiness"]["ready_for_m63_04"] else 1
