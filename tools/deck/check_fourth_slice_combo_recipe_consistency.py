"""Check M48-05 fourth-slice combo-to-recipe consistency."""
...
def build_fourth_slice_consistency_report(
    ...
    return {
        "version": "M48-05",
        "description": "Fourth-slice candidate edge to recipe consistency check",
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M48-06: `{summary['ready_for_m48_06']}`",
        "",
        "## Status Counts",
        "",
    ]
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check M48-05 fourth-slice combo-to-recipe consistency.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_fourth_slice_consistency_report()
    json_path = args.output_dir / "m48_05_fourth_slice_combo_recipe_consistency_report.json"
    md_path = args.output_dir / "m48_05_fourth_slice_combo_recipe_consistency_report.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M48-05 fourth-slice combo recipe consistency wrote {json_path}")
    print(f"M48-05 fourth-slice combo recipe consistency summary wrote {md_path}")
    print(
        "ready_for_m48_06={ready} checked={checked} promotion_allowed={allowed}".format(
            ready=report["summary"]["ready_for_m48_06"],
            checked=report["summary"]["consistency_check_count"],
            allowed=report["summary"]["promotion_allowed_count"],
        )
    )
    return 0 if report["summary"]["ready_for_m48_06"] else 1
