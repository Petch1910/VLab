"""Build the M64-01 eighth-slice fixture scaffold artifact."""
...
JSON_OUTPUT = OUTPUT_DIR / "m64_01_eighth_slice_fixture_scaffold.json"
MD_OUTPUT = OUTPUT_DIR / "m64_01_eighth_slice_fixture_scaffold.md"
...
def build_eighth_slice_fixture_scaffold(
    ...
    if not gate_report.get("summary", {}).get("ready_for_m64"):
        issues.append(
            _issue(
                "entry_gate_not_ready",
                "blocker",
                "M63-04 must allow the offline M64 pipeline before scaffolding fixtures.",
    ...
    if not evidence.get("new_format_or_mechanic_fixtures_required"):
        issues.append(
            _issue(
                ...
                "warning",
                "M64-01 is intended for an eighth slice that requires a new fixture scaffold.",
                {"evidence": evidence},
            )
        )
    ...
    return {
        "version": "M64-01",
        "description": "Eighth-slice fixture scaffold",
        "source_inputs": {
            "eighth_slice_recipe_pipeline_entry_gate": str(M63_04_GATE.relative_to(ROOT)),
        ...
        "fixture_scaffold": {
            "policy_level": "eighth_slice_link_joker_legion_mate_fixture_scaffold_not_full_official_legality",
            ...
            "deferred_source_backed_special_limits": [
                ...
                "Legion pair and mate deck-building constraints",
                "Link Joker/Legion Mate card-specific exceptions not represented in local source data",
            ],
        },
        "mechanic_scope": {
            "format_family": "Link Joker / Legion Mate era",
            "runtime_lock_enabled": False,
            "runtime_legion_enabled": False,
            "legion_pair_validation_deferred": True,
            ...
            "grade4_cards_advisory_only_until_format_support": True,
            ...
            "notes": [
                ...
                "Lock, Unlock, Legion, Mate, and generation-adjacent Grade 4 text stay manual-review until dedicated rules modules exist.",
            ],
        },
        "validator_contract_for_m64_04": {
        ...
        "summary": {
            ...
            "ready_for_m64_02": scaffold_ready,
        },
        "next_target": {
            "milestone": "M64-02" if scaffold_ready else "M64-repair",
            "task": "Eighth-slice review packet"
            if scaffold_ready
            else "Repair eighth-slice fixture scaffold blockers",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines.extend(
        [
            ...
            f"- Ready for M64-02: `{summary['ready_for_m64_02']}`",
            "",
            "## Boundary",
            "",
        ]
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M64-01 eighth-slice fixture scaffold.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_eighth_slice_fixture_scaffold()
    json_path = args.output_dir / JSON_OUTPUT.name
    md_path = args.output_dir / MD_OUTPUT.name
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M64-01 eighth-slice fixture scaffold wrote {json_path}")
    print(f"M64-01 eighth-slice fixture scaffold summary wrote {md_path}")
    print(
        "ready={ready} blockers={blockers} next={next_milestone}".format(
            ready=report["summary"]["ready_for_m64_02"],
            blockers=report["summary"]["blocking_issue_count"],
            next_milestone=report["next_target"]["milestone"],
        )
    )
    return 0 if report["summary"]["ready_for_m64_02"] else 1
