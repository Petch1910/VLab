"""Build the M55-04 sixth-slice recipe pipeline entry gate artifact."""
...
M55_02_READINESS = OUTPUT_DIR / "m55_02_sixth_slice_fixture_readiness.json"
M55_03_PROBE = OUTPUT_DIR / "m55_03_sixth_slice_semantic_compatibility_probe.json"
...
def build_sixth_slice_recipe_pipeline_entry_gate(
    readiness_report: dict[str, Any] | None = None,
    semantic_probe: dict[str, Any] | None = None,
) -> dict[str, Any]:
    readiness_report = readiness_report or load_json(M55_02_READINESS)
    semantic_probe = semantic_probe or load_json(M55_03_PROBE)
    ...
    if not readiness.get("all_fixture_expectations_met"):
        issues.append(
            _issue(
                ...
                "blocker",
                "M55-02 fixture/format readiness must pass before recipe pipeline entry.",
                {"readiness": readiness},
            )
        )
    if not readiness.get("semantic_probe_ready"):
        issues.append(
            _issue(
                ...
                "blocker",
                "M55-02 did not mark the selected slice ready for semantic probe.",
                {"readiness": readiness},
            )
        )
    if not probe_readiness.get("ready_for_m55_04"):
        issues.append(
            _issue(
                "semantic_compatibility_probe_not_ready",
                "blocker",
                "M55-03 semantic/compatibility probe must pass before recipe pipeline entry.",
    ...
    if probe_summary.get("candidate_edge_count", 0) <= 0:
        issues.append(
            _issue(
                "no_candidate_edges",
                "blocker",
                "Sixth slice needs at least one candidate edge before recipe work.",
    ...
    if readiness.get("runtime_or_bot_promotion_allowed") or probe_readiness.get("runtime_or_bot_promotion_allowed"):
        issues.append(
            _issue(
                ...
                {"m55_02": readiness, "m55_03": probe_readiness},
            )
        )
    ...
    return {
        "version": "M55-04",
        "description": "Sixth-slice recipe pipeline entry gate",
        "source_inputs": {
            "sixth_slice_fixture_readiness": str(M55_02_READINESS.relative_to(ROOT)),
            "sixth_slice_semantic_compatibility_probe": str(M55_03_PROBE.relative_to(ROOT)),
        },
        ...
        "evidence_summary": {
            ...
            "semantic_probe_passed": bool(probe_readiness.get("sixth_slice_semantic_compatibility_probe_passed")),
        ...
        "decision": {
            ...
            "recommended_next_queue": "M56 Sixth-slice offline recipe pipeline"
            if offline_recipe_pipeline_allowed
            else "Hold sixth-slice recipe pipeline",
            "reason": (
                "Sixth slice has source-backed readiness plus a passed semantic/compatibility probe; offline recipe work may start with a new fixture scaffold."
                if offline_recipe_pipeline_allowed
                else "Sixth-slice recipe work is blocked until readiness/probe issues are repaired."
            ),
        },
        "proposed_m56_queue": [
            {
                "milestone": "M56-01",
                "task": "Sixth-slice fixture scaffold",
                "scope": "Define source-backed fixture policy for the Shadow Paladin G NEXT/Z slice before validator work.",
            },
            {
                "milestone": "M56-02",
                "task": "Sixth-slice review packet",
                "scope": "Export candidate edges, manual-review cards, and format notes for human review.",
            },
            {
                "milestone": "M56-03",
                "task": "Sixth-slice recipe draft model",
                "scope": "Create advisory recipe drafts only; no saved deck or UI injection.",
            },
            {
                "milestone": "M56-04",
                "task": "Sixth-slice recipe validator",
                "scope": "Validate count, trigger, grade, identity, copy limits, missing cards, and fixture scaffold constraints.",
            },
            {
                "milestone": "M56-05",
                "task": "Sixth-slice combo-to-recipe consistency",
                "scope": "Check candidate combo cards are present and not blocked by manual-review dependency.",
            },
            {
                "milestone": "M56-06",
                "task": "Sixth-slice blocker repair candidates",
                "scope": "Generate source-backed repair candidates for blocked recipes.",
            },
            {
                "milestone": "M56-closeout",
                "task": "Sixth-slice runtime readiness decision",
        ...
        "summary": {
            ...
            "offline_recipe_pipeline_allowed": offline_recipe_pipeline_allowed,
            "ready_for_m56": offline_recipe_pipeline_allowed,
        },
        "next_target": {
            "milestone": "M56-01" if offline_recipe_pipeline_allowed else "M55-repair-semantic",
            "task": "Sixth-slice fixture scaffold"
            if offline_recipe_pipeline_allowed
            else "Repair sixth-slice recipe pipeline entry blockers",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    for item in report["proposed_m56_queue"]:
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M55-04 sixth-slice recipe pipeline entry gate.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_sixth_slice_recipe_pipeline_entry_gate()
    json_path = args.output_dir / "m55_04_sixth_slice_recipe_pipeline_entry_gate.json"
    md_path = args.output_dir / "m55_04_sixth_slice_recipe_pipeline_entry_gate.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M55-04 sixth-slice recipe gate wrote {json_path}")
    print(f"M55-04 sixth-slice recipe gate summary wrote {md_path}")
    print(
        "ready={ready} blockers={blockers} next={next_milestone}".format(
            ready=report["summary"]["ready_for_m56"],
            blockers=report["summary"]["blocking_issue_count"],
            next_milestone=report["next_target"]["milestone"],
        )
    )
    return 0 if report["summary"]["ready_for_m56"] else 1
