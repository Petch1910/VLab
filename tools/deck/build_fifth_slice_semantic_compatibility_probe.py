"""Run the fifth-slice semantic/compatibility probe for M51-03.

This reuses the generalized M35 B/C probe in memory after normalizing the
M51 selection/readiness reports. The probe remains advisory and offline.
"""
...
def _normalize_readiness(readiness_report: dict[str, Any]) -> dict[str, Any]:
    ...
    normalized["fixture_policy"] = {
        "policy_level": "fifth_slice_source_backed_fixture_readiness_only",
...
def build_fifth_slice_semantic_compatibility_probe(
    selection_report: dict[str, Any] | None = None,
    readiness_report: dict[str, Any] | None = None,
    selection_path: Path = M51_01_SELECTION,
    readiness_path: Path = M51_02_READINESS,
) -> dict[str, Any]:
    ...
    return {
        "version": "M51-03",
        "description": "Fifth-slice semantic/compatibility probe",
        "selected_target": normalized_selection["selected_target"],
        "source_inputs": {
            "fifth_target_slice_selection": str(selection_path.relative_to(ROOT)),
            "fifth_slice_fixture_readiness": str(readiness_path.relative_to(ROOT)),
        },
        "normalization": {
            "uses_m35_b_c_probe_in_memory": True,
            "adds_format_policy_for_contract": True,
            "adds_fixture_policy_for_contract": True,
            "maps_m51_readiness_to_semantic_scaleout_ready": True,
        ...
        "readiness": {
            "fifth_slice_semantic_compatibility_probe_passed": all_ready,
            "ready_for_m51_04": all_ready,
        ...
        "next_target": {
            "milestone": "M51-04" if all_ready else "M51-repair-semantic",
            "task": "Fifth-slice recipe pipeline entry gate"
            if all_ready
            else "Repair fifth-slice semantic/compatibility blockers",
        },
    }
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M51-03 fifth-slice semantic/compatibility probe.")
    parser.add_argument("--selection-report", type=Path, default=M51_01_SELECTION)
    parser.add_argument("--readiness-report", type=Path, default=M51_02_READINESS)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_fifth_slice_semantic_compatibility_probe(
        selection_report,
        readiness_report,
        args.selection_report,
        args.readiness_report,
    )
    json_path = args.output_dir / "m51_03_fifth_slice_semantic_compatibility_probe.json"
    md_path = args.output_dir / "m51_03_fifth_slice_semantic_compatibility_probe.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M51-03 fifth-slice semantic probe wrote {json_path}")
    print(f"M51-03 fifth-slice semantic probe summary wrote {md_path}")
    print(
        "ready={ready} cards={cards} edges={edges} candidates={candidates} next={next_target}".format(
            ready=report["readiness"]["ready_for_m51_04"],
    ...
    return 0 if report["readiness"]["ready_for_m51_04"] else 1
