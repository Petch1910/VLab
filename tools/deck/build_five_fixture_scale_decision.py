"""Build the M54-04 five-fixture scale decision artifact."""
...
M54_03_SMOKE = OUTPUT_DIR / "m54_03_fifth_fixture_load_smoke.json"
JSON_OUTPUT = OUTPUT_DIR / "m54_04_five_fixture_scale_decision.json"
MD_OUTPUT = OUTPUT_DIR / "m54_04_five_fixture_scale_decision.md"
...
def build_five_fixture_scale_decision(
    ...
    scale_allowed = not failed and len(fixture_evidence) == 5 and bool(candidates)

    return {
        "version": "M54-04",
        "description": "Five-fixture scale decision",
        "source_inputs": {
        ...
        "decision": {
            "sixth_slice_offline_pipeline_allowed": scale_allowed,
            "sixth_slice_selected_now": False,
            "recommended_next_action": "M55-01 sixth target slice selection" if scale_allowed else "repair_failed_fixture_evidence",
            ...
            "stride_runtime_enabled": False,
            "decision_notes": [
                "Five independent fixtures now pass offline and Unity headless load smoke.",
                "The next step may select a sixth slice for offline analysis only.",
                "Do not publish fixture decks to UI or bot runtime without a later explicit gate.",
                "G Zone and Stride runtime remain disabled despite legacy and G-series fixture evidence.",
            ]
            if scale_allowed
            else [
        ...
        "summary": {
            ...
            "sixth_slice_offline_pipeline_allowed": scale_allowed,
            "ready_for_m55": scale_allowed,
        },
        "next_target": {
            "milestone": "M55-01" if scale_allowed else "M54-repair",
            "task": "Sixth target slice selection" if scale_allowed else "Repair failed fixture evidence",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Sixth-slice offline pipeline allowed: `{summary['sixth_slice_offline_pipeline_allowed']}`",
        f"- Ready for M55: `{summary['ready_for_m55']}`",
        "",
        "## Fixture Evidence",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "## Decision",
            "",
            f"- Sixth slice selected now: `{decision['sixth_slice_selected_now']}`",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M54-04 five-fixture scale decision.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"M54-04 five-fixture scale decision wrote {json_path}")
    print(f"M54-04 five-fixture scale decision summary wrote {md_path}")
    print(
        "ready_for_m55={ready} fixtures_passed={passed} candidates={candidates}".format(
            ready=report["summary"]["ready_for_m55"],
            passed=report["summary"]["passed_fixture_count"],
            candidates=report["summary"]["candidate_count"],
        )
    )
    return 0 if report["summary"]["ready_for_m55"] else 1
