"""Build the M61-01 seventh-slice human repair review packet."""
...
M60_CLOSEOUT = OUTPUT_DIR / "m60_closeout_seventh_slice_runtime_readiness.json"
M60_REPAIRS = OUTPUT_DIR / "m60_06_seventh_slice_blocker_repair_candidates.json"
...
def _decision_options() -> list[dict[str, str]]:
    return [
        {
            "option_id": "accept_recipe_repairs_keep_g_zone_and_bloom_deferred_for_validation_rerun",
            "label": "Accept recipe repairs and keep G Zone/Bloom deferred for validation rerun",
            "effect": "Allows M61-02/M61-03 to record recipe/repair choice, but runtime promotion remains blocked by system decisions.",
        },
        {
        ...
        },
        {
            ...
            "effect": "Keeps the recipe advisory and asks for another repair candidate or explicit G Zone/Bloom scope decision.",
        },
        {
...
def _g_zone_decision_options() -> list[dict[str, str]]:
    return [
        {
            "option_id": "defer_until_g_zone_stride_runtime_exists",
            "label": "Defer until G Zone and Stride runtime exists",
            "effect": "Keeps the recipe advisory until G Zone deck slots, Stride validation, and Generation Break support exist.",
        },
        {
...
def _bloom_token_decision_options() -> list[dict[str, str]]:
    return [
        {
            "option_id": "defer_until_bloom_token_runtime_exists",
            "label": "Defer until Bloom/token runtime exists",
            "effect": "Keeps the recipe advisory until Bloom templates, same-name checks, token lifecycle, and duration cleanup exist.",
        },
        {
            "option_id": "manual_semantic_review_only_no_runtime_promotion",
            "label": "Manual semantic review only, no runtime promotion",
            "effect": "Allows review of Bloom/token-like card text but blocks fixture/runtime promotion.",
        },
    ]
...
def _review_item(repair_item: dict[str, Any], draft: dict[str, Any] | None) -> dict[str, Any]:
    ...
    bloom_token_package = repair_item.get("bloom_token_deferred_package") or {}
    ...
    return {
        "review_item_id": f"m61_01_{repair_item['recipe_id']}_repair_review",
        "item_type": "seventh_slice_human_repair_g_zone_bloom_review",
        ...
        "g_zone_can_be_repaired_in_m61_01": False,
        "g_zone_future_system_work": g_zone_package.get("requires_future_system_work", []),
        "bloom_token_package_id": bloom_token_package.get("package_id", ""),
        "bloom_token_deferred": bool(bloom_token_package),
        "bloom_token_can_be_repaired_in_m61_01": False,
        "bloom_token_future_system_work": bloom_token_package.get("requires_future_system_work", []),
        ...
        "bloom_token_decision_options": _bloom_token_decision_options(),
        "recommended_reviewer_action": "select_one_recipe_review_manual_grade_repairs_and_record_g_zone_bloom_decisions_before_m61_02",
    }


def build_seventh_slice_human_repair_review_packet(
    closeout: dict[str, Any] | None = None,
    repairs: dict[str, Any] | None = None,
    drafts: dict[str, Any] | None = None,
) -> dict[str, Any]:
    closeout = closeout or load_json(M60_CLOSEOUT)
    repairs = repairs or load_json(M60_REPAIRS)
    ...
    bloom_token_items = [item for item in review_items if item["bloom_token_deferred"]]
    ...
    return {
        "version": "M61-01",
        "description": "Seventh-slice human repair review packet",
        "selected_target": repairs.get("selected_target", closeout.get("key_results", {}).get("selected_target", {})),
        "source_inputs": {
            "m60_closeout": str(M60_CLOSEOUT.relative_to(ROOT)),
            "m60_repair_candidates": str(M60_REPAIRS.relative_to(ROOT)),
        ...
        "review_policy": {
            ...
            "packet_is_not_bloom_token_decision": True,
            "runtime_promotion_allowed": False,
            "m61_02_must_record_explicit_selection": True,
            "m61_03_must_record_explicit_acceptance": True,
            "m61_04_must_record_g_zone_stride_bloom_token_decision": True,
        ...
        "summary": {
            ...
            "bloom_token_deferred_item_count": len(bloom_token_items),
            ...
            "bloom_token_decision_option_count": len(_bloom_token_decision_options()),
            "ready_for_m61_02": ready_for_m61_02,
        },
        "review_items": review_items,
        "next_target": {
            "milestone": "M61-02",
            "task": "Seventh-slice human-selected recipe artifact",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        "# M61-01 Seventh-Slice Human Repair Review Packet",
        "",
        "## Summary",
        "",
        ...
        f"- Bloom/token deferred items: `{summary['bloom_token_deferred_item_count']}`",
        ...
        f"- Ready for M61-02: `{summary['ready_for_m61_02']}`",
        "",
        "## Review Items",
        "",
    ]
    for item in report["review_items"]:
        ...
        lines.extend(
            [
                ...
                f"- Bloom/token package: `{item['bloom_token_package_id']}`",
    ...
    for option in _g_zone_decision_options():
        lines.append(f"- `{option['option_id']}`: {option['label']} - {option['effect']}")
    lines.extend(["", "## Bloom/Token Decision Options", ""])
    for option in _bloom_token_decision_options():
        lines.append(f"- `{option['option_id']}`: {option['label']} - {option['effect']}")
    lines.extend(
        [
            "",
            "## Policy",
            "",
            "- This packet is not human selection, acceptance, G Zone decision, or Bloom/token decision.",
            "- Runtime promotion remains disabled.",
            "- M61-02 must record exactly one selected recipe.",
            "- M61-03 must record explicit acceptance or rejection.",
            "- M61-04 must record explicit G Zone/Stride and Bloom/token decisions.",
            "- M61-05 must rerun repaired validation before any fixture gate.",
            "",
            "## Next",
            "",
            "`M61-02`: Seventh-slice human-selected recipe artifact.",
            "",
        ]
    )
    path.parent.mkdir(parents=True, exist_ok=True)
        ...
        writer.writerow(
            {
                ...
                "bloom_token_package_id": item["bloom_token_package_id"],
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M61-01 seventh-slice human repair review packet.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_seventh_slice_human_repair_review_packet()
    json_path = args.output_dir / "m61_01_seventh_slice_human_repair_review_packet.json"
    md_path = args.output_dir / "m61_01_seventh_slice_human_repair_review_packet.md"
    csv_path = args.output_dir / "m61_01_seventh_slice_human_repair_review_packet.csv"
    write_json(report, json_path)
    write_markdown(report, md_path)
    write_csv(report, csv_path)
    print(f"M61-01 seventh-slice human repair review packet wrote {json_path}")
    print(f"M61-01 seventh-slice human repair review packet summary wrote {md_path}")
    print(f"M61-01 seventh-slice human repair review packet CSV wrote {csv_path}")
    print(
        "ready_for_m61_02={ready} review_items={items}".format(
            ready=report["summary"]["ready_for_m61_02"],
            items=report["summary"]["review_item_count"],
        )
    )
    return 0 if report["summary"]["ready_for_m61_02"] else 1
