"""Build fourth-slice fixture/format readiness evidence for M47-02."""
...
ERA_SCOPES = {
    "g_series_first": {
        "trial_decks": [f"G-TD{i:02d}" for i in range(1, 10)],
        "start_decks": [f"G-SD{i:02d}" for i in range(1, 3)],
        "booster_packs": [f"G-BT{i:02d}" for i in range(1, 9)],
        "clan_boosters": [f"G-CB{i:02d}" for i in range(1, 5)],
        "technical_boosters": [f"G-TCB{i:02d}" for i in range(1, 3)],
    }
}
...
def _series_scope(era_preset: str) -> list[str]:
    ...
    return (
        ...
        + list(scope.get("start_decks", []))
        + list(scope.get("booster_packs", []))
        + list(scope.get("clan_boosters", []))
        + list(scope.get("technical_boosters", []))
    )
...
def build_fourth_slice_fixture_readiness(selection_report: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        "version": "M47-02",
        "description": "Fourth-slice fixture/format readiness",
        "source_inputs": {
            "fourth_target_slice_selection": str(M47_01_SELECTION.relative_to(ROOT)),
        ...
        "format_scope": {
            ...
            "start_decks": ERA_SCOPES.get(era_preset, {}).get("start_decks", []),
            "booster_packs": ERA_SCOPES.get(era_preset, {}).get("booster_packs", []),
            "clan_boosters": ERA_SCOPES.get(era_preset, {}).get("clan_boosters", []),
            "technical_boosters": ERA_SCOPES.get(era_preset, {}).get("technical_boosters", []),
            ...
            else "requires_fourth_slice_readiness_repair",
        },
        "card_pool_summary": {
        ...
        "next_target": {
            "milestone": "M47-03" if fixture_expectations_met else "M47-repair",
            "task": "Fourth-slice semantic/compatibility probe" if fixture_expectations_met else "Repair fourth-slice readiness blockers",
        },
        "summary": {
            ...
            "all_fixture_expectations_met": fixture_expectations_met,
            "semantic_probe_ready": fixture_expectations_met,
            "ready_for_m47_03": fixture_expectations_met,
            "repair_required": not fixture_expectations_met,
            "repair_reasons": repair_reasons,
        },
    }
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M47-02 fourth-slice fixture readiness report.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    json_path = args.output_dir / "m47_02_fourth_slice_fixture_readiness.json"
    md_path = args.output_dir / "m47_02_fourth_slice_fixture_readiness.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M47-02 fourth-slice fixture readiness wrote {json_path}")
    print(f"M47-02 fourth-slice fixture readiness summary wrote {md_path}")
    print(
