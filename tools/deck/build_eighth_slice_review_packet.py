"""Build the M64-02 eighth-slice review packet."""
...
from tools.deck.build_eighth_slice_semantic_compatibility_probe import (  # noqa: E402
...
M63_01_SELECTION = OUTPUT_DIR / "m63_01_eighth_target_slice_selection.json"
...
M63_04_GATE = OUTPUT_DIR / "m63_04_eighth_slice_recipe_pipeline_entry_gate.json"
M64_01_SCAFFOLD = OUTPUT_DIR / "m64_01_eighth_slice_fixture_scaffold.json"
JSON_OUTPUT = OUTPUT_DIR / "m64_02_eighth_slice_review_packet.json"
MD_OUTPUT = OUTPUT_DIR / "m64_02_eighth_slice_review_packet.md"
CSV_OUTPUT = OUTPUT_DIR / "m64_02_eighth_slice_review_packet.csv"
...
def _scaffold_note_item(scaffold_report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "item_id": "m64_01_fixture_scaffold",
        ...
        "mechanic_manual_review_required": bool(
            ...
            or mechanic.get("legion_or_mate_text_requires_manual_review")
            or mechanic.get("grade4_cards_advisory_only_until_format_support")
        ),
        "lock_runtime_enabled": bool(mechanic.get("runtime_lock_enabled")),
        "legion_runtime_enabled": bool(mechanic.get("runtime_legion_enabled")),
        "legion_pair_validation_deferred": bool(mechanic.get("legion_pair_validation_deferred")),
        "lock_or_unlock_text_manual_review_required": bool(
            mechanic.get("lock_or_unlock_text_requires_manual_review")
        ),
        "legion_or_mate_text_manual_review_required": bool(
            mechanic.get("legion_or_mate_text_requires_manual_review")
        ),
        "grade4_cards_advisory_only": bool(mechanic.get("grade4_cards_advisory_only_until_format_support")),
        "review_reasons": [
            "eighth_slice_link_joker_legion_mate_fixture_scaffold",
            "lock_legion_manual_review_boundary",
            "must_review_before_recipe_validator",
        ],
        "blocked_until": [
            "fixture_scaffold_reviewed",
            "m64_04_recipe_validator_uses_scaffold",
            "lock_legion_manual_review_policy_confirmed",
...
def _candidate_edge_item(edge: dict[str, Any], rank: int) -> dict[str, Any]:
    return {
        ...
        "review_reasons": [
            "eighth_slice_candidate_compatibility_edge",
            "requires_human_recipe_selection_before_m64_03",
        ],
        "blocked_until": [
        ...
        "recommended_next_action": "consider_for_m64_03_advisory_recipe_draft",
    }
...
def build_eighth_slice_review_packet(
    ...
    selection_report = selection_report or load_json(M63_01_SELECTION)
    readiness_report = readiness_report or load_json(M63_02_READINESS)
    probe_report = probe_report or load_json(M63_03_PROBE)
    gate_report = gate_report or load_json(M63_04_GATE)
    ...
    gate_allows_offline = bool(gate_report.get("summary", {}).get("ready_for_m64"))
    scaffold_ready = bool(scaffold_report.get("summary", {}).get("ready_for_m64_02"))
    ...
    return {
        "version": "M64-02",
        "description": "Eighth-slice review packet before advisory deck recipe drafting",
        "selected_target": selection_report.get("selected_target", {}),
        "source_inputs": {
            "eighth_target_slice_selection": _rel(M63_01_SELECTION),
            "eighth_slice_fixture_readiness": _rel(M63_02_READINESS),
            "eighth_slice_semantic_compatibility_probe": _rel(M63_03_PROBE),
            "eighth_slice_recipe_pipeline_entry_gate": _rel(M63_04_GATE),
            "eighth_slice_fixture_scaffold": _rel(M64_01_SCAFFOLD),
        },
        "scope": {
        ...
        "review_policy": {
            ...
            "lock_and_legion_runtime_support_deferred": True,
            "lock_or_unlock_text_requires_manual_review": True,
            "legion_or_mate_text_requires_manual_review": True,
            "grade4_cards_advisory_only": True,
            "no_live_card_text_parsing": True,
            "no_direct_GameState_mutation": True,
        },
        "evidence_summary": {
            "m63_04_offline_recipe_pipeline_allowed": gate_allows_offline,
            "m63_04_runtime_promotion_closed": promotion_closed,
            "m64_01_scaffold_ready": scaffold_ready,
        ...
        "next_target": {
            ...
            "blocked_until": [
                "M64-02 review packet is available",
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M64-03: `{summary['ready_for_m64_03']}`",
        "",
        "## Fixture Scaffold",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "- Lock and Legion runtime support remains deferred.",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M64-02 eighth-slice review packet.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"M64-02 review packet wrote {json_path}")
    print(f"M64-02 review packet summary wrote {md_path}")
    print(f"M64-02 review packet CSV wrote {csv_path}")
    print(
        "ready={ready} scaffold={scaffold} manual={manual} candidates={candidates}".format(
            ready=report["summary"]["ready_for_m64_03"],
    ...
    return 0 if report["summary"]["ready_for_m64_03"] else 1
