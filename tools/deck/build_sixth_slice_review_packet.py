"""Build the M56-02 sixth-slice review packet."""
...
M55_04_GATE = OUTPUT_DIR / "m55_04_sixth_slice_recipe_pipeline_entry_gate.json"
...
def _scaffold_note_item(scaffold_report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "item_id": "m56_01_fixture_scaffold",
        ...
        "mechanic_manual_review_required": bool(
            ...
            or mechanic.get("ritual_or_retire_text_requires_manual_review")
        ),
        ...
        "ritual_or_retire_text_manual_review_required": bool(
            mechanic.get("ritual_or_retire_text_requires_manual_review")
        ),
        "review_reasons": [
            "new_format_or_mechanic_fixture_scaffold",
            "g_next_z_grade4_g_zone_boundary_review",
            "shadow_paladin_ritual_retire_boundary_review",
            "must_review_before_recipe_validator",
        ],
        "blocked_until": [
            "fixture_scaffold_reviewed",
            "m56_04_recipe_validator_uses_scaffold",
            "g_zone_support_decision",
            "ritual_retire_manual_review_policy_confirmed",
        ],
...
def _candidate_edge_item(edge: dict[str, Any], rank: int) -> dict[str, Any]:
    return {
        ...
        "review_reasons": [
            "sixth_slice_candidate_compatibility_edge",
            "requires_human_recipe_selection_before_m56_03",
        ],
        "blocked_until": [
            "human_recipe_selection",
            "m56_04_recipe_validator",
            "combo_line_to_recipe_consistency_check",
        ],
        "recommended_next_action": "consider_for_m56_03_advisory_recipe_draft",
    }
...
def build_sixth_slice_review_packet(
    ...
    probe_report = probe_report or load_json(M55_03_PROBE)
    gate_report = gate_report or load_json(M55_04_GATE)
    ...
    gate_allows_offline = bool(gate_report.get("summary", {}).get("ready_for_m56"))
    scaffold_ready = bool(scaffold_report.get("summary", {}).get("ready_for_m56_02"))
    ...
    return {
        "version": "M56-02",
        "description": "Sixth-slice review packet before advisory deck recipe drafting",
        "selected_target": selection_report.get("selected_target", {}),
        "source_inputs": {
            "sixth_target_slice_selection": _rel(M55_01_SELECTION),
            "sixth_slice_fixture_readiness": _rel(M55_02_READINESS),
            "sixth_slice_semantic_compatibility_probe": _rel(M55_03_PROBE),
            "sixth_slice_recipe_pipeline_entry_gate": _rel(M55_04_GATE),
            "sixth_slice_fixture_scaffold": _rel(M56_01_SCAFFOLD),
        },
        "scope": {
        ...
        "review_policy": {
            ...
            "ritual_retire_text_requires_manual_review": True,
            "no_live_card_text_parsing": True,
            "no_direct_GameState_mutation": True,
        },
        "evidence_summary": {
            "m55_04_offline_recipe_pipeline_allowed": gate_allows_offline,
            "m55_04_runtime_promotion_closed": promotion_closed,
            "m56_01_scaffold_ready": scaffold_ready,
        ...
        "next_target": {
            ...
            "blocked_until": [
                "M56-02 review packet is available",
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M56-03: `{summary['ready_for_m56_03']}`",
        "",
        "## Fixture Scaffold",
        "",
    ]
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M56-02 sixth-slice review packet.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_sixth_slice_review_packet()
    json_path = args.output_dir / "m56_02_sixth_slice_review_packet.json"
    md_path = args.output_dir / "m56_02_sixth_slice_review_packet.md"
    csv_path = args.output_dir / "m56_02_sixth_slice_review_packet.csv"
    write_json(report, json_path)
    write_markdown(report, md_path)
    write_csv(report, csv_path)
    print(f"M56-02 review packet wrote {json_path}")
    print(f"M56-02 review packet summary wrote {md_path}")
    print(f"M56-02 review packet CSV wrote {csv_path}")
    print(
        "ready={ready} scaffold={scaffold} manual={manual} candidates={candidates}".format(
            ready=report["summary"]["ready_for_m56_03"],
    ...
    return 0 if report["summary"]["ready_for_m56_03"] else 1
