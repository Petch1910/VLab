"""Build M48-06 fourth-slice blocker repair candidates.

This creates advisory repair candidates for blockers reported by M48-04 and
carried through M48-05. It does not mutate M48-03 drafts and does not create
...
M48_RECIPE_DRAFTS = OUTPUT_DIR / "m48_03_fourth_slice_recipe_draft_model.json"
M48_VALIDATION = OUTPUT_DIR / "m48_04_fourth_slice_recipe_validation_report.json"
M48_CONSISTENCY = OUTPUT_DIR / "m48_05_fourth_slice_combo_recipe_consistency_report.json"
M48_SCAFFOLD = OUTPUT_DIR / "m48_01_fourth_slice_fixture_scaffold.json"
    ...
    rows = [
        {
            ...
            "source": "m48_03_fourth_slice_recipe_draft_model",
        }
        for card_id, quantity in quantities.items()
...
def _build_g_zone_package(recipe_id: str, validation: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        ...
        "advisory_only": True,
        "can_be_repaired_in_m48_06": False,
...
def build_fourth_slice_blocker_repair_candidates(
    recipe_report: dict[str, Any] | None = None,
    validation_report: dict[str, Any] | None = None,
    consistency_report: dict[str, Any] | None = None,
    scaffold: dict[str, Any] | None = None,
) -> dict[str, Any]:
    recipe_report = recipe_report or load_json(M48_RECIPE_DRAFTS)
    validation_report = validation_report or load_json(M48_VALIDATION)
    consistency_report = consistency_report or load_json(M48_CONSISTENCY)
    ...
    return {
        "version": "M48-06",
        "description": "Fourth-slice blocker repair candidates",
        "selected_target": recipe_report.get("selected_target", {}),
        "source_inputs": {
            "fourth_slice_recipe_draft_model": str(M48_RECIPE_DRAFTS.relative_to(ROOT)),
            "fourth_slice_recipe_validation_report": str(M48_VALIDATION.relative_to(ROOT)),
            "fourth_slice_combo_recipe_consistency_report": str(M48_CONSISTENCY.relative_to(ROOT)),
        ...
        "repair_policy": {
            ...
            "series_scope_from_m48_01_fixture_scaffold": series_scope,
        ...
        "summary": {
            ...
            "runtime_promotion_allowed": False,
            "ready_for_m48_closeout": bool(repair_items) and unexpected_structural_blocker_count == 0,
        },
        "next_target": {
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M48-closeout: `{summary['ready_for_m48_closeout']}`",
        "",
        "## Repair Items",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "- M48-03 draft files are not modified.",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M48-06 fourth-slice blocker repair candidates.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_fourth_slice_blocker_repair_candidates()
    json_path = args.output_dir / "m48_06_fourth_slice_blocker_repair_candidates.json"
    md_path = args.output_dir / "m48_06_fourth_slice_blocker_repair_candidates.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M48-06 fourth-slice blocker repair candidates wrote {json_path}")
    print(f"M48-06 fourth-slice blocker repair candidates summary wrote {md_path}")
    print(
        "ready_for_m48_closeout={ready} recipes={recipes} human_review_ready={ready_count} g_zone_deferred={g_zone}".format(
            ready=report["summary"]["ready_for_m48_closeout"],
    ...
    return 0 if report["summary"]["ready_for_m48_closeout"] else 1
