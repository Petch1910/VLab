"""Validate M58-01 sixth runtime fixture schema and G Zone / Stride boundary."""
...
def _normalize_fixture_for_generic_validator(fixture: dict[str, Any]) -> dict[str, Any]:
    """Keep M57-06 plural source metadata compatible with the shared v1 validator."""
...
def _append_g_zone_stride_boundary_issues(report: dict[str, Any], fixture: dict[str, Any]) -> None:
    ...
    if source_packages.get("selected_g_zone_option_id") != EXPECTED_G_ZONE_OPTION:
        issues.append(
            _issue(
                "g_zone_policy_mismatch",
                "Sixth fixture must come from the accepted main-deck-only G Zone / Stride option.",
                {
                    "expected": EXPECTED_G_ZONE_OPTION,
                    "actual": source_packages.get("selected_g_zone_option_id"),
                },
            )
        )
        ...
        if g_zone_boundary.get(field) != expected:
            issues.append(
                _issue(
                    ...
                    "Sixth fixture G Zone / Stride boundary has an unsafe value.",
        ...
        if runtime_boundaries.get(field) != expected:
            issues.append(
                _issue(
                    ...
                    "Sixth fixture runtime boundary must keep G Zone and Stride disabled.",
    ...
    if count_summary.get("grade4_main_deck_count") != 0:
        issues.append(
            _issue(
                ...
                "Sixth fixture main deck must not include G Units or grade 4 cards.",
...
def build_sixth_runtime_fixture_schema_validation_report(
    ...
    report["version"] = "M58-01"
    report["description"] = "Sixth runtime fixture schema validation"
    report["source_inputs"] = {
        "runtime_fixture": _display_path(fixture_path),
        "runtime_cards_sqlite": str(CARDS_SQLITE.relative_to(ROOT)),
    }
    report["scope"]["sixth_fixture_schema_validator"] = True
    report["scope"]["g_zone_stride_boundary_enforced"] = True
    report["validation_policy"]["g_zone_stride_boundary"] = {
        "selected_option_id": EXPECTED_G_ZONE_OPTION,
        "main_deck_only_validation_allowed": True,
        "g_zone_runtime_enabled": False,
        "stride_runtime_enabled": False,
        ...
        "accepts_m57_06_source_artifacts_plural": True,
    }
    _append_g_zone_stride_boundary_issues(report, fixture)
    _refresh_summary(report)
    report["summary"].pop("ready_for_m39_02", None)
    report["summary"]["ready_for_m58_02"] = report["summary"]["schema_valid"]
    report["next_target"] = {
        "milestone": "M58-02",
        "task": "Sixth fixture deck text exporter",
    }
    return report
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    policy = report["validation_policy"]["g_zone_stride_boundary"]
    lines = [
        ...
        f"- Ready for M58-02: `{summary['ready_for_m58_02']}`",
        "",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate M58-01 sixth runtime fixture schema.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_sixth_runtime_fixture_schema_validation_report(fixture, fixture_path)
    json_path = args.output_dir / "m58_01_sixth_fixture_schema_validation.json"
    md_path = args.output_dir / "m58_01_sixth_fixture_schema_validation.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M58-01 sixth fixture schema validation wrote {json_path}")
    print(f"M58-01 sixth fixture schema validation summary wrote {md_path}")
    print(
        "schema_valid={valid} blockers={blockers} next={next_ready}".format(
            ...
            next_ready=report["summary"]["ready_for_m58_02"],
        )
    )
