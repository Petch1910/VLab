"""Build the M62-04 seven-fixture scale decision artifact."""
...
M62_03_SMOKE = OUTPUT_DIR / "m62_03_seventh_fixture_load_smoke.json"
JSON_OUTPUT = OUTPUT_DIR / "m62_04_seven_fixture_scale_decision.json"
MD_OUTPUT = OUTPUT_DIR / "m62_04_seven_fixture_scale_decision.md"
...
def build_seven_fixture_scale_decision(
    ...
    scale_allowed = not failed and len(fixture_evidence) == 7 and bool(candidates)

    return {
        "version": "M62-04",
        "description": "Seven-fixture scale decision",
        "source_inputs": {
        ...
        "decision": {
            "eighth_slice_offline_pipeline_allowed": scale_allowed,
            "eighth_slice_selected_now": False,
            "recommended_next_action": "M63-01 eighth target slice selection" if scale_allowed else "repair_failed_fixture_evidence",
            ...
            "bloom_token_runtime_enabled": False,
            "decision_notes": [
                "Seven independent fixtures now pass offline and Unity headless load smoke.",
                "The next step may define and select an eighth slice for offline analysis only.",
                "Do not publish fixture decks to UI or bot runtime without a later explicit gate.",
                "G Zone, Stride, and Bloom/token runtime remain disabled despite G-series fixture evidence.",
            ]
            if scale_allowed
            else [
        ...
        "summary": {
            ...
            "eighth_slice_offline_pipeline_allowed": scale_allowed,
            "ready_for_m63": scale_allowed,
        },
        "next_target": {
            "milestone": "M63-01" if scale_allowed else "M62-repair",
            "task": "Eighth target slice selection" if scale_allowed else "Repair failed fixture evidence",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Eighth-slice offline pipeline allowed: `{summary['eighth_slice_offline_pipeline_allowed']}`",
        f"- Ready for M63: `{summary['ready_for_m63']}`",
        "",
        "## Fixture Evidence",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "## Decision",
            "",
            f"- Eighth slice selected now: `{decision['eighth_slice_selected_now']}`",
            ...
            "- It does not enable G Zone, Stride, or Bloom/token runtime.",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M62-04 seven-fixture scale decision.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"M62-04 seven-fixture scale decision wrote {json_path}")
    print(f"M62-04 seven-fixture scale decision summary wrote {md_path}")
    print(
        "ready_for_m63={ready} fixtures_passed={passed} candidates={candidates}".format(
            ready=report["summary"]["ready_for_m63"],
            passed=report["summary"]["passed_fixture_count"],
            candidates=report["summary"]["candidate_count"],
        )
    )
    return 0 if report["summary"]["ready_for_m63"] else 1
