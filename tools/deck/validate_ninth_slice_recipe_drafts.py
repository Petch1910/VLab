"""Validate M68-03 ninth-slice recipe drafts for M68-04."""
...
RECIPE_DRAFTS = OUTPUT_DIR / "m68_03_ninth_slice_recipe_draft_model.json"
CARDS_SQLITE = ROOT / "data" / "packs" / "vanguard_th" / "cards.sqlite"

EXPECTED_SERIES_SCOPE = {"G-BT02", "G-CB02", "G-TD04"}
EXPECTED_TRIGGER_PROFILE = {"Critical": 4, "Draw": 4, "Heal": 4, "Stand": 4}
EXPECTED_GRADE_TARGETS = {"0": 17, "1": 14, "2": 11, "3": 8}
...
def load_card_rows(card_ids: Sequence[str], sqlite_path: Path = CARDS_SQLITE) -> dict[str, dict[str, Any]]:
    ...
    query = (
        "select card_id, name_th, clan, grade, trigger, deck_limit, type_1, series_code "
...
def _validate_recipe(recipe: dict[str, Any], card_rows: dict[str, dict[str, Any]], expected_clan: str) -> dict[str, Any]:
    ...
    for row in recipe.get("quantities", []):
        ...
        series_counts[card.get("series_code") or ""] += quantity
    ...
    if trigger_total != 16:
        issues.append(
            _issue(
                ...
                "blocker",
                "G-era trigger count must be 16 before recipe can be promoted.",
                {"expected": 16, "actual": trigger_total, "by_trigger": dict(trigger_counts)},
            )
        )
    trigger_profile = {key: trigger_counts.get(key, 0) for key in EXPECTED_TRIGGER_PROFILE}
    if trigger_profile != EXPECTED_TRIGGER_PROFILE:
        issues.append(
            _issue(
                "trigger_profile_mismatch",
                "blocker",
                "Ninth-slice fixture requires the 4/4/4/4 Critical/Draw/Heal/Stand trigger profile.",
                {"expected": EXPECTED_TRIGGER_PROFILE, "actual": trigger_profile},
            )
        )
    if trigger_counts.get("Heal", 0) > 4:
        issues.append(
            _issue(
                ...
                "blocker",
                "Heal trigger count exceeds G-era guardrail.",
                {"heal_count": trigger_counts["Heal"]},
            )
        )
    if grade_counts.get("4", 0) > 0:
        issues.append(
            _issue(
                "grade4_main_deck_not_allowed",
                "blocker",
                "Grade 4/G-unit cards must not enter main deck drafts until G Zone support exists.",
                {"grade4_count": grade_counts["4"]},
            )
        )

    missing_required_grades = [
        grade for grade in ("0", "1", "2", "3")
        if grade_counts.get(grade, 0) <= 0
    ]
    if missing_required_grades:
        issues.append(
            _issue(
                "required_grade_coverage_missing",
                "blocker",
                "Recipe is missing one or more required setup grades.",
                {"missing_grades": missing_required_grades, "grade_counts": dict(sorted(grade_counts.items()))},
            )
        )
    ...
    off_series = {
        ...
        if series and series not in EXPECTED_SERIES_SCOPE
    }
    if off_series:
        issues.append(
            _issue(
                "set_scope_mismatch",
                "blocker",
                "Recipe contains cards outside the ninth-slice source series scope.",
                {"expected_series_scope": sorted(EXPECTED_SERIES_SCOPE), "off_series_counts": off_series},
            )
        )

    grade_mismatches = {
        grade: {"expected": expected, "actual": grade_counts.get(grade, 0)}
        for grade, expected in EXPECTED_GRADE_TARGETS.items()
    ...
    if manual_ids:
        issues.append(
            _issue(
                ...
                "blocker",
                "Recipe contains cards that M68-02 marked as manual-review required.",
                {"card_ids": sorted(manual_ids)},
            )
        )

    metadata = recipe.get("validation_metadata", {})
    if metadata.get("g_zone_support_deferred"):
        issues.append(
            _issue(
                "g_zone_support_deferred",
                "review",
                "G Zone support is deferred; this draft can only validate main-deck quantities.",
                {},
            )
        )
    if metadata.get("stride_support_deferred"):
        issues.append(
            _issue(
                "stride_support_deferred",
                "review",
                "Stride/G-unit support is deferred until dedicated rules support exists.",
                {},
            )
        )
    if metadata.get("aqua_force_battle_order_support_deferred"):
        issues.append(
            _issue(
                "aqua_force_battle_order_support_deferred",
                "review",
                "Aqua Force battle-count and attack-order text remains manual-review only.",
                {},
            )
        )
...
def build_ninth_slice_validation_report(
    ...
    return {
        "version": "M68-04",
        "description": "Ninth-slice recipe validator for M68-03 advisory drafts",
        "selected_target": recipe_report.get("selected_target", {}),
        "source_inputs": {
            "ninth_slice_recipe_draft_model": str(RECIPE_DRAFTS.relative_to(ROOT)),
        ...
        "validation_policy": {
            "main_deck_size": 50,
            "trigger_count": 16,
            "required_trigger_profile": EXPECTED_TRIGGER_PROFILE,
            "heal_trigger_max": 4,
            "required_setup_grades": ["0", "1", "2", "3"],
            "grade4_main_deck_max_until_g_zone_support": 0,
            "expected_series_scope": sorted(EXPECTED_SERIES_SCOPE),
            ...
            "g_zone_support_deferred_is_review_not_blocker": True,
            "stride_support_deferred_is_review_not_blocker": True,
            "aqua_force_battle_order_deferred_is_review_not_blocker": True,
        },
        "summary": {
            ...
            "trigger_profile_mismatch_recipe_count": sum(
                1 for item in validations if any(issue["code"] == "trigger_profile_mismatch" for issue in item["issues"])
            ),
            "required_grade_coverage_missing_recipe_count": sum(
                1 for item in validations if any(issue["code"] == "required_grade_coverage_missing" for issue in item["issues"])
            ),
            "set_scope_mismatch_recipe_count": sum(
                1 for item in validations if any(issue["code"] == "set_scope_mismatch" for issue in item["issues"])
            ),
            "grade4_main_deck_recipe_count": sum(
                1 for item in validations if any(issue["code"] == "grade4_main_deck_not_allowed" for issue in item["issues"])
            ),
            ...
            "g_zone_deferred_recipe_count": sum(
                1 for item in validations if any(issue["code"] == "g_zone_support_deferred" for issue in item["issues"])
            ),
            "stride_deferred_recipe_count": sum(
                1 for item in validations if any(issue["code"] == "stride_support_deferred" for issue in item["issues"])
            ),
            "aqua_force_battle_order_deferred_recipe_count": sum(
                1 for item in validations if any(
                    issue["code"] == "aqua_force_battle_order_support_deferred"
                    for issue in item["issues"]
                )
            ),
            "issue_counts": dict(sorted(issue_counts.items())),
            "ready_for_m68_05": bool(validations),
        },
        "recipe_validations": validations,
        "next_target": {
            "milestone": "M68-05",
            "task": "Ninth-slice combo-to-recipe consistency",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        "# M68-04 Ninth-Slice Recipe Validation Report",
        "",
        "## Summary",
        "",
        ...
        f"- Trigger-profile mismatch recipes: `{summary['trigger_profile_mismatch_recipe_count']}`",
        f"- Required-grade coverage missing recipes: `{summary['required_grade_coverage_missing_recipe_count']}`",
        f"- Set-scope mismatch recipes: `{summary['set_scope_mismatch_recipe_count']}`",
        f"- Grade 4 main-deck recipes: `{summary['grade4_main_deck_recipe_count']}`",
        ...
        f"- Stride deferred recipes: `{summary['stride_deferred_recipe_count']}`",
        f"- Aqua Force battle-order deferred recipes: `{summary['aqua_force_battle_order_deferred_recipe_count']}`",
        f"- Ready for M68-05: `{summary['ready_for_m68_05']}`",
        "",
        "## Recipe Status",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "- Grade 4 / G-unit cards must stay out of the main deck until G Zone support exists.",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate M68-03 ninth-slice recipe drafts.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_ninth_slice_validation_report()
    json_path = args.output_dir / "m68_04_ninth_slice_recipe_validation_report.json"
    md_path = args.output_dir / "m68_04_ninth_slice_recipe_validation_report.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M68-04 ninth-slice recipe validation wrote {json_path}")
    print(f"M68-04 ninth-slice recipe validation summary wrote {md_path}")
    print(
        "ready_for_m68_05={ready} runtime_ready={runtime_ready} manual_blocked={manual_blocked} grade4={grade4}".format(
            ready=report["summary"]["ready_for_m68_05"],
            runtime_ready=report["summary"]["runtime_ready_recipe_count"],
            manual_blocked=report["summary"]["blocked_by_manual_review_count"],
            grade4=report["summary"]["grade4_main_deck_recipe_count"],
        )
    )
    return 0 if report["summary"]["ready_for_m68_05"] else 1
