"""Build the M48-02 fourth-slice review packet."""
...
def _build_pipeline_details(selection_report: dict[str, Any], applied_scope_report: dict[str, Any]) -> dict[str, Any]:
...
def _scaffold_note_item(scaffold_report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "item_id": "m48_01_fixture_scaffold",
        ...
        "review_reasons": [
            ...
            "g_era_grade4_g_zone_boundary_review",
            "must_review_before_recipe_validator",
        ],
        "blocked_until": [
            "fixture_scaffold_reviewed",
            "m48_04_recipe_validator_uses_scaffold",
...
def _candidate_edge_item(edge: dict[str, Any], rank: int) -> dict[str, Any]:
    return {
        ...
        "review_reasons": [
            "fourth_slice_candidate_compatibility_edge",
            "requires_human_recipe_selection_before_m48_03",
        ],
        "blocked_until": [
            "human_recipe_selection",
            "m48_04_recipe_validator",
            "combo_line_to_recipe_consistency_check",
        ],
        "recommended_next_action": "consider_for_m48_03_advisory_recipe_draft",
    }
...
def build_fourth_slice_review_packet(
    ...
    selection_report = selection_report or load_json(M47_01_SELECTION)
    applied_scope_report = applied_scope_report or load_json(M47_APPLIED_SCOPE)
    probe_report = probe_report or load_json(M47_03_PROBE)
    gate_report = gate_report or load_json(M47_04_GATE)
    scaffold_report = scaffold_report or load_json(M48_01_SCAFFOLD)
    pipeline = _build_pipeline_details(selection_report, applied_scope_report)
    ...
    gate_allows_offline = bool(gate_report.get("summary", {}).get("ready_for_m48"))
    scaffold_ready = bool(scaffold_report.get("summary", {}).get("ready_for_m48_02"))
    ...
    return {
        "version": "M48-02",
        "description": "Fourth-slice review packet before advisory deck recipe drafting",
        "selected_target": selection_report.get("selected_target", {}),
        "source_inputs": {
            "fourth_target_slice_selection": _rel(M47_01_SELECTION),
            "fourth_slice_applied_scope": _rel(M47_APPLIED_SCOPE),
            "fourth_slice_semantic_compatibility_probe": _rel(M47_03_PROBE),
            "fourth_slice_recipe_pipeline_entry_gate": _rel(M47_04_GATE),
            "fourth_slice_fixture_scaffold": _rel(M48_01_SCAFFOLD),
        },
        "scope": {
        ...
        "review_policy": {
            ...
            "g_zone_and_grade4_require_manual_review": True,
            "no_live_card_text_parsing": True,
            "no_direct_GameState_mutation": True,
        },
        "evidence_summary": {
            "m47_04_offline_recipe_pipeline_allowed": gate_allows_offline,
            "m47_04_runtime_promotion_closed": promotion_closed,
            "m48_01_scaffold_ready": scaffold_ready,
        ...
        "next_target": {
            ...
            "blocked_until": [
                "M48-02 review packet is available",
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M48-03: `{summary['ready_for_m48_03']}`",
        "",
        "## Fixture Scaffold",
        "",
    ]
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M48-02 fourth-slice review packet.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    json_path = args.output_dir / "m48_02_fourth_slice_review_packet.json"
    md_path = args.output_dir / "m48_02_fourth_slice_review_packet.md"
    csv_path = args.output_dir / "m48_02_fourth_slice_review_packet.csv"
    write_json(report, json_path)
    write_markdown(report, md_path)
    write_csv(report, csv_path)
    print(f"M48-02 review packet wrote {json_path}")
    print(f"M48-02 review packet summary wrote {md_path}")
    print(f"M48-02 review packet CSV wrote {csv_path}")
    print(
        "ready={ready} scaffold={scaffold} manual={manual} candidates={candidates}".format(
            ready=report["summary"]["ready_for_m48_03"],
    ...
    return 0 if report["summary"]["ready_for_m48_03"] else 1
