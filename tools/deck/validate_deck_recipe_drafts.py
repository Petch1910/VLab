"""Validate M36-02 deck recipe drafts for M36-03.

The validator is offline. It checks card existence, quantity bounds, clan,
main/trigger counts, grade profile, and review blockers without creating
runtime decks.
"""
...
def _validate_recipe(recipe: dict[str, Any], card_rows: dict[str, dict[str, Any]], expected_clan: str) -> dict[str, Any]:
    ...
    for row in recipe.get("quantities", []):
        ...
        if quantity <= 0:
            issues.append(_issue("non_positive_quantity", "blocker", "Card quantity must be positive.", {"card_id": card_id, "quantity": quantity}))
        if quantity > deck_limit:
            issues.append(_issue("copy_limit_exceeded", "blocker", "Card quantity exceeds deck_limit.", {"card_id": card_id, "quantity": quantity, "deck_limit": deck_limit}))
    ...
    if missing_cards:
        issues.append(_issue("missing_cards", "blocker", "Recipe contains card ids missing from SQLite.", {"card_ids": sorted(missing_cards)}))

    slot_summary = recipe.get("slot_summary", {})
    target_total = int(slot_summary.get("main_deck_target", 50))
    if explicit_total != target_total:
        issues.append(_issue("main_deck_size_mismatch", "blocker", "Explicit quantities do not equal main deck target.", {"expected": target_total, "actual": explicit_total}))
    if int(slot_summary.get("total_unfilled_slots", 0)) > 0:
        issues.append(_issue("unfilled_slots", "blocker", "Draft still has unfilled slots.", {"unfilled": slot_summary.get("total_unfilled_slots")}))

    trigger_total = sum(trigger_counts.values())
    if trigger_total != 16:
        issues.append(_issue("trigger_count_mismatch", "blocker", "Classic trigger count must be 16 before recipe can be promoted.", {"expected": 16, "actual": trigger_total, "by_trigger": dict(trigger_counts)}))
    if trigger_counts.get("Heal", 0) > 4:
        issues.append(_issue("heal_trigger_limit_exceeded", "blocker", "Heal trigger count exceeds classic guardrail.", {"heal_count": trigger_counts["Heal"]}))

    off_clan = {clan: count for clan, count in clan_counts.items() if clan and clan != expected_clan}
    if off_clan:
        issues.append(_issue("clan_mismatch", "blocker", "Recipe contains cards outside selected clan.", {"expected_clan": expected_clan, "off_clan_counts": off_clan}))
    ...
    if grade_mismatches:
        issues.append(_issue("grade_profile_review", "review", "Grade profile differs from advisory target.", grade_mismatches))

    if recipe.get("review_status", "").startswith("blocked_rejected_line"):
        issues.append(_issue("review_status_blocked", "blocker", "Recipe source line is still rejected by M36-01.", {"review_status": recipe.get("review_status")}))
    elif "human_acceptance" in recipe.get("review_blockers", []):
        issues.append(_issue("human_acceptance_pending", "review", "Accepted seed still requires human acceptance.", {}))

    blocker_count = sum(1 for issue in issues if issue["severity"] == "blocker")
    if any(issue["code"] == "review_status_blocked" for issue in issues):
        status = "blocked_by_review"
    elif blocker_count:
        status = "invalid_draft"
    elif any(issue["code"] == "human_acceptance_pending" for issue in issues):
        status = "validator_passed_pending_human_acceptance"
    else:
    ...
    return {
        ...
        "source_skeleton_id": recipe.get("source_skeleton_id", ""),
...
def build_validation_report(
    ...
    return {
        "version": "M36-03",
        "description": "Deck recipe validator for M36-02 draft recipes",
        ...
        "validation_policy": {
            ...
            "review_blockers_are_blocking": True,
        },
        "summary": {
            ...
            "validator_passed_pending_human_acceptance_count": status_counts.get("validator_passed_pending_human_acceptance", 0),
            "invalid_draft_count": status_counts.get("invalid_draft", 0),
            "blocked_by_review_count": status_counts.get("blocked_by_review", 0),
            "missing_card_recipe_count": sum(1 for item in validations if any(issue["code"] == "missing_cards" for issue in item["issues"])),
            "copy_limit_violation_recipe_count": sum(1 for item in validations if any(issue["code"] == "copy_limit_exceeded" for issue in item["issues"])),
            "slot_gap_recipe_count": sum(1 for item in validations if any(issue["code"] == "unfilled_slots" for issue in item["issues"])),
            "trigger_count_mismatch_recipe_count": sum(1 for item in validations if any(issue["code"] == "trigger_count_mismatch" for issue in item["issues"])),
            "issue_counts": dict(sorted(issue_counts.items())),
            "ready_for_m36_04": bool(validations),
        },
        "recipe_validations": validations,
        "next_target": {
            "milestone": "M36-04",
            "task": "Combo-line to recipe consistency check",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Passed pending human acceptance: `{summary['validator_passed_pending_human_acceptance_count']}`",
        f"- Invalid drafts: `{summary['invalid_draft_count']}`",
        f"- Blocked by review: `{summary['blocked_by_review_count']}`",
        ...
        f"- Ready for M36-04: `{summary['ready_for_m36_04']}`",
        "",
        "## Recipe Status",
        "",
    ]
    for item in report["recipe_validations"]:
        ...
        lines.append(
            f"- `{item['recipe_id']}` line=`{item['source_line_id']}` "
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate M36-02 deck recipe drafts.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    json_path = args.output_dir / "m36_03_deck_recipe_validation_report.json"
    md_path = args.output_dir / "m36_03_deck_recipe_validation_report.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M36-03 deck recipe validation wrote {json_path}")
    print(f"M36-03 deck recipe validation summary wrote {md_path}")
    print(
        "ready_for_m36_04={ready} runtime_ready={runtime_ready} invalid={invalid} blocked={blocked}".format(
            ready=report["summary"]["ready_for_m36_04"],
            runtime_ready=report["summary"]["runtime_ready_recipe_count"],
            invalid=report["summary"]["invalid_draft_count"],
            blocked=report["summary"]["blocked_by_review_count"],
        )
    )
    return 0 if report["summary"]["ready_for_m36_04"] else 1
