"""Build M56-03 sixth-slice advisory recipe drafts."""
...
M56_REVIEW_PACKET = OUTPUT_DIR / "m56_02_sixth_slice_review_packet.json"
        ...
        rows.append(
            {
                ...
                "quantity_source": "m56_03_fixture_scaffold_plus_candidate_edge_pair",
            }
        )
    return rows
...
def _build_recipe(
    ...
    return {
        ...
        "validation_metadata": {
            "draft_only": True,
            "requires_m56_04_validator": True,
...
def build_sixth_slice_recipe_drafts(
    review_packet: dict[str, Any] | None = None,
    scaffold: dict[str, Any] | None = None,
    draft_limit: int = DEFAULT_DRAFT_LIMIT,
) -> dict[str, Any]:
    review_packet = review_packet or load_json(M56_REVIEW_PACKET)
    scaffold = scaffold or load_json(M56_SCAFFOLD)
    if not review_packet.get("summary", {}).get("ready_for_m56_03"):
        raise ValueError("M56-02 review packet is not ready for M56-03.")
    if not scaffold.get("summary", {}).get("ready_for_m56_02"):
        raise ValueError("M56-01 fixture scaffold is not ready for M56-03.")
    ...
    return {
        "version": "M56-03",
        "description": "Sixth-slice advisory recipe draft model from M56-02 candidate edges",
        "selected_target": review_packet.get("selected_target", {}),
        "source_inputs": {
            "sixth_slice_review_packet": str(M56_REVIEW_PACKET.relative_to(ROOT)),
        ...
        "draft_policy": {
            "pair_anchored_from_m56_02_candidate_edges": True,
            "fixture_scaffold_source": "m56_01_sixth_slice_fixture_scaffold",
        ...
        "summary": {
            ...
            "ready_for_m56_04": bool(recipes) and quantity_complete_count == len(recipes),
        },
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M56-04: `{summary['ready_for_m56_04']}`",
        "",
        "## Drafts",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "- M56-04 must validate deck legality and manual-review blockers.",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M56-03 sixth-slice recipe draft model.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_sixth_slice_recipe_drafts(draft_limit=args.draft_limit)
    json_path = args.output_dir / "m56_03_sixth_slice_recipe_draft_model.json"
    md_path = args.output_dir / "m56_03_sixth_slice_recipe_draft_model.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M56-03 recipe draft model wrote {json_path}")
    print(f"M56-03 recipe draft model summary wrote {md_path}")
    print(
        "ready={ready} drafts={drafts} complete={complete} skipped={skipped}".format(
            ready=report["summary"]["ready_for_m56_04"],
    ...
    return 0 if report["summary"]["ready_for_m56_04"] else 1
