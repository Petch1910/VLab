"""Build the M70-04 nine-fixture scale decision artifact."""
...
from tools.deck.build_eight_fixture_scale_decision import M66_03_SMOKE  # noqa: E402
...
from tools.deck.build_seven_fixture_scale_decision import M62_03_SMOKE  # noqa: E402
from tools.deck.build_six_fixture_scale_decision import M58_03_SMOKE  # noqa: E402


OUTPUT_DIR = ROOT / "outputs" / "target_slice"
M70_03_SMOKE = OUTPUT_DIR / "m70_03_ninth_fixture_load_smoke.json"
JSON_OUTPUT = OUTPUT_DIR / "m70_04_nine_fixture_scale_decision.json"
MD_OUTPUT = OUTPUT_DIR / "m70_04_nine_fixture_scale_decision.md"
...
def build_nine_fixture_scale_decision(
    ...
    seventh_smoke: dict[str, Any] | None = None,
    eighth_smoke: dict[str, Any] | None = None,
    ninth_smoke: dict[str, Any] | None = None,
    ranking: dict[str, Any] | None = None,
    sixth_fixture: dict[str, Any] | None = None,
    seventh_fixture: dict[str, Any] | None = None,
    eighth_fixture: dict[str, Any] | None = None,
    ninth_fixture: dict[str, Any] | None = None,
) -> dict[str, Any]:
    ...
    seventh_smoke = seventh_smoke or load_json(M62_03_SMOKE)
    eighth_smoke = eighth_smoke or load_json(M66_03_SMOKE)
    ninth_smoke = ninth_smoke or load_json(M70_03_SMOKE)
    ...
    fixtures = [
        ...
        seventh_fixture if seventh_fixture is not None else _load_fixture_from_smoke(seventh_smoke),
        eighth_fixture if eighth_fixture is not None else _load_fixture_from_smoke(eighth_smoke),
        ninth_fixture if ninth_fixture is not None else _load_fixture_from_smoke(ninth_smoke),
    ]
    fixture_evidence = [
        ...
        _fixture_evidence("seventh_fixture_neo_nectar", seventh_smoke),
        _fixture_evidence("eighth_fixture_kagero", eighth_smoke),
        _fixture_evidence("ninth_fixture_aqua_force", ninth_smoke),
    ]
    ...
    scale_ready = not failed and len(fixture_evidence) == 9

    return {
        "version": "M70-04",
        "description": "Nine-fixture scale decision",
        "source_inputs": {
            ...
            "seventh_fixture_headless_smoke": str(M62_03_SMOKE.relative_to(ROOT)),
            "eighth_fixture_headless_smoke": str(M66_03_SMOKE.relative_to(ROOT)),
            "ninth_fixture_headless_smoke": str(M70_03_SMOKE.relative_to(ROOT)),
            "archetype_priority_ranking": str(ARCHETYPE_RANKING.relative_to(ROOT)),
        },
        "scope": {
            "offline_scale_decision": True,
            "post_nine_fixture_queue_gate": True,
            "selects_runtime_deck": False,
            "selects_tenth_slice": False,
        ...
        "decision": {
            "post_m70_queue_review_ready": scale_ready,
            "tenth_slice_selected_now": False,
            "recommended_next_action": "M71-01 post-nine fixture queue planning" if scale_ready else "repair_failed_fixture_evidence",
            ...
            "mate_identity_check_enabled": False,
            "decision_notes": [
                "Nine independent fixtures pass offline and Unity headless load smoke.",
                "The next step should define the post-M70 queue before selecting any further slice.",
                "Do not publish fixture decks to UI or bot runtime without a later explicit gate.",
                "G Zone, Stride, Aqua Force battle-order, Bloom/token, Lock, Unlock, Legion, and Mate runtime remain disabled.",
            ]
            if scale_ready
            else [
                "At least one fixture smoke is not ready for scale-out.",
                "Repair failed evidence before defining another queue.",
            ],
        },
        "summary": {
            ...
            "post_m70_queue_review_ready": scale_ready,
            "ready_for_m71_planning": scale_ready,
        },
        "next_target": {
            "milestone": "M71-01" if scale_ready else "M70-repair",
            "task": "Post-nine fixture queue planning" if scale_ready else "Repair failed fixture evidence",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Post-M70 queue review ready: `{summary['post_m70_queue_review_ready']}`",
        f"- Ready for M71 planning: `{summary['ready_for_m71_planning']}`",
        "",
        "## Fixture Evidence",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "- It does not select a tenth slice.",
            ...
            "- It does not enable G Zone, Stride, Aqua Force battle-order, Bloom/token, Lock, Unlock, Legion, or Mate runtime.",
...
def parse_args(argv: Any = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M70-04 nine-fixture scale decision.")
...
def main(argv: Any = None) -> int:
    ...
    print(f"M70-04 nine-fixture scale decision wrote {json_path}")
    print(f"M70-04 nine-fixture scale decision summary wrote {md_path}")
    print(
        "ready_for_m71_planning={ready} fixtures_passed={passed} candidates={candidates}".format(
            ready=report["summary"]["ready_for_m71_planning"],
            passed=report["summary"]["passed_fixture_count"],
            candidates=report["summary"]["candidate_count"],
        )
    )
    return 0 if report["summary"]["ready_for_m71_planning"] else 1
