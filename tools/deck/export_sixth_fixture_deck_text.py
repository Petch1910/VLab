"""Export the sixth offline runtime fixture as count-line deck text (M58-02)."""
...
DEFAULT_FIXTURE = OUTPUT_DIR / "runtime_fixtures" / "m56_recipe_001_shadow_paladin_m57_06.json"
DEFAULT_VALIDATION_REPORT = OUTPUT_DIR / "m58_01_sixth_fixture_schema_validation.json"
CARDS_SQLITE = ROOT / "data" / "packs" / "vanguard_th" / "cards.sqlite"
PACK_MANIFEST = ROOT / "data" / "packs" / "vanguard_th" / "manifest.json"

TEXT_OUTPUT = OUTPUT_DIR / "m58_02_sixth_fixture_deck_text_export.txt"
JSON_OUTPUT = OUTPUT_DIR / "m58_02_sixth_fixture_deck_text_export.json"
MD_OUTPUT = OUTPUT_DIR / "m58_02_sixth_fixture_deck_text_export.md"
...
def _is_validation_ready(validation_report: dict[str, Any]) -> bool:
    ...
    return bool(summary.get("schema_valid")) and bool(summary.get("ready_for_m58_02"))
...
def _build_deck_text(
    ...
    era_preset = str(format_policy.get("era_preset") or selected_target.get("era_preset") or "g_next_z")
    slice_name = str(format_policy.get("slice") or selected_target.get("slice") or "Sixth Offline Slice")
    group_name = str(format_policy.get("group") or selected_target.get("group") or "Shadow Paladin")

    lines = [
        "# Vanguard Thai Sim Deck List",
        "# M58-02 review-only export from an offline runtime fixture.",
        f"# FixtureId: {fixture_id}",
        f"# RecipeId: {recipe_id}",
        f"# Slice: {slice_name}",
        f"# Group: {group_name}",
        f"# GZoneBoundary: {g_zone_boundary.get('selected_option_id') or ''}",
        "# Scope: review_only_offline_fixture",
        "Name: G NEXT/Z Shadow Paladin Fixture (" + recipe_id + ")",
        "Format: " + era_preset,
    ...
    lines.extend(["", "[Ride]", "", "[G]"])
    if g_zone_boundary.get("g_zone_runtime_enabled") is False and g_zone_boundary.get("stride_runtime_enabled") is False:
        lines.append("# Empty: G Zone and Stride runtime are disabled for this fixture.")
    lines.append("")
    return "\n".join(lines).rstrip() + "\n"


def build_sixth_fixture_deck_text_export(
    ...
    if not _is_validation_ready(validation_report):
        issues.append(
            _issue(
                "m58_01_validation_not_ready",
                "blocker",
                "M58-01 validation report must be schema-valid and ready before deck text export.",
                {"summary": validation_report.get("summary", {})},
            )
        )
    ...
    return {
        "version": "M58-02",
        "description": "Sixth fixture deck text exporter",
        "source_inputs": {
            "runtime_fixture": str(DEFAULT_FIXTURE.relative_to(ROOT)),
            "m58_01_validation_report": str(DEFAULT_VALIDATION_REPORT.relative_to(ROOT)),
        ...
        "scope": {
            ...
            "sixth_fixture_deck_text_export": True,
        ...
        "summary": {
            ...
            "ready_for_m58_03": export_ready,
        },
        "next_target": {
            "milestone": "M58-03",
            "task": "Sixth fixture headless load smoke",
        },
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        "# M58-02 Sixth Fixture Deck Text Export",
        "",
        "## Summary",
        "",
        ...
        f"- Ready for M58-03: `{summary['ready_for_m58_03']}`",
        "",
        "## Runtime Boundary",
        "",
        "- Review text only.",
    ...
    lines.extend(
        [
            ...
            "## Next",
            "",
            "`M58-03`: Sixth fixture headless load smoke.",
            "",
        ]
    )
    path.parent.mkdir(parents=True, exist_ok=True)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export M58-02 sixth fixture count-line deck text.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_sixth_fixture_deck_text_export(fixture, validation_report)
    report["source_inputs"]["runtime_fixture"] = str(args.fixture)
    report["source_inputs"]["m58_01_validation_report"] = str(args.validation_report)
    ...
    print(f"M58-02 sixth fixture deck text export wrote {json_path}")
    print(f"M58-02 sixth fixture deck text summary wrote {md_path}")
    if report["summary"]["export_ready"]:
        print(f"M58-02 sixth fixture deck text wrote {text_path}")
    print(
        "export_ready={ready} blockers={blockers} card_lines={lines} next={next_ready}".format(
            ...
            next_ready=report["summary"]["ready_for_m58_03"],
        )
    )
