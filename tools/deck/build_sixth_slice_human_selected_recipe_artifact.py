"""Build the M57-02 sixth-slice human-selected recipe artifact.

This tool intentionally has no default selected review item. A caller must pass
an explicit M57-01 review item id and non-empty selection text.
"""
...
def _find_review_item(review_packet: dict[str, Any], review_item_id: str) -> dict[str, Any]:
    if not review_item_id.strip():
        raise ValueError("M57-02 requires an explicit non-empty review_item_id.")
    matches = [
        item
    ...
    if not matches:
        raise ValueError(f"M57-01 review item not found: {review_item_id}")
    if len(matches) > 1:
        raise ValueError(f"M57-01 review item id is not unique: {review_item_id}")
    return matches[0]
...
def _require_selection_text(selection_text: str) -> str:
    ...
    if not normalized:
        raise ValueError("M57-02 requires non-empty selection_text.")
    return normalized
...
def build_sixth_slice_human_selected_recipe_artifact(
    ...
    *,
    ...
    ready_for_m57_03 = (
        bool(review_packet.get("summary", {}).get("ready_for_m57_02"))
    ...
    return {
        "version": "M57-02",
        "description": "Sixth-slice human-selected recipe artifact",
        ...
        "selection_policy": {
            ...
            "m57_04_must_record_explicit_g_zone_decision": True,
        ...
        "selection": {
            ...
            "selected_action": "select_recipe_for_m57_03_acceptance_review",
            ...
            "g_zone_can_be_repaired_in_m57_01": bool(selected_item.get("g_zone_can_be_repaired_in_m57_01")),
        ...
        "summary": {
            ...
            "runtime_promotion_allowed": False,
            "g_zone_deferred": bool(selected_item.get("g_zone_deferred")),
            "ready_for_m57_03": ready_for_m57_03,
        },
        "next_target": {
            "milestone": "M57-03",
            "task": "Sixth-slice human-accepted repair artifact",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M57-03: `{summary['ready_for_m57_03']}`",
        "",
        "## Selection",
        "",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M57-02 sixth-slice human-selected recipe artifact.")
    parser.add_argument("--review-item-id", required=True, help="Explicit M57-01 review_item_id selected by the user.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_sixth_slice_human_selected_recipe_artifact(
        selected_review_item_id=args.review_item_id,
        selection_text=args.selection_text,
        selected_by=args.selected_by,
        selected_at=args.selected_at,
    )
    json_path = args.output_dir / "m57_02_sixth_slice_human_selected_recipe_artifact.json"
    md_path = args.output_dir / "m57_02_sixth_slice_human_selected_recipe_artifact.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M57-02 sixth-slice human-selected recipe artifact wrote {json_path}")
    print(f"M57-02 sixth-slice human-selected recipe artifact summary wrote {md_path}")
    print(
        "ready_for_m57_03={ready} selected_recipe={recipe}".format(
            ready=report["summary"]["ready_for_m57_03"],
            recipe=report["summary"]["selected_recipe_id"],
        )
    )
    return 0 if report["summary"]["ready_for_m57_03"] else 1
