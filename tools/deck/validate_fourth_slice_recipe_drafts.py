"""Validate M48-03 fourth-slice recipe drafts for M48-04."""
...
RECIPE_DRAFTS = OUTPUT_DIR / "m48_03_fourth_slice_recipe_draft_model.json"
    ...
    if manual_ids:
        issues.append(
            _issue(
                ...
                "blocker",
                "Recipe contains cards that M48-02 marked as manual-review required.",
...
def build_fourth_slice_validation_report(
    ...
    return {
        "version": "M48-04",
        "description": "Fourth-slice recipe validator for M48-03 advisory drafts",
        "selected_target": recipe_report.get("selected_target", {}),
        "source_inputs": {
            "fourth_slice_recipe_draft_model": str(RECIPE_DRAFTS.relative_to(ROOT)),
        ...
        "summary": {
            ...
            "ready_for_m48_05": bool(validations),
        },
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M48-05: `{summary['ready_for_m48_05']}`",
        "",
        "## Recipe Status",
        "",
    ]
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate M48-03 fourth-slice recipe drafts.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_fourth_slice_validation_report()
    json_path = args.output_dir / "m48_04_fourth_slice_recipe_validation_report.json"
    md_path = args.output_dir / "m48_04_fourth_slice_recipe_validation_report.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M48-04 fourth-slice recipe validation wrote {json_path}")
    print(f"M48-04 fourth-slice recipe validation summary wrote {md_path}")
    print(
        "ready_for_m48_05={ready} runtime_ready={runtime_ready} manual_blocked={manual_blocked} grade4={grade4}".format(
            ready=report["summary"]["ready_for_m48_05"],
    ...
    return 0 if report["summary"]["ready_for_m48_05"] else 1
