"""Export the third offline runtime fixture as count-line deck text (M46-02)."""
...
DEFAULT_VALIDATION_REPORT = OUTPUT_DIR / "m46_01_third_fixture_schema_validation.json"
CARDS_SQLITE = ROOT / "data" / "packs" / "vanguard_th" / "cards.sqlite"
PACK_MANIFEST = ROOT / "data" / "packs" / "vanguard_th" / "manifest.json"

TEXT_OUTPUT = OUTPUT_DIR / "m46_02_third_fixture_deck_text_export.txt"
JSON_OUTPUT = OUTPUT_DIR / "m46_02_third_fixture_deck_text_export.json"
MD_OUTPUT = OUTPUT_DIR / "m46_02_third_fixture_deck_text_export.md"
...
def _is_validation_ready(validation_report: dict[str, Any]) -> bool:
    ...
    return bool(summary.get("schema_valid")) and bool(summary.get("ready_for_m46_02"))
...
def _build_deck_text(
    ...
    era_preset = str(format_policy.get("era_preset") or "link_joker_legion_mate")
    slice_name = str(format_policy.get("slice") or selected_target.get("slice") or "Third Offline Slice")

    lines = [
        "# Vanguard Thai Sim Deck List",
        "# M46-02 review-only export from an offline runtime fixture.",
        f"# FixtureId: {fixture_id}",
        f"# RecipeId: {recipe_id}",
        f"# Slice: {slice_name}",
        "# Scope: review_only_offline_fixture",
        "Name: Link Joker Legion Bermuda Triangle Fixture (" + recipe_id + ")",
        "Format: " + era_preset,
...
def build_third_fixture_deck_text_export(
    ...
    if not _is_validation_ready(validation_report):
        issues.append(
            _issue(
                "m46_01_validation_not_ready",
                "blocker",
                "M46-01 validation report must be schema-valid and ready before deck text export.",
                {"summary": validation_report.get("summary", {})},
            )
        )
    ...
    return {
        "version": "M46-02",
        "description": "Third fixture deck text exporter",
        "source_inputs": {
            "runtime_fixture": str(DEFAULT_FIXTURE.relative_to(ROOT)),
            "m46_01_validation_report": str(DEFAULT_VALIDATION_REPORT.relative_to(ROOT)),
        ...
        "scope": {
            ...
            "third_fixture_deck_text_export": True,
        ...
        "summary": {
            ...
            "ready_for_m46_03": export_ready,
        },
        "next_target": {
            "milestone": "M46-03",
            "task": "Third fixture headless load smoke",
        },
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        "# M46-02 Third Fixture Deck Text Export",
        "",
        "## Summary",
        "",
        ...
        f"- Ready for M46-03: `{summary['ready_for_m46_03']}`",
        "",
        "## Runtime Boundary",
        "",
        "- Review text only.",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export M46-02 third fixture count-line deck text.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_third_fixture_deck_text_export(fixture, validation_report)
    report["source_inputs"]["runtime_fixture"] = str(args.fixture)
    report["source_inputs"]["m46_01_validation_report"] = str(args.validation_report)
    ...
    print(f"M46-02 third fixture deck text export wrote {json_path}")
    print(f"M46-02 third fixture deck text summary wrote {md_path}")
    if report["summary"]["export_ready"]:
        print(f"M46-02 third fixture deck text wrote {text_path}")
    print(
        "export_ready={ready} blockers={blockers} card_lines={lines} next={next_ready}".format(
            ...
            next_ready=report["summary"]["ready_for_m46_03"],
        )
    )
