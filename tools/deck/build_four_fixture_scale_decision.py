"""Build the M50-04 four-fixture scale decision artifact."""
...
M50_03_SMOKE = OUTPUT_DIR / "m50_03_fourth_fixture_load_smoke.json"
...
def _fixture_evidence(label: str, smoke: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "g_zone_count": fixture.get("g_zone_count", 0),
...
def build_four_fixture_scale_decision(
    ...
    scale_allowed = not failed and len(fixture_evidence) == 4 and bool(candidates)

    return {
        "version": "M50-04",
        "description": "Four-fixture scale decision",
        "source_inputs": {
        ...
        "decision": {
            "fifth_slice_offline_pipeline_allowed": scale_allowed,
            "fifth_slice_selected_now": False,
            "recommended_next_action": "M51-01 fifth target slice selection" if scale_allowed else "repair_failed_fixture_evidence",
            ...
            "stride_runtime_enabled": False,
            "decision_notes": [
                "Four independent fixtures now pass offline and Unity headless load smoke.",
                "The next step may select a fifth slice for offline analysis only.",
                "Do not publish fixture decks to UI or bot runtime without a later explicit gate.",
                "G Zone and Stride runtime remain disabled despite the fourth G-series source fixture.",
            ]
            if scale_allowed
            else [
        ...
        "summary": {
            ...
            "fifth_slice_offline_pipeline_allowed": scale_allowed,
            "ready_for_m51": scale_allowed,
        },
        "next_target": {
            "milestone": "M51-01" if scale_allowed else "M50-repair",
            "task": "Fifth target slice selection" if scale_allowed else "Repair failed fixture evidence",
        },
    }
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Fifth-slice offline pipeline allowed: `{summary['fifth_slice_offline_pipeline_allowed']}`",
        f"- Ready for M51: `{summary['ready_for_m51']}`",
        "",
        "## Fixture Evidence",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "## Decision",
            "",
            f"- Fifth slice selected now: `{decision['fifth_slice_selected_now']}`",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M50-04 four-fixture scale decision.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    json_path = args.output_dir / "m50_04_four_fixture_scale_decision.json"
    md_path = args.output_dir / "m50_04_four_fixture_scale_decision.md"
    write_json(report, json_path)
    write_markdown(report, md_path)
    print(f"M50-04 four-fixture scale decision wrote {json_path}")
    print(f"M50-04 four-fixture scale decision summary wrote {md_path}")
    print(
        "ready_for_m51={ready} fixtures_passed={passed} candidates={candidates}".format(
            ready=report["summary"]["ready_for_m51"],
            passed=report["summary"]["passed_fixture_count"],
            candidates=report["summary"]["candidate_count"],
        )
    )
    return 0 if report["summary"]["ready_for_m51"] else 1
