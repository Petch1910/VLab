"""Build and verify the M62-03 seventh fixture headless load smoke artifacts."""
...
DEFAULT_M62_02_REPORT = OUTPUT_DIR / "m62_02_seventh_fixture_deck_text_export.json"
DEFAULT_DECK_TEXT = OUTPUT_DIR / "m62_02_seventh_fixture_deck_text_export.txt"
CARDS_SQLITE = ROOT / "data" / "packs" / "vanguard_th" / "cards.sqlite"

DECK_CODE_OUTPUT = OUTPUT_DIR / "m62_03_seventh_fixture_deck_code.txt"
JSON_OUTPUT = OUTPUT_DIR / "m62_03_seventh_fixture_load_smoke.json"
MD_OUTPUT = OUTPUT_DIR / "m62_03_seventh_fixture_load_smoke.md"
DEFAULT_UNITY_RESULT = OUTPUT_DIR / "m62_03_seventh_fixture_unity_result.json"
DEFAULT_UNITY_REPLAY = OUTPUT_DIR / "m62_03_seventh_fixture_unity_replay.json"
...
def _build_deck_object(parsed: dict[str, Any]) -> dict[str, Any]:
    ...
    deck_id = "m6203" + hashlib.sha256(digest_source.encode("utf-8")).hexdigest()[:26]
    return {
        "deck_id": deck_id,
        "name": metadata["Name"],
...
def build_seventh_headless_fixture_load_smoke(
    fixture: dict[str, Any] | None = None,
    m62_02_report: dict[str, Any] | None = None,
    ...
    m62_02_report = m62_02_report or load_json(DEFAULT_M62_02_REPORT)
    deck_text = deck_text if deck_text is not None else DEFAULT_DECK_TEXT.read_text(encoding="utf-8")
    issues: list[dict[str, Any]] = []

    if not m62_02_report.get("summary", {}).get("export_ready"):
        issues.append(
            _issue(
                "m62_02_export_not_ready",
                "blocker",
                "M62-02 deck text export must be ready before headless load smoke.",
                {"summary": m62_02_report.get("summary", {})},
            )
        )
    ...
    if parsed["zones"]["G"]:
        issues.append(_issue("g_zone_not_empty", "blocker", "Seventh fixture G section must stay empty.", {}))
    ...
    ready_for_m62_04 = offline_load_ready and unity_smoke_passed

    return {
        "version": "M62-03",
        "description": "Seventh fixture headless load smoke",
        "source_inputs": {
            "runtime_fixture": str(DEFAULT_FIXTURE.relative_to(ROOT)),
            "m62_02_report": str(DEFAULT_M62_02_REPORT.relative_to(ROOT)),
        ...
        "scope": {
            ...
            "seventh_fixture_headless_load_smoke": True,
        ...
        "summary": {
            ...
            "ready_for_m62_04": ready_for_m62_04,
        },
        "next_target": {
            "milestone": "M62-04",
            "task": "Seven-fixture scale decision",
        },
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        ...
        f"- Ready for M62-04: `{summary['ready_for_m62_04']}`",
        "",
        "## Unity Headless",
        "",
        ...
        "- No Bloom/token runtime enablement.",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build M62-03 seventh fixture headless load smoke artifacts.")
    parser.add_argument("--fixture", type=Path, default=DEFAULT_FIXTURE)
    parser.add_argument("--m62-02-report", type=Path, default=DEFAULT_M62_02_REPORT)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    m62_02_report = load_json(args.m62_02_report)
    deck_text = args.deck_text.read_text(encoding="utf-8")
    report = build_seventh_headless_fixture_load_smoke(
        fixture,
        m62_02_report,
        deck_text,
        args.unity_result,
    ...
    report["source_inputs"]["m62_02_report"] = str(args.m62_02_report)
    ...
    print(f"M62-03 seventh fixture headless load smoke wrote {json_path}")
    print(f"M62-03 seventh fixture headless load smoke summary wrote {md_path}")
    if report["summary"]["deck_code_created"]:
        print(f"M62-03 seventh fixture deck code wrote {deck_code_path}")
    print(
        "offline_ready={offline} unity_provided={provided} unity_passed={passed} blockers={blockers} next={next_ready}".format(
            ...
            next_ready=report["summary"]["ready_for_m62_04"],
        )
    )
